(function(){
    var Data = {
        ...
        getPinyin : function(word){
            var oResult = Data.getResult(word);
            return oResult.p;
        },
        ...
        getResult : function(words){
            ...
            for(i = 0; i<end ; i++){
                ...
                oResult = Data.getMutiPinyin(sWord);
                if(!oResult){
                    oResult = Data.getWord(sWord);
                }
                aP.push(oResult.p);
                aL.push(oResult.l);
                length = length * oResult.p.length;
            }
            for(j = 0; j<length; j++){
                for(i = 0; i<end; i++){
                    l = j%aP[i].length;
        ...
        getMutiPinyin: function(word){
            var oResult = {
                p : "",//拼音
                l : ""//首字母
            };
            var db = Data.mutiPinyin, i = 0;
            if(db[word] !== undefined){
                oResult.p = [];
                oResult.l = [];
                for(; i<db[word].length; i++){
                    oResult.l.push(db[word][i][0].toLowerCase());
                    oResult.p.push(db[word][i].toLowerCase());
                }
                return oResult;
            }else{
                return false;
    ...
    window.GetPinyin = Data;
})();
//console.log(GetPinyin.getResult('瓗'));
//console.log(GetPinyin.getFirstLetter('我keyi加123'));
//console.log(GetPinyin.getPinyin('我keyi加123'));