...
/**
 * location.href = "{php echo $this->createMobileUrl('order/pay')}&orderid=" + create_json.result.orderid +"&openid={$openid}&ischannelpay=" + create_json.result.ischannelpay;
 * 从这里跳过来
 */
//从 confirm.php 点击确认订单，跳转到pay.php中处理。
global $_W;
global $_GPC;
/** $_GPC
 * Array
...
if (($operation == 'display') && $_W['isajax']) {
	...
	if (is_weixin()) {		
		...
		//通莞支付  2017-11-01 
		$tgpay = array('success' => false);
		if (isset($set['pay']) && ($set['pay']['tgpaystatus'] == 1)) {		
...
//	http://shop.gogo198.cn/app/index.php?i=3&c=entry&p=pay&do=order&m=sz_yi&orderid=158&openid=ov3-btyLPTGwIduBvEXdiGSnpUK4&ischannelpay=0
	show_json(1, array('order' => $order, 'set' => $set, 'credit' => $credit, 'wechat' => $wechat, 'alipay' => $alipay, 'app_wechat' => $app_wechat, 'app_alipay' => $app_alipay, 'unionpay' => $unionpay, 'yunpay' => $yunpay, 'tgpay' => $tgpay, 'cash' => $cash, 'storecash' => $storecash, 'yeepay' => $yeepay, 'paypal' => $paypal, 'isweixin' => is_weixin(), 'currentcredit' => $currentcredit, 'returnurl' => $returnurl, 'goods' => $order_goods));
...
//show_json(1, array('order' => $order, 'set' => $set, 'credit' => $credit, 'wechat' => $wechat, 'alipay' => $alipay, 'app_wechat' => $app_wechat, 'app_alipay' => $app_alipay, 'unionpay' => $unionpay, 'yunpay' => $yunpay, 'cash' => $cash, 'storecash' => $storecash, 'yeepay' => $yeepay, 'paypal' => $paypal, 'isweixin' => is_weixin(), 'currentcredit' => $currentcredit, 'returnurl' => $returnurl, 'goods' => $order_goods));
	...
	if (($operation == 'pay') && $_W['ispost']) {
		...
		if (!in_array($type, array('weixin', 'alipay', 'app_alipay', 'app_weixin', 'unionpay', 'yunpay', 'yeepay', 'paypal', 'yeepay_wy','tgwechat'))) {
			show_json(0, '未找到支付方式');
		}
		...
		if ($type == 'weixin') {
			...
			if (strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger') !== false) {
				if (is_array($setting['payment'])) {
					...
					if (is_weixin()) {
						if (empty($set['pay']['weixin_jie'])) {
							...
							//请求 model/common/wechat_build 方法。接收返回的支付数组
				...
				//更新sz_yi_order 表中 paytype = 21 支付类型    $where_update（id= orderid)  uniacid = 公众号ID
				pdo_query('update ' . tablename('sz_yi_order') . ' set paytype=21 where ' . $where_update . ' and uniacid=:uniacid ', array(':uniacid' => $uniacid));
				show_json(1, array('wechat' => $wechat));//把微信请求的数组数据以json格式返回给页面前端！唤起支付
			}
			else {
				if (is_array($setting['payment'])) {
		...
		else if ($type == 'tgwechat') {

			if (empty($set['pay']['tgpaystatus'])) {
				show_json(0, '未开启微信支付!');
			}
			...
			$params['tid'] = $log['tid'];//订单编号
			if (!empty($order['ordersn2'])) {
				$var = sprintf('%02d', $order['ordersn2']);
				$params['tid'] .= 'GJ' . $var;
			}
			$params['user'] = 'om44I1X2wIlTWdN3pSsgoJkfMLtI';//$openid;//用户ID 2017-11-4
			$params['fee'] = $order['price'];//交易金额
			$params['title'] = $param_title;//描述
			load()->model('payment');
//			$setting = uni_setting($_W['uniacid'], array('payment'));
...
//						if (!empty($set['pay']['tgpay']) {
							...
							$options['account'] = $set['pay']['tgpay']['mchid'];
							$options['key'] = $set['pay']['tgpay']['key'];
							//请求 model/common/wechat_build 方法。接收返回的支付数组
							$tgwechat['payurl'] = m('common')->tgwechat($params, $options);
//							$tgwechat['name'] = $options;
//							$tgwechat['account'] = $options['account'];
//							$tgwechat['key'] = $options['key']''
//							echo $tgwechat;die;
							if (!is_error($tgwechat)) {
								$tgwechat['success'] = true;
							}else {
								show_json(0, $tgwechat['message']);
							}
//						}
					}
				}
				...
				show_json(1, array('wechat' => $tgwechat));//把微信请求的数组数据以json格式返回给页面前端！唤起支付
			}
		...
		if (($operation == 'complete') && $_W['ispost']) {
			...
			if (!in_array($type, array('weixin', 'alipay', 'credit', 'cash', 'storecash', 'paypal','tgwechat'))) {
				show_json(0, '未找到支付方式');
			}
