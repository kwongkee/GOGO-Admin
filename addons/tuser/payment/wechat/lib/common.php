...
class commons
{
	 ...
	 * 通莞微信公众号支付。
	 * */
	public function Tgwechat_jsapi($params, $wechat, $type = 0){
		echo '121adfdsf';die;
		global $_W;
		load()->func('communication');
		$wOpt = array();
		$package = array();
		$package['account'] = $wechat['account'];
		...
		$str .= 'key=' . $wechat['key'];
		$str = md5($str);//加密字串
		$package['sign'] = strtoupper($str);//返回加密字串转换成大写字母
		$data =  json_encode($package);//数据包转换成json格式
		//数据请求地址，post形式传输
		$response = ihttp_request('http://tgjf.833006.biz/tgPosp/payApi/wxJspay', $data);
	...
	/*-----------2017-10-24----------------*/
    public function postss($url,$data) {
        //初始化curl
		$ch = curl_init();
		//设置超时
