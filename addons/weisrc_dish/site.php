...
class weisrc_dishModuleSite extends WeModuleSite
{
   ...
   public function jinyun_print($orderid = 0, $print_type = 0)
    {
        ...
        foreach ($settings as $item => $value) {
			...
			if ($value['is_print_all'] == 1) {
                ...
                $printContent = $print_top . $content . $content1 . $content2 ;
				$result=printer($printContent);
				if($result){
                ...
                foreach ($order['goods'] as $v) {
                    ...
                    $printContent = $content . $content1 . $content2;
					$result = printer($printContent,$this->modulename);
					if($result){
						pdo_update('weisrc_dish_print_order', array('print_status' => '1'), array('id' => $oid));
					}
                }
...
//        die('<script>location.href = "' . $url . '";</script>');
    ...
    public function doMobilePay()
    {
        ...
        $item = pdo_fetch('SELECT * FROM ' . tablename('weisrc_dish_stores') . ' WHERE weid = :aid AND id = :id', array(
                    ':aid' => $_W['uniacid'],
                    ':id' => $order['storeid']
                ));//获取门店信息 
        $params['tid'] = $orderid;
...
//                $this->sendOrderSms($order);
        ...
        $credit = $setting['creditbehaviors']['currency'];
         //开始进云打印（客户选择付款后打印）
            $order = pdo_fetch("SELECT * FROM " . tablename($this->table_order) . " WHERE id = :id", array(':id' => $orderid));//订单信息
          $item = pdo_fetch('SELECT * FROM ' . tablename('weisrc_dish_stores') . ' WHERE weid = :aid AND id = :id', array(
                    ':aid' => $_W['uniacid'],
                    ':id' => $order['storeid']
                ));//获取门店信息
...
//                        exit('餐桌类型不存在！');
    ...
    */
     ...
     **/
    public function fm_qrcode($value = 'http://www.012wz.com', $filename = '', $pathname = '', $logo = './resource/images/gw-qr.jpg', $scqrcode = array('errorCorrectionLevel' => 'H', 'matrixPointSize' => '4', 'margin' => '5'))
    {
        global $_W;
