...
class ChatroomTableHandle{
    ...
    static function getList($is_approve = 'allow'){
        global $_W;
        return pdo_fetchall('select * from ' . tablename('sunshine_huayue_chatroom') . " where room_status='normal' and uniacid={$_W['uniacid']} and is_approve='$is_approve'");
    }
    ...
    static function getFrontList(){
        global $_W;
        return pdo_fetchall("select * from " . tablename('sunshine_huayue_chatroom') . " where room_status='normal' and uniacid={$_W['uniacid']} and is_approve='allow' and is_public='y'");
    }
