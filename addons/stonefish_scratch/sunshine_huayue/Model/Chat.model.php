...
class ChatTableHandle{
    static function chatInit($chat_openid, $self_openid){
        ...
        if(!$res){
            $defriend_status = DefriendTableHandle :: status($self_openid, $chat_openid);
            if($defriend_status){
                return array('res' => '104', 'msg' => '您将对方加入了黑名单，移出后可以发起聊天');
            }
            $defriend_status = DefriendTableHandle :: status($chat_openid, $self_openid);
            if($defriend_status){
