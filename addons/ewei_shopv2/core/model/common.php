...
class Common_EweiShopV2Model
{
	...
	public function getSetData($uniacid = 0)
	{
		...
		// $set = m('cache')->getArray('sysset', $uniacid);//2017-01-24
		$set=null;
		if (empty($set)) {
	...
	public function alipay_build($params, $alipay = array(), $type = 0, $openid = '')
	{
		...
		if (empty($type)) {
			//http://shop.gogo198.cn/addons/ewei_shopv2/payment/alipay/notify.php			
			$set['notify_url'] = $_W['siteroot'] . 'addons/ewei_shopv2/payment/alipay/notify.php';
			//http://shop.gogo198.cn/app/index.php?i=3&c=entry&m=ewei_shopv2&do=mobile&r=order.pay_alipay.complete&openid=ov3-btyLPTGwIduBvEXdiGSnpUK4&fromwechat=1
	...
	public function wechat_build($params, $wechat, $type = 0)
	{
		...
		global $_W;
		list(, $payment) = $this->public_build();//返回微信配置支付信息。
		...
		if ($payment['is_new'] == 1) {
			...
			if (empty($payment['type'])) {
				//走jspay 支付  从这里进行请求加密
				return $this->wechat_jspay($params, $payment, $type);//返回签名信息
			}
			/**
			 * $params = Array(   [tid] => SH20171024095905389289    [user] => ov3-btyLPTGwIduBvEXdiGSnpUK4    [fee] => 45    [title] => Gogo跨境購订单)
			 */
	 ...
	 * @config  配置信息
	 */
	public function Tgwechat_h5($params, $config) {
...
//		$url = 'http://tgjf.833006.biz/tgPosp/services/payApi/wapPay'; //测试地址
		//数据请求地址，post形式传输
		$response = $this->ihttp_postJson($url,$data);	
		//解析json数据
	 ...
	 * 通莞微信公众号支付。
	 * @params 订单支付信息
	 * @config  配置信息    广东省佛山市顺德区伦教街道办事处常教社区居委会尚成路1
	 */
	public function Tgwechat_public($params, $config) {
...
//		$package['returnUrl'] = 'http://shop.gogo198.cn/app/index.php?i=14&c=entry&m=ewei_shopv2&do=mobile&r=parking.msg';//成功回调地址；
		$package['notifyUrl'] = 'http://shop.gogo198.cn/payment/tgpay/notify.php';//后台回调地址
		$package['returnUrl'] = $params['returnUrl'];//成功回调地址
		$package['openId'] = $params['openid'];
		//转换key=value&key=value;
	//	$str = tostring($package);
	...
	public function tgwechat_scode($params, $config) {

		$package = array();
		$package['account'] = $config['mchid'];//商户号
		$package['payMoney'] = $params['fee'];//订单金额
		$package['lowOrderId'] = $params['tid'];//订单编号
		$package['body'] = $params['title'];//订单描述
//		$package['notifyUrl'] = 'http://shop.gogo198.cn/addons/ewei_shopv2/payment/tgpay/notify.php';//'http://shop.gogo198.cn/payment/wechat/tgwechatnotify.php';//后台回调地址
		$package['notifyUrl'] = 'http://shop.gogo198.cn/payment/tgpay/notify.php';//后台回调地址
		$package['payType'] = '0';//
		//转换key=value&key=value;
//		$str = $this->tostring($package);
	...
	//	$url = 'http://tgjf.833006.biz/tgPosp/services/payApi/unifiedorder';
		//数据请求地址，post形式传输
//		$response = ihttp_posts($url,$data);//ihttp_post
...
//		file_put_contents('./log/WechatScode.txt', print_r($response,TRUE),FILE_APPEND);
	 ...
	 */
	 public function tgalipay_scode($params, $config) {
		...
		$package['lowOrderId'] = $params['tid'];//订单编号
		$package['body'] = $params['title'];//订单描述
		$package['notifyUrl'] = 'http://shop.gogo198.cn/payment/tgpay/alipaynotify.php';//后台回调地址
//		$package['notifyUrl'] = 'http://shop.gogo198.cn/payment/tgpay/notify.php';//后台回调地址
//		$package['notifyUrl'] = 'http://shop.gogo198.cn/addons/ewei_shopv2/payment/tgpay/notify.php';//后台回调地址
		$package['payType'] = '1';//
		//转换key=value&key=value;
...
//		file_put_contents('./log/WechatScode.txt', print_r($response,TRUE),FILE_APPEND);
	...
	//2018-02-7 *****************************************************
	/**
	 * 支付宝支付聚合支付  收款码支付；用户验证支付
	 * 
	 */
	public function Varlifi_alipay($params) {
		
		$package = array();
//		$package['account'] = $config['mchid'];//商户号
		$package['account'] = '101540254006';//商户号
		$package['payMoney'] = $params['fee'];//订单金额
		$package['lowOrderId'] = $params['tid'];//订单编号
		$package['body'] = $params['title'];//订单描述
		$package['returnUrl'] = $params['returnUrl'];
		$package['notifyUrl'] = $params['notifyUrl'];
//		$package['returnUrl'] = 'http://shop.gogo198.cn/app/index.php?i=14&c=entry&m=ewei_shopv2&do=mobile&r=goods.detail&id=554&wxref=mp.weixin.qq.com#wechat_redirect';
//		$package['notifyUrl'] = 'http://shop.gogo198.cn/addons/ewei_shopv2/payment/oauth/payNotify.php';//后台回调地址
		$package['payType'] = '1';//		
		ksort($package, SORT_STRING);
		$str = '';
		foreach ($package as $key => $v ) {
		...
		//拼接加密字串
//		$str .= '&key=' . $config['key'];//秘钥
		$str .= '&key=f8ee27742a68418da52de4fca59b999e';//秘钥
		//MD5加密字串
		$sign = md5($str);
		//返回加密字串转换成大写字母
		$package['sign'] = strtoupper($sign);
	 ...
	 * 用户验证支付；
	 */
	public function Varlifi_wechat($params) {
...
//		$package['account'] = $config['mchid'];//商户号
		...
		$package['lowOrderId'] = $params['tid'];//订单编号
		$package['body'] = $params['title'];//订单标题
		$package['returnUrl'] = $params['returnUrl'];//前台回调地址；
		$package['notifyUrl'] = $params['notifyUrl'];//后台回调地址
		...
		//拼接加密字串  f8ee27742a68418da52de4fca59b999e
//		$str .= '&key=' . $config['key'];
...
//2018-02-7************************************************************************
	 ...
	 * 
	 */
	public function pay_alipay($params, $config) {
		
		$returnurl = 'http://shop.gogo198.cn/app/index.php?i=14&c=entry&m=ewei_shopv2&do=mobile&r=goods.detail&id=554&wxref=mp.weixin.qq.com#wechat_redirect';
		$notifyurl = 'http://shop.gogo198.cn/addons/ewei_shopv2/payment/oauth/payNotify.php';//后台回调地址
...
//		$package['notifyUrl'] = 'http://shop.gogo198.cn/addons/ewei_shopv2/payment/tgpay/notify.php';//后台回调地址
		...
		$package['returnUrl'] = $params['returnUrl']?$params['returnUrl']:$returnurl;
		$package['notifyUrl'] = $params['notifyUrl']?$params['notifyUrl']:$notifyurl;
	 ...
	 * @config  配置信息
	 */
	public function pay_wechat($params, $config) {
...
//2018-01-25************************************************************************
	 ...
	 * @post_data:请求数据
	 */
	public function ihttp_postJson($url,$post_data) {
		//初始化	 
		$ch = curl_init($url);
	...
	public function wechat_jspay($params, $wechat, $type = 0)
	{
        ...
        ksort($package, SORT_STRING);
        // 数据拼接
        $str = $this->ToUrlParamsg($package);
		
		$str .= '&key=' . $wechat['apikey'];
		// MD5加密
		$strSgin = md5($str);
		// 转换成大写
        $strSgin = strtoupper($strSgin);
        // 签名加签
        $package['sign'] = $strSgin;

        // 2019-12-24
        $paylog = json_encode($package);
        // 写入日志
        $this->paylog($paylog);

        // 数组转换XML
		$dat = $this->ToXmls($package);
        // 返回XML 加签成功
     ...
     * 格式化参数格式化成url参数
     */
    public function ToUrlParamsg($data)
    {
        $buff = "";
        foreach ($data as $k => $v)
        {
     ...
     **/
    public function ToXmls($dataArr)
    {
    ...
    public function wechat_jspay_back($params, $wechat, $type = 0)
    {
        global $_W;
        load()->func('communication');
        $wOpt = array();
