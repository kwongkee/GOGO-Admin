...
class Detail_EweiShopV2Page extends MobilePage
{
	public function main()
	{
        ...
        if(!$this->isRegister($openid)) {
            ...
            message('您还没注册会员，请先注册！',$url,'error');
        }
	...
	public function isRegister($openid) {
        ...
        $uniacid = $_W['uniacid'];
        // 获取sz_yi_member
        $sql = 'SELECT realname,id_card FROM ' . tablename('sz_yi_member') . ' WHERE `openid`=:openid AND uniacid=:uniacid limit 1';
        $params = array();
