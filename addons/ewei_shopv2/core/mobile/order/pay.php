...
class Pay_EweiShopV2Page extends MobileLoginPage
{
	public function main()
	{
		...
		$ispeerpay = m('order')->checkpeerpay($orderid);//查询ewei_shop_order_peerpay 关联 ewei_shop_order 条件：订单id，与公众号ID
		...
		//查询ewei_show_order 表中id = $orderid 的所有数据。
		...
		if (empty($order)) {//如果$order == null;
			header('location: ' . mobileUrl('order'));
			exit();
		}

		// $order[status] = 0;
		if ($order['status'] == -1) {//订单状态等于0
		...
		if (empty($log)) {//如果支付日志表没有该信息则添加信息
			$log = array('uniacid' => $uniacid, 'openid' => $openid ? $openid : $member['uid'], 'module' => 'ewei_shopv2', 'tid' => $order['ordersn'], 'fee' => $order['price'], 'status' => 0);
			pdo_insert('core_paylog', $log);
			$plid = pdo_insertid();//返回插入数据的ID
		}

		//获取商品信息，与支付信息表   获取shop与pay字段；
		$set = m('common')->getSysset(array('shop', 'pay'));

		//获取微信设置取反，返回1，否则返回0
		$set['pay']['weixin'] = ((!(empty($set['pay']['weixin_sub'])) ? 1 : $set['pay']['weixin']));// =1;
		
		$set['pay']['weixin_jie'] = ((!(empty($set['pay']['weixin_jie_sub'])) ? 1 : $set['pay']['weixin_jie']));//=1
		...
		if (empty($order['price']) && !($credit['success'])) {
			//./index.php?i=3&c=entry&m=ewei_shopv2&do=mobile&r=order.pay.complete&id=229&type=credit
		...
		if (is_weixin()) {
			...
			if (isset($set['pay']) && ($set['pay']['weixin'] == 1) && ($jie !== 1)) {
				$options = array();
				//$setting[payment][wechat] 是数组，并且 $setting[payment][wechat][switch] 为 true;
				if (is_array($setting['payment']['wechat']) && $setting['payment']['wechat']['switch']) {
					...
					if (is_array($setting['payment'])) {
						$options = $setting['payment']['wechat'];//微信支付配置
				...
				/**
				 * $wechat['success'] = true;  显示微信支付
				 *  原微信支付
                 * 可以使用；2019-11-19
                 * author: 赵金如
                 *
                 * JSAPI授权目录：
                 * https://shop.gogo198.cn/addons/ewei_shopv2/core/mobile/order/
                 * 回调：addons/ewei_shopv2/payment/wechat/notify.php
				 */
				//微信支付返回的JSAPI支付信息
...
//			$params = array();
			//订单编号
//			$params['tid'] = $log['tid']; //需要改回来
			
//			$params['tid'] = 'GOGOP2017121211'.rand(1, time());
//			
//			if (!(empty($order['ordersn2']))) {
//				$var = sprintf('%02d', $order['ordersn2']);
//				$params['tid'] .= 'GJ' . $var;
...
//			$params['title'] = $param_title;
			...
			//条件成立
//			if (isset($set['pay']) && ($set['pay']['app_tg'] == 1)) {
			if (($set['pay']['app_tg'] == 1)) {
				
//				$options = array();
//				if (is_array($sec) && (!empty($sec['app_tg']))) {
//
...
//				$tgwechat['weixin'] = true;
				...
				if($tgwechat_scode['status'] == '100' && $tgwechat_scode['message'] == '获取二维码成功') {
						...
						$tgwechat['success'] = true;
					if (!(empty($tgwechat_scode['codeUrl']))) {
						$tgwechat['codeUrl'] = 	$tgwechat_scode['codeUrl'];		
						$tgwechat['weixin'] = true;
						
					}else{
						$tgwechat['weixin'] = FALSE;
					}
...
//		$tgalipay['weixin'] = true;
		...
		if($tgalipay_scode['status'] == '100' && $tgalipay_scode['message'] == '获取二维码成功') {
			...
			if (isset($tgalipay_scode['codeUrl'])) {
				...
				$tgalipay['codeUrl'] = 	$tgalipay_scode['codeUrl'];
				$tgalipay['weixin'] = true;
				
			}else {
				
				$tgalipay['weixin'] = false;
				$tgalipay['msg'] = $tgalipay_scode['message'];
			}
	...
	//聚合支付，支付宝结束  2017-12-11
		...
		if (empty($seckill_goods) && empty($ispeerpay)) {
			...
			if (isset($set['pay']) && ($set['pay']['alipay'] == 1)) {
				// $setting[payment][alipay] 是数组 并且 $setting[payment][alipay][switch] 为true
				if (is_array($setting['payment']['alipay']) && $setting['payment']['alipay']['switch']) {
					...
					if (is_array($setting['payment'])) {
						//获取支付宝支付信息
						$options = $setting['payment']['alipay'];//Array ( [switch] => 1 [account] => 179078286@qq.com [partner] => 2088801372822014 [secret] => k9d3shvm1vmh3i0wnmjhrum95n180z84 )
						//返回支付拼接URL
...
//				echo '<pre>';print_r($AliPay);die;
		...
		// 支付信息包含，订单id,余额，支付宝支付信息，微信支付信息，cash信息 ，交易金额：money
		$payinfo = array('orderid' => $orderid,'opid' => $openid,'uniacid'=>$uniacid, 'credit' => $credit, 'alipay' => $alipay, 'wechat' => $wechat, 'tgwechat' => $tgwechat, 'tgalipay' => $tgalipay, 'cash' => $cash, 'money' => $order['price']);
 ...
 */
	...
	public function complete()
	{
		global $_W;
		global $_GPC;
		$orderid = intval($_GPC['id']);//获取订单编号
		$uniacid = $_W['uniacid'];//获取当前公众号ID
		$openid = $_W['openid'];//获取Openid
		$ispeerpay = m('order')->checkpeerpay($orderid);//查询表中信息并返回 ewei_shop_order_peerpay  an ewei_shop_order
