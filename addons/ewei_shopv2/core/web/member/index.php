...
class Index_EweiShopV2Page extends WebPage
{
	...
	public function query()
	{
		...
		$ds = pdo_fetchall('SELECT uid,openid,nickname,avatar FROM ' . tablename('ewei_shop_member') . ' WHERE 1 ' . $condition . ' order by id asc', $params);

/**
 * 修改时间2017.10.27  16.20
 * 代码待优化
 */
		if(empty($ds)) {
			$DataUeser=pdo_fetchall('SELECT `uniacid`,`uid`,`realname`,`mobile`,`createtime`,`nickname`,`avatar` FROM'. tablename('mc_members').'where 1 and uniacid=:uniacid AND `nickname` LIKE :keyword or `realname` LIKE :keyword
 or `mobile` LIKE :keyword  limit 1',array(':uniacid' =>$_W['uniacid'] ,':keyword'=>'%'.$kwd.'%'));
			$openId=pdo_get('mc_mapping_fans',array('uniacid'=>$_W['uniacid'],'uid'=>$DataUeser[0]['uid']),array('openid','follow'));
			$data=[
				'uniacid'=>$_W['uniacid'],
				'uid'=>$DataUeser[0]['uid'],
				'openid'=>$openId['openid'],
				'realname'=>$DataUeser[0]['realname'],
				'mobile'=>$DataUeser[0]['mobile'],
				'createtime'=>$DataUeser[0]['createtime'],
				'nickname'=>$DataUeser[0]['nickname'],
				'avatar'=>$DataUeser[0]['avatar'],
			];

			if(!empty($openId)&&$openId['follow']!='0'){
				$bool=pdo_insert('ewei_shop_member',$data);
				if($bool){
					//$ds = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_member') . ' WHERE 1 ' . $condition . ' order by id asc', $params);
					// 2019-10-17
					$ds = pdo_fetchall('SELECT uid,openid,nickname,avatar FROM ' . tablename('ewei_shop_member') . ' WHERE 1 ' . $condition . ' order by id asc', $params);
