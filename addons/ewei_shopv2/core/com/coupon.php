...
class Coupon_EweiShopV2ComModel extends ComModel
{
	...
	public function useConsumeCoupon($orderid = 0)
	{
		...
		$order = pdo_fetch('select ordersn,createtime,couponid,price,goodsprice from ' . tablename('ewei_shop_order') . ' where id=:id and status>=0 and uniacid=:uniacid limit 1', array(':id' => $orderid, ':uniacid' => $_W['uniacid']));
		...
		if (!empty($coupon['coupon_num'])){
            $cid = pdo_get('foll_coupon',['coupon_num'=>$coupon['coupon_num']]);
            pdo_insert('foll_coupon_use',[
                'order_id'   =>  $order['ordersn'],
                'coupon_id'  =>  $cid['id'],
                'user_id'    => $_W['openid'],
                'original_amout'=>$order['goodsprice'],
                'discount_amout'=>$order['goodsprice']-$order['price'],
                'create_time'  => time(),
                'busin_id'   => $cid['busin_id'],
                'use_busin_name'=>$_W['account']['name'],
                'application'=>'shop'
            ]);
        }
