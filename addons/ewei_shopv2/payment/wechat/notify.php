...
class EweiShopWechatPay
{
	...
	public function order()
	{
		...
		// 微信支付订单号
		$transaction_no = $this->get['transaction_id'];
		$isborrow = 0;
		$borrowopenid = '';
		...
		if (!(empty($log)) && (($log['status'] == '0') || $ispeerpay) && (($log['fee'] == $this->total_fee) || $ispeerpay)) {
			pdo_update('ewei_shop_order', array(
			    'paytype' => 21, 'isborrow' => $isborrow, 'borrowopenid' => $borrowopenid,
                'apppay' => ($this->isapp ? 1 : 0),'transid'=>$transaction_no,
			...
			if (!(is_error($site))) {
				...
				if (method_exists($site, $method)) {
					...
					if ($result) {
						...
						// 2019-12-24 存放支付响应数据；
						$record['reqRes'] = json_encode($this->get);
						pdo_update('core_paylog', $record, array('plid' => $log['plid']));

                        // 2020-04-08
						$this->getUserinfo($this->get['openid'],$tid,$transaction_no);
					}
     ...
     *  2020-04-08
     */
	public function getUserinfo($openid,$trade_no,$trade_id)
    {
        // 获取sz_yi_member
     ...
     *  2020-04-08
     */
	public function SelfCustoms($senArr)
    {
