...
file_put_contents('./log/buslog.txt', print_r($receive,TRUE),FILE_APPEND);
/**
	...
	if ($receive['state'] == 0 && $receive['orderDesc'] == '支付成功') {//支付成功
			...
			if(!empty($vfold)) {
				...
				if(!empty($vfold['idcard']) && !empty($vfold['username'])) {
					...
					$host = "http://bussinfo.market.alicloudapi.com";
				    $path = "/v2/industry_business_normal/search/";
				    $method = "POST";
				    ...
				    $querys = "";
				    $bodys = '{"keyword":"'.$vfold['idcard'].'"}';//91440605782023871F
				    $url = $host . $path;
				    ...
				    file_put_contents('./log/business.txt', print_r($rData,TRUE),FILE_APPEND);
					
					if($rData['status'] == '00000' && $rData['remark'] == '查询成功') {//查询成功
						
						//是否接收到回调  SUCCESS表示成功  发送消息模板；
						
						$sendArr = [
							'uid' =>  $vfold['uid'],//验证用户ID
//							'uName' => $vfold['username'],//公司名称
							'code' => $rData['status'],//验证状态
							'msg' => $rData['remark'],//验证有效性
							'key_no' =>$rData['result'][0]['key_no'],
							'reg_status' =>$rData['result'][0]['reg_status'],
							'register_num' =>$rData['result'][0]['register_num'],
							'organization_name' =>$rData['result'][0]['organization_name'],
							'establish_date' =>$rData['result'][0]['establish_date'],
							'operator_name' =>$rData['result'][0]['operator_name'],
							'credit_code' =>$rData['result'][0]['credit_code'],
						];
						...
						//跳转地址
						$Reurl = "http://shop.gogo198.cn/app/index.php?i=3&c=entry&m=ewei_shopv2&do=mobile&r=money.salesman.ReurlBusInfo&key=".$keys;
