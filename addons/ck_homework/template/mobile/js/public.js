...
function Savenamepost(){
	...
	$.post(wapurl+"/member/index.php?c=setname",{username:username,password:pass},function(data){
		if(data==1){
			layermsg("修改成功，请重新登陆！", 2,function(){location.href=wapurl+"/index.php?m=login"});return false;
		}else{
	...
	if(id){
		$.post(url,{id:id},function(data){
			if(data==1){
				...
				layermsg('请先登陆！只有个人用户才能关注');return false;
			}else if(data==4){
...
function submitreply(id,fid,url){
	...
	$.post(url,{nid:id,reply:content,fid:fid},function(data){
		if(data==1){ 
			layer.msg('请先登陆！', 2,8);return false;
		}else{
			var data = eval("("+data+")");
			var content = "";
...
function invite(url,jobid){ 
	$.post(url,{show_job:'1'},function(data){
		...
		var integral=data.integral;
		if(status == 6){
			layermsg('请先登陆！',2,function(){location.href='index.php?c=login';}); 
		}
		if(!status || status == 0){
