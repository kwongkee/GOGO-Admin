...
if(checksubmit('add_submit') || checksubmit('edit_submit')){
	...
	if(checksubmit('add_submit')){
		 ...
		 pdo_insert('onljob_notice', $data);
		 message('添加成功', $urlt, 'success');
	}
	
	//修改
	if(!empty($_GPC['idd'])){
		pdo_update('onljob_notice', $data, array('id' => $_GPC['idd'],'weid' => $_W['uniacid']));
...
if($op == 'delete'){
	$id = intval($_GPC['id']);
	if($id){
		pdo_delete('onljob_notice', array('id' => $id,'weid' => $_W['uniacid']));
...
if (checksubmit('listsubmit')) {
	if($_GPC['ids'] && is_array($_GPC['ids'])) {
		...
		for($i=0;$i < count($ids); $i++){
			pdo_delete('onljob_notice', array('id' => $ids[$i],'weid' => $_W['uniacid']));
		}
...
$list = pdo_fetchall("SELECT * FROM " . tablename('onljob_notice') . " WHERE weid = '{$_W['uniacid']}' ORDER BY listorder ASC,id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize);
$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('onljob_notice') . " WHERE weid = '{$_W['uniacid']}'");
