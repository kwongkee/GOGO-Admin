...
$total = pdo_fetchcolumn("SELECT COUNT(*) FROM ".tablename('onljob_user')."  WHERE weid = '{$_W['uniacid']}' AND type = '0' {$where}");
if($total){
	$list = pdo_fetchall("SELECT * FROM ".tablename('onljob_user')." WHERE weid = '{$_W['uniacid']}' AND type = '0' {$where} {$ordersql} LIMIT " . ($pindex - 1) * $psize . ',' . $psize);
	if (!empty($list)) {
		$fasshow = '';
