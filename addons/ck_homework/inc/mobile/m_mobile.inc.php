...
$urltk = $this->createMobileUrl('m_mobile');
$url_sms = $this->createMobileUrl('sms');

//提交处理
if(checksubmit('save_submit')){
	
	$urlt = $this->createMobileUrl('m_index');
	...
	if (!empty($phone)) {
		...
		if(!empty($mc_members) && $mc_members['uid'] != $_W['member']['uid']){
			message_app('抱歉！你输入手机号已经被注册，请从新更换一个！', '', 'error');
		}
	}
