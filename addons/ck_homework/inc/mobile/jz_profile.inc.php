...
$urltk = $this->createMobileUrl('jz_profile');
...
if(checksubmit('save_submit')){
	...
	if(!empty($user_show['id'])){
		...
		message_app('修改成功！', array($this->createMobileUrl('jz_index')), 'success');
	}else{
		message_app('修改失败！', array($this->createMobileUrl('jz_index')), 'error');
	}
	
}
	
include template_app('jz_profile');