...
$urltk = $this->createMobileUrl('jz_theclass');
...
$total = pdo_fetchcolumn("SELECT COUNT(*) FROM ".tablename('onljob_theclass_apply')." AS a LEFT JOIN ".tablename('onljob_theclass')." AS c ON a.bjid = c.id LEFT JOIN ".tablename('onljob_user')." AS b ON c.uid = b.uid WHERE a.weid = '{$_W['uniacid']}' and a.uid = '{$_SESSION['hzuid']}' and a.state = '1' {$where}");
if($total){
	$list = pdo_fetchall("SELECT a.id as sqid,c.*,c.uid as lsuid,b.name FROM ".tablename('onljob_theclass_apply')." AS a LEFT JOIN ".tablename('onljob_theclass')." AS c ON a.bjid = c.id LEFT JOIN ".tablename('onljob_user')." AS b ON c.uid = b.uid WHERE a.weid = '{$_W['uniacid']}' and a.uid = '{$_SESSION['hzuid']}' and a.state = '1' {$where} ORDER BY a.id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize);
	if (!empty($list)) {
		$xstotal = '';
...
include template_app('jz_theclass');
