...
$urltk = $this->createMobileUrl('jz_theclass_show').'&id='.$id;
...
if($op == 1){
	...
	include template_app('jz_theclass_notice');
	exit();
	...
	if($tab > 0){
		...
		$total = pdo_fetchcolumn("SELECT COUNT(*) FROM ".tablename('onljob_work_fen')." AS a LEFT JOIN ".tablename('onljob_work')." AS b ON a.wid = b.wid LEFT JOIN ".tablename('onljob_theclass')." AS c ON a.bjid = c.id WHERE a.weid = '{$_W['uniacid']}' and a.uid = '{$_SESSION['hzuid']}' and a.bjid = '{$id}' {$where} ");
		if($total){
			$list = pdo_fetchall("SELECT b.titlename,a.* FROM ".tablename('onljob_work_fen')." AS a LEFT JOIN ".tablename('onljob_work')." AS b ON a.wid = b.wid LEFT JOIN ".tablename('onljob_theclass')." AS c ON a.bjid = c.id WHERE a.weid = '{$_W['uniacid']}' and a.uid = '{$_SESSION['hzuid']}' and a.bjid = '{$id}' {$where} ORDER BY a.fid DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize);
			if (!empty($list)) {
				$list_yz = array();
		...
		$list = pdo_fetchall("SELECT a.* FROM ".tablename('onljob_work')." AS a LEFT JOIN ".tablename('onljob_theclass_apply')." AS b ON a.bjid = b.bjid LEFT JOIN ".tablename('onljob_theclass')." AS c ON a.bjid = c.id WHERE a.weid = '{$_W['uniacid']}' and a.releaset = '1' and b.uid = '{$_SESSION['hzuid']}' and a.bjid = '{$id}' {$where} ORDER BY a.wid DESC ");
		if (!empty($list)) {
			$list_wz = array();
	...
	include template_app('jz_theclass_homework');
	exit();
	...
	include template_app('jz_theclass_show');
