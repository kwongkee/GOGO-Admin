...
$urlt = $this->createMobileUrl('jz_cwanswer');
...
//学科搜索
if(!empty($xkid)){
	$where .= " AND a.bjid in (SELECT d.bjid FROM ".tablename('onljob_theclass_apply')." AS d LEFT JOIN ".tablename('onljob_theclass')." AS c ON d.bjid = c.id WHERE b.weid = '{$_W['uniacid']}' and d.uid = '{$_SESSION['hzuid']}' and c.xkid = '{$xkid}')";
}

$total = pdo_fetchcolumn("SELECT COUNT(*) FROM ".tablename('onljob_work_answer')." AS a LEFT JOIN ".tablename('onljob_questions')." AS b ON a.qid = b.id WHERE a.weid = '{$_W['uniacid']}' and a.uid = '{$_SESSION['hzuid']}' and a.stateh = '1' and b.type < 6 {$where} ");
if($total){
	$list = pdo_fetchall("SELECT a.*,b.titlename,b.type,b.parentid FROM ".tablename('onljob_work_answer')." AS a LEFT JOIN ".tablename('onljob_questions')." AS b ON a.qid = b.id WHERE a.weid = '{$_W['uniacid']}' and a.uid = '{$_SESSION['hzuid']}'  and a.stateh = '1' and b.type < 6 {$where} ORDER BY a.qid DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize);
	if (!empty($list)) {
