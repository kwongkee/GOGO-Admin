...
if (!$award_info) {
    message("奖品不存在", $this->createMobileUrl('award_list_coupon'));
}

if ($award_info['level'] > $user['level']) {
    message("会员等级不够，不能兑换", $this->createMobileUrl('award_list_coupon'));
}

//未兑换
if ($award_info['num'] <= 0) {
    message("优惠券已发完", $this->createMobileUrl('award_list_coupon'));
}

if ($user['left_credit'] < $award_info['credit']) {
    message("积分不足", $this->createMobileUrl('award_list_coupon'));
}
...
if ($ret2) {
    message("兑换成功", $this->createMobileUrl('award_list_coupon'));
} else {
    message("兑换失败", $this->createMobileUrl('award_list_coupon'));
}
?>
