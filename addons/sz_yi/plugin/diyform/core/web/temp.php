...
if ($operation == 'display') {
	...
	foreach ($items as $key => $value) {
		...
		if ($set['user_diyform_open'] && ($set['user_diyform'] == $value['id'])) {
			...
			$items[$key]['template'] = 'user_diyform'; //xj 20170304 添加某个模板标识
		}
		//申请供应商资料
		if ($set['supplier_diyform_open'] && ($set['supplier_diyform'] == $value['id'])) {
			$items[$key]['supplier_flag1'] = 1;
			$items[$key]['template'] = 'supplier_diyform'; //xj 20170304 添加某个模板标识

		}
		//申请渠道商
		if ($set['channel_diyform_open'] && ($set['channel_diyform'] == $value['id'])) {
			$items[$key]['channel_flag1'] = 1;
			$items[$key]['template'] = 'channel_diyform'; //xj 20170304 添加某个模板标识

		}
		//分销商申请资料
		if ($set['commission_diyform_open'] && ($set['commission_diyform'] == $value['id'])) {
			$items[$key]['use_flag2'] = 1;
			$items[$key]['template'] = 'commission_diyform'; //xj 20170304 添加某个模板标识

		}

		//订单统一下单表单 xj 20170304 添加
		if ($set['order_diyform_open'] && ($set['order_diyform'] == $value['id'])) {
			$items[$key]['order_diyform'] = 1;
			$items[$key]['template'] = 'order_diyform'; //xj 20170304 添加某个模板标识
