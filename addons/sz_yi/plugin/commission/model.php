		...
		public function getInfo($openid, $options = NULL)
		{
			...
			if (1 <= $level) {
				...
				if (in_array('pay', $options)) {
					//手动提现合并免审核提现
					$level1_commissions2 = pdo_fetchall('select og.commission1,og.commissions  from ' . tablename('sz_yi_order_goods') . ' og ' . ' left join  ' . tablename('sz_yi_order') . ' o on o.id = og.orderid' . ' where o.agentid=:agentid and o.status>=3 and  o.status<>6 and o.status<>4 and o.status<>5 and (og.status1=3 OR og.status1=1) and og.nocommission=0 and o.uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
			...
			if (2 <= $level) {
				if (0 < $level1) {
					...
					if (in_array('pay', $options)) {
						//手动提现合并免审核提现
						$level2_commissions3 = pdo_fetchall('select og.commission2,og.commissions  from ' . tablename('sz_yi_order_goods') . ' og ' . ' left join  ' . tablename('sz_yi_order') . ' o on o.id = og.orderid ' . ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')  and o.status>=3 and o.status<>6 and o.status<>4 and o.status<>5 and (og.status2=3 OR og.status2=1) and og.nocommission=0 and o.uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
			...
			if (3 <= $level) {
				if (0 < $level2) {
					...
					if (in_array('pay', $options)) {
						//手动提现合并免审核提现
						$level3_commissions3 = pdo_fetchall('select og.commission3,og.commissions  from ' . tablename('sz_yi_order_goods') . ' og ' . ' left join  ' . tablename('sz_yi_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ')  and o.status>=3  and o.status<>4 and o.status<>5 and o.status<>6 and (og.status3=3 OR og.status3=1) and og.nocommission=0 and o.uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
