	...
	if ($_W['ispost']) {
		...
		if ($creditnocheck == 1) {
			if (($commission_ok <= $closewithdrawcheck) || ($apply['type'] == 0)) {
				...
				//xj 自动打款 新增更新佣金打款状态  start
				foreach ($orderids as $o) {
					pdo_update('sz_yi_order_goods', array('checktime' . $o['level'] => time(), 'paytime' . $o['level'] => time(),'status'.$o['level']=>3), array('orderid' => $o['orderid'], 'uniacid' => $_W['uniacid']));
				}
				//xj  自动打款  新增更新佣金打款状态 end
