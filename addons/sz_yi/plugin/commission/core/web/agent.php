...
if ($operation == 'display') {
	...
	$total = pdo_fetchcolumn('select count(dm.id) from' . tablename('sz_yi_member') . ' dm  ' . ' left join ' . tablename('sz_yi_member') . ' p on p.id = dm.agentid ' . ' left join ' . tablename('mc_mapping_fans') . 'f on f.openid=dm.openid and f.uniacid=' . $_W['uniacid'] . ' where dm.uniacid =' . $_W['uniacid'] . ' and dm.isagent =1 ' . $condition, $params); //lotodo 20170612 添加 and f.uniacid=' . $_W['uniacid'] 确保每个公众的统计正常
	...
	if ($hasagent) {
		...
		foreach ($list as &$row) {
			...
			//新增 下线会员分销等级  20170801 s
			$sql = 'SELECT levelname FROM '.tablename('sz_yi_commission_level').' WHERE id=:id AND uniacid=:uniacid';
			$row['levelname'] = pdo_fetchcolumn($sql,array(':id'=>$row['agentlevel'],':uniacid'=>$_W['uniacid']));
			//新增 下线会员分销等级  20170801 e
