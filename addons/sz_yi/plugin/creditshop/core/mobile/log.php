...
if ($_W['isajax']) {
	if ($operation == 'display') {
		...
		//han 20170401 添加查询物流信息
		$to = $_GPC['to'];
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT * FROM ' . tablename('sz_yi_creditshop_log') . ' WHERE id = :id and uniacid=:uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		if ($to == 'express') {
		...
		//han 20170401 添加积分兑换的物品确认收货
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT * FROM ' . tablename('sz_yi_creditshop_log') . ' WHERE id = :id and uniacid=:uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		if($item['status'] == '3'){
			$item['status'] = 4;
		}
		pdo_update('sz_yi_creditshop_log', array('status' => $item['status'], 'usetime' => time()), array('id' => $id));
		exit();
