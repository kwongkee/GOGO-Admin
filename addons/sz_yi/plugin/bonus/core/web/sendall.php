...
/* xj 201704 查询最后一次发送全球分红的时间 begin */
$sql='SELECT create_time FROM '.tablename('sz_yi_sendall_log').' WHERE uniacid=:uniacid ORDER BY id DESC';
$lastSendallTime=pdo_fetchcolumn($sql,array(':uniacid'=>$_W['uniacid']));
/* end */

if (empty($set['sendmonth'])) {
	$dayOrders=$set['day_orders']*24*60*60;		//xj 多少天内的订单
	//$stattime = $daytime - $day_times - 86400; //xj
	$stattime = $daytime - $day_times - 86400 - $dayOrders;	
	...
	$logs_text = '今天';
	$lastSendTime = $lastSendallTime + $day_times + $dayOrders;  //xj 最后一次发送的时间+间隔时间+指定天数  时间戳
}
else {
	if ($set['sendmonth'] == 1) {
		...
		$logs_text = '本月';
		$lastSendTime =mktime(0,0,0,date('m',$lastSendallTime)+1,1); 	//xj 下个月一号时间戳
	}
}

/*xj 如果 最后一次分红时间+间隔时间  大于当前时间 则不能分红*/
...
if ($operation != 'sub_bonus') {
	foreach ($list as $key => &$row) {
		...
		//xj $row['commission_ok'] = number_format($levelmoneys[$row['bonuslevel']], 2);
		if($isGlobal){
			$row['commission_ok'] = 0;	//待发分红
		}else{
			$row['commission_ok'] = number_format($levelmoneys[$row['bonuslevel']], 2);	//待发分红
		}
...
if (!empty($_POST)) {
	...
	$sendallLog=array(
		'uniacid'	  =>	$_W['uniacid'],
		'create_time' =>	mktime(0,0,0,date('m'),date('d')+1) , //明天的00:00:00时间段
		'uid'		  =>  	$_GPC['__uid'],
	);
	pdo_insert('sz_yi_sendall_log',$sendallLog);
