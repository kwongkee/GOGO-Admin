...
//写入日志
file_put_contents('./log/gatherbalancenotify.log', $input."\r\n",FILE_APPEND);
//将接受到的Json数据转换成数组格式。
$data = json_decode($input, true);

//echo $_W['siteroot'] . 'addons/sz_yi/payment/tgwechat/notify.log';
if (!empty($data)) {
    $order = pdo_fetch('select * from ' . tablename('centralize_balance_order') . ' where ordersn=:ordersn limit 1', array(':ordersn' => $data['lowOrderId']));
    if(empty($order)){
    ...
    if ($data['state'] == '0' && $data['orderDesc'] == '支付成功') {
        ...
        if ($order['status'] == 0) {
            ...
            #充值前余额
            $user = pdo_fetch('select * from '.tablename('website_user').' where id=:id',[':id'=>$order['uid']]);
            #获取赠送金额
            $fee = pdo_fetch('select * from '.tablename('centralize_balance_list').' where id=:id',[':id'=>$order['balance_id']]);
            $balance = floatval($user['balance']) + floatval($fee['recharge_money']) + floatval($fee['gift_money']);
            pdo_update('centralize_balance_order', array(
            ...
            #修改会员余额
            pdo_update('website_user',['balance'=>$balance],['id'=>$user['id']]);
            pdo_insert('centralize_pay_log',['user_id'=>$order['uid'],'orderid'=>$order['id'],'money'=>$order['recharge_money'],'status'=>1,'paytype'=>$order['paytype'],'createtime'=>time()]);
            $zf_type='';
            if($order['type']==1){
                $zf_type = '微信支付';
            }elseif($order['type']==2){
            ...
            $name = '余额充值';
            $order['total_money'] = $order['recharge_money'];
            $order['uniacid']=3;
//            $notice = pdo_fetch('select * from '.tablename('centralize_system_notice').' where uid=0');
            ...
            foreach($servicers as $k=>$v) {
                ...
                if (!empty($muser['openid'])) {
                    ...
                    $post = json_encode([
                        ...
                        'keyword2' => 'CNY ' . $order['recharge_money'],
                        'keyword3' => $zf_type,
                        ...
                        'url' => 'https://shop.gogo198.cn/app/index.php?i=3&c=entry&do=gather&p=balance&m=sz_yi&id=' . $order['id'] . '&isadmin=1'
                    ]);
            ...
            $post2 = json_encode([
                ...
                'keyword2'=>$type,
                'keyword3'=>'CNY '.$order['recharge_money'],
                'keyword4'=>$zf_type,
                ...
                'url'=>'https://shop.gogo198.cn/app/index.php?i=3&c=entry&do=gather&p=balance&m=sz_yi&id='.$order['id']
            ]);
