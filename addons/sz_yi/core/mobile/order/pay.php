...
if (($operation == 'display') && $_W['isajax']) {
	...
	//2018-08-08  邦付宝支付
	$helpay = array('success' => false);
	if (isset($set['pay']) && ($set['pay']['helpaystatus'] == 1)) {
		$helpay['success'] = true;
	}
	//2018-08-08  邦付宝支付
...
//	if (isset($set['pay']) && !empty($set['pay']['tgpay'])) {
	...
	if(is_array($order_goods)) {
		...
		foreach($order_goods as $key=>$vs){
			$mtitle .= $vs['title'];
			$mCount += $vs['buycount'];
		}
		$order['title'] = $mtitle;
		$order['goodsCount'] = $mCount;
	}
	
	show_json(1, array('order' => $order, 'set' => $set, 'credit' => $credit, 'wechat' => $wechat, 'tgwechat' => $tgwechat, 'tgalipay' => $tgalipay, 'alipay' => $alipay, 'app_wechat' => $app_wechat, 'app_alipay' => $app_alipay,'helpay'=>$helpay,'unionpay' => $unionpay, 'yunpay' => $yunpay, 'cash' => $cash, 'storecash' => $storecash, 'yeepay' => $yeepay, 'paypal' => $paypal, 'isweixin' => is_weixin(), 'currentcredit' => $currentcredit, 'returnurl' => $returnurl, 'goods' => $order_goods));

} else {
	//点击对应的支付方式 2017-11-16
	if (($operation == 'pay') && $_W['ispost']) {
		$set = m('common')->getSysset(array('shop', 'pay'));
		// han 20171018
		if(!empty($orderid)){
			$new_order = pdo_fetch('select * from ' . tablename('sz_yi_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1', array(':id' => $orderid, ':uniacid' => $uniacid, ':openid' => $openid));;
		...
		// han 20170925 g.totalcnf
		$order_goods = pdo_fetchall('select og.id,g.title, og.goodsid,og.optionid,g.total as stock,g.totalcnf,og.total as buycount,g.status,g.deleted,g.maxbuy,g.usermaxbuy,g.istime,g.timestart,g.timeend,g.buylevels,g.buygroups from  ' . tablename('sz_yi_order_goods') . ' og ' . ' left join ' . tablename('sz_yi_goods') . ' g on og.goodsid = g.id ' . ' where ' . $where_orderid . ' and og.uniacid=:uniacid ', array(':uniacid' => $_W['uniacid']));
		...
		foreach ($order_goods as $data) {
			...
			if (!empty($data['optionid'])) {
				...
				if (!empty($option)) {
					if ($option['stock'] != -1) {
						// han 20170925 !empty($data['totalcnf']) && (empty($option['stock']) || $option['stock'] < $data['buycount'])
						// if (empty($option['stock'])) {
						if (!empty($data['totalcnf']) && (empty($option['stock']) || $option['stock'] < $data['buycount'])) {
				...
				if ($data['stock'] != -1) {
					// han 20170925 !empty($data['totalcnf']) && (empty($data['stock']) || $data['stock'] < $data['buycount'])
					// if (empty($data['stock'])) {
					if (!empty($data['totalcnf']) && (empty($data['stock']) || $data['stock'] < $data['buycount'])) {
		...
		if ($type == 'weixin') {
		...
		} else if ($type == 'tgwechat') {//聚合支付，微信支付
			...
			$where_orderid = 'og.orderid=' . $orderid;
			$order_goods1 = pdo_fetch('select og.id,g.title, og.goodsid,og.optionid,g.total as stock from  ' . tablename('sz_yi_order_goods') . ' og ' . ' left join ' . tablename('sz_yi_goods') . ' g on og.goodsid = g.id ' . ' where ' . $where_orderid . ' and og.uniacid=:uniacid ', array(':uniacid' => $_W['uniacid']));
			
			$params['openid']  = $openid;//2017-11-17
			//$params['user'] = 'om44I1X2wIlTWdN3pSsgoJkfMLtI';//测试Openid
			$params['fee']   = $order['price'];
			//$params['title'] = $param_title;
			$params['title'] = $order_goods1['title'];
			
			load()->model('payment');
			//$setting = uni_setting($_W['uniacid'], array('payment'));//获取系统配置参数
			...
			if (is_array($set['pay']['tgpay'])) {
				...
				if (is_weixin()) {
					...
					if ($set['pay']['tgpaystatus']) {
						...
						//测试
						/*$url = 'http://shop.gogo198.cn/addons/sz_yi/payment/tgwechat/Payments.php';
						$res = m('common')->tgwechat($url,$params);
						$Res   = json_decode($res,true);
						if ($Res['status'] == '100') {
							$tgwechat['payurl']    = $Res['pay_url'];
							$tgwechat['pay_info']  = $Res['pay_info'];
							$tgwechat['success']   = true;
						} else {
						...
						//返回支付URL
						/*$tgwechatRes = m('common')->tgwechats($params, $options);
						if ($tgwechatRes['status'] == '100') {
							$tgwechat['payurl']    = $tgwechatRes['pay_url'];
							$tgwechat['pay_info']  = $tgwechatRes['pay_info'];
							$tgwechat['success'] = true;
						} else {
							show_json(0, $tgwechatRes['message']);
						}*/
	...
	} else if (($operation == 'level') && $_W['ispost']) {

	    // 检测会员等级  查询到会员等级
        $level = (int)$_SESSION['level_id'];// 用户会员等级ID
        // 默认1倍
        $multiple = 1;
        // 查询是否有金额倍数；有就按倍数；没有按默认；
        $mul = pdo_fetch('SELECT `multiple` FROM ' . tablename('sz_yi_member_level') . ' WHERE `id`=:id limit 1', array(':id' => $level));
        $multiple = $mul['multiple'];
        // 如果数据为空，则用openid 再查询一次；
        if(empty($mul)) {
            // 默认1倍
            $multiple = 1;
            $openid = $_GPC['openid'];
            // 查询用户表
            $lev = pdo_fetch('SELECT `level` FROM ' . tablename('sz_yi_member') . ' WHERE `openid`=:openid limit 1', array(':openid' => $openid));
            if(!empty($lev)) {

                $mul = pdo_fetch('SELECT `multiple` FROM ' . tablename('sz_yi_member_level') . ' WHERE `id`=:id limit 1', array(':id' => $lev['level']));
                $multiple = $mul['multiple'];
            }
        }

        // 当前交易金额 * 倍数  先计算不更新，用户确认在更新；
        $price = sprintf("%.2f",$_GPC['price']);
        $mon = $price;
        if($multiple > 0) {
            $mon = sprintf("%.2f",($price * $multiple));
        }
        $money = $mon;

        show_json(1, array('money'=>$money));

    } else if(($operation == 'upMoney') && $_W['ispost']) {

	    // 更新ID
        $order_id = (int)$_GPC['orderid'];
        // 交易金额
        $money = $_GPC['money'];
        // 更新操作
        $record = [];
        $record['price'] = $money;
        $record['goodsprice'] = $money;

        // 更新订单金额
        pdo_update('sz_yi_order', $record, array('id' => $order_id));
		...
		if (($operation == 'complete') && $_W['ispost']) {
			...
			if ($log['status'] != 1) {
				...
				$ret['weid'] = $log['weid'];
				$ret['uniacid'] = $log['uniacid'];
				$ret['trade_no'] = $_GPC['trade_no'];  //peng 171018 支付单号
				$this->payResult($ret);
			}
