...
if ($_W['isajax']) {
	...
	if ($operation == 'display') {
		...
		if ($isverify) {
			...
			foreach($stores as $kk => $vv){
				...
				if($stores[$kk]['storename'] == $shop_name){
					$arrb = $stores[0];
					$stores[0] = $stores[$kk];
					$stores[$kk] = $arrb;
				}
			...
			$arra = array();
			foreach($stores_send as $k1 => $v1){
				
				if($stores_send[$k1]['storename'] == $shop_name){
					$arra = $stores_send[0];
					$stores_send[0] = $stores_send[$k1];
					$stores_send[$k1] = $arra;
				}
			}
		...
		if( $store_total ){
			...
			if( !empty($carrier) ){
				...
				$sql = 'SELECT total FROM '.tablename('sz_yi_store_goods').' WHERE goodsid=:id AND storeid=:sid AND uniacid=:uniacid';
				$stock = pdo_fetchcolumn( $sql,array(':id'=>$id,':sid'=>$carrier['id'],':uniacid'=>$_W['uniacid']) );
				$stock = $stock ? $stock : 0 ;
			}
		}
		// 20170815 如果是核销商品
		...
		show_json(1, array('member' => $member, 'deductmoney' => $deductmoney, 'deductcredit2' => $deductcredit2, 'saleset' => $saleset, 'goods' => $goods, 'has' => $has, 'weight' => $weight / $buytotal, 'set' => $shopset, 'fromcart' => $fromcart, 'haslevel' => !empty($level) && (0 < $level['discount']) && ($level['discount'] < 10), 'total' => $total, 'totalprice' => number_format($totalprice, 2), 'goodsprice' => number_format($goodsprice, 2), 'discountprice' => number_format($discountprice, 2), 'discount' => $level['discount'], 'realprice_total' => number_format($realprice_total, 2), 'address' => $address, 'carrier' => $stores[0], 'carrier_list' => $stores, 'carrier_send' => $stores_send[0], 'carrier_list_send' => $stores_send, 'dispatch_list' => $dispatch_list, 'isverify' => $isverify, 'isverifysend' => $isverifysend, 'dispatchsend' => $dispatchsend, 'stores' => $stores, 'stores_send' => $stores_send, 'isvirtual' => $isvirtual, 'changenum' => $changenum, 'order_all' => $order_all, 'supplierids' => $supplierids, 'deposit' => number_format($deposit, 2), 'price_list' => $price_list, 'realprice' => number_format($realprice, 2), 'type' => $goods[0]['type'],'stock'=>$stock));
	}
	...
	//xj 20170815 新增 改变店铺库存 s
		elseif($operation == 'changeStock'){
			$storeId = intval($_GPC['store']);
			$goodsId = intval($_GPC['id']);
			$sql = 'SELECT total FROM '.tablename('sz_yi_store_goods').' WHERE goodsid=:id AND storeid=:sid AND uniacid=:uniacid';
			$condition = array(':id'=>$goodsId,':sid'=>$storeId,':uniacid'=>$_W['uniacid']);
			$stockNum = pdo_fetchcolumn($sql,$condition);
			show_json(1,array('number'=>$stockNum));
		}
		//xj 20170815 新增 改变店铺库存 e
	else {//创建订单2017-12-18
		if (($operation == 'create') && $_W['ispost']) {
			...
			$ordersn_general = m('common')->createNO('order', 'ordersn', 'GG');
			$member = m('member')->getMember($openid);
			$level = m('member')->getLevel($openid);
			$can_buy = array();
			...
			foreach ($order_data as $key => $order_row) {
				...
				foreach ($goodsarr as $g) {
					...
					$goodsinfo = explode(',', $g);
					$goodsid = (!empty($goodsinfo[0]) ? intval($goodsinfo[0]) : '');
					$cartGoodsId[] = $goodsid; //xj 新增把确购物车确认订单的商品id添加到清空购物车的条件 批量删除购物车
					$optionid = (!empty($goodsinfo[1]) ? intval($goodsinfo[1]) : 0);
					$goodstotal = (!empty($goodsinfo[2]) ? intval($goodsinfo[2]) : '1');
					...
					if (!empty($optionid)) {
						...
						if (!empty($option)) {
							if ($option['stock'] != -1) {
								// han 20170925 || (empty($data['totalcnf']) && ($option['stock'] < $data['total']))
								if (empty($option['stock']) || (empty($data['totalcnf']) && ($option['stock'] < $data['total']))) {
						...
						if ($data['stock'] != -1) {
							// han 20170925 || (empty($data['totalcnf']) && ($data['stock'] < $data['total']))
							if (empty($data['stock']) || (empty($data['totalcnf']) && ($data['stock'] < $data['total']))) {
					...
					$redprice = intval($redprice) * intval($goodstotal);
					...
					$gprice = intval($data['marketprice']) * intval($goodstotal);
					$goodsprice += $gprice;
					$ggprice = 0;
				...
				if ($order_row['fromcart'] == 1) {
					...
					if (empty($data['optionid'])) { //lotodo 20170506 下单成功时购物车不会情况，因$data['optionid']为NULL，与数据库库内的项的类型不符，加判断令其为0
						$data['optionid']=0;
					}

					$cardGoodsIdStr = implode(',',$cartGoodsId); //xj购物车商品Id  批量删除购物车
...
//						pdo_query('update ' . tablename('sz_yi_member_cart') . ' set deleted=1 where id in (' . $cartids . ') and openid=:openid and goodsid=:goodsid and optionid=:optionid and uniacid=:uniacid ', array(':uniacid' => $uniacid, ':openid' => $openid, ':goodsid' => $data['goodsid'], ':optionid' => $data['optionid']));
						//xj批量删除购物车商品
						pdo_query('update ' . tablename('sz_yi_member_cart') . ' set deleted=1 where id in (' . $cartids . ') and openid=:openid and goodsid IN ('.$cardGoodsIdStr.') and optionid=:optionid and uniacid=:uniacid ', array(':uniacid' => $uniacid, ':openid' => $openid, ':optionid' => $data['optionid']));
					}
					else {
