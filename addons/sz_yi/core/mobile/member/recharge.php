...
if (($operation == 'display') && $_W['isajax']) {
	...
	// han 20171018 删除订单改为改变订单状态
	// pdo_delete('sz_yi_member_log', array('openid' => $openid, 'status' => 0, 'type' => 0, 'uniacid' => $_W['uniacid']));
	pdo_update('sz_yi_member_log', array('status'=>-1), array('openid' => $openid, 'status' => 0, 'type' => 0, 'money' => 0, 'uniacid' => $_W['uniacid']));
	// han end
	...
	if (($operation == 'recharge') && $_W['ispost']) {
		...
		if (($operation == 'complete') && $_W['ispost']) {
			...
			if (!empty($log) && empty($log['status'])) {
				...
				//han 171020 新增备注 , array(0, '会员余额充值' . $log['money'] . ' 元')
