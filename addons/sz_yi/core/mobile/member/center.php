...
if ($_W['isajax']) {
	...
	/*xj 20170426 会员中心酒店订单模块显示*/
	$sql='SELECT COUNT(id) FROM '.tablename('sz_yi_goods').' WHERE type=99 AND uniacid=:uniacid';
	$isHotel=pdo_fetchcolumn($sql,array(':uniacid'=>$_W['uniacid']));
	/*xj 20170426 会员中心酒店订单模块显示*/

	//xj新增酒店订单模块显示'is_hotel'=>$isHotel
	show_json(1, array('member' => $member, 'referrer' => $referrer, 'shop_set' => $shop_set, 'order' => $order, 'orderhotel' => $orderhotel, 'memberhotel' => $memberhotel, 'level' => $level, 'open_creditshop' => $open_creditshop, 'counts' => $counts, 'shopset' => $shopset, 'trade' => $trade, 'app' => $app,'is_hotel'=>$isHotel));
}
