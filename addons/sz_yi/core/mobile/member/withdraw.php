...
// han 20171013
$on_withdraw = 0;
if($set['trade']['withdrawtime'] == 1) {
	$wday = localtime();
	$wday = $wday[6];
	if(in_array($wday, $set['trade']['withdrawpermit'])){
		$on_withdraw = 1;
		
	}else {
		$str_wday = '';
		foreach ($set['trade']['withdrawpermit'] as $value) {
			switch ($value) {
				case '1':
					$str_wday .= "周一";
					break;
		...
		$notiy = "提现开放时间为".$str_wday;
	}
}elseif($set['trade']['withdrawtime'] == 0){
	$on_withdraw = 1;
}
// end
...
if (($operation == 'display') && $_W['isajax']) {
	...
	if (($operation == 'submit') && $_W['ispost']) {
		...
		$pay_mode = $_GPC['pay_mode']; //peng 20170224 
		...
		$logdata = array('uniacid' => $uniacid, 'logno' => $logno, 'openid' => $openid, 'title' => '余额提现', 'type' => 1, 'createtime' => time(), 'status' => 0, 'money' => $actual, 'poundage' => $poundage, 'withdrawal_money' => $money,'pay_mode' =>$pay_mode);
