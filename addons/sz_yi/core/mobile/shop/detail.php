...
if (is_weixin()){
	...
	$user = pdo_get('sz_yi_member',['unionid'=>$fans_info['unionid']]);
	if (!$user){
		//跳转认证
		$url = 'Location:/app/index.php?i=' . $_W['uniacid'] . '&c=entry&p=member_auth&do=member&m=sz_yi&from=app&mid=' . $_GPC['mid'];
		return header($url);
		//echo $fans_info['unionid'];
	}
	$isCertifiedOtherApp = pdo_get('sz_yi_member',['openid'=>$_W['openid'],'uniacid'=>$_W['uniacid']],['id','unionid','mobile','id_card']);
	//待新增其他公众认证添加本公众信息
	...
	// $isRealName = pdo_get('sz_yi_member',['openid'=>$_W['openid']],['id_card']);
}
...
$merchant = pdo_get('sz_yi_perm_user',['uid'=>$goods['supplier_uid']]);
if ($goods['pcate']) {
...
if ($_W['isajax']) {
	...
	pdo_query('update ' . tablename('sz_yi_goods') . ' set viewcount=viewcount+3 where id=:id and uniacid=\'' . $uniacid . '\' ', array(':id' => $goodsid));
