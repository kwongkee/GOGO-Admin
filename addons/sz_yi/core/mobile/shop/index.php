...
$s = m('cache')->get('sysset');
$yzShopSet = iunserializer($s['sets'])['shop'];
...
if ($operation == 'index') {
	...
	if ($operation == 'goods') {
		$type = $_GPC['type'];
		$args = array('page' => $_GPC['page'], 'pagesize' => 6, 'isrecommand' => 1, 'order' => 'displayorder asc,createtime asc', 'by' => '');
...
if ($_W['isajax']) {
	if ($operation == 'index') {
		...
		foreach ($category as $key => $value) {
			...
			foreach ($children as $key1 => $value1) {
				$category[$key]['children'][$key1] = $value1;
				$ch_goods = set_medias(pdo_fetchall(' select * from ' . tablename('sz_yi_goods') . ' where ( pcate=:pcate and ccate=:ccate or find_in_set(\'' . $value['id'] . '\',pcates) )  and uniacid=:uniacid and isrecommand =1 and deleted = 0 and status = 1 limit 8', array(':pcate'=>$value1['parentid'],':ccate' => $value1['id'], ':uniacid' => $_W['uniacid'])), 'thumb');
				$category[$key]['children'][$key1]['ch_goods'] = $ch_goods;
