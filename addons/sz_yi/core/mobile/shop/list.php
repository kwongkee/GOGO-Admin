...
if(empty($_GPC['order']))
{
	$args['order'] = 'displayorder asc,createtime asc';
}
...
if (!empty($maxprice) || !empty($minprice)) {
	$goods = set_medias(pdo_fetchall('SELECT * FROM ' . tablename('sz_yi_goods') . ' where 1 ' . $condition .' ORDER BY  displayorder asc,createtime asc', $params), 'thumb');
}
else {
