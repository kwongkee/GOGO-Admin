...
if ($_W['isajax']) {
	...
	if ($operation == 'display') {
		...
		if ( $cartids ) {
			//$condition .= ' and f.id in (' . $cartids . ')';
			$condition = ' and f.uniacid= :uniacid and f.openid=:openid and f.deleted=0 and f.id in (' .$cartids. ')';
		}else{
		...
		$sql = 'SELECT f.id,f.total,' . $channel_condtion . $yunbi_condtion . 'f.goodsid,g.total as stock, o.stock as optionstock, g.maxbuy,g.title,g.thumb,ifnull(o.marketprice, g.marketprice) as marketprice,g.productprice,o.title as optiontitle,f.optionid,o.specs,p.id as pid,p.username FROM ' . tablename('sz_yi_member_cart') . ' f ' . ' left join ' . tablename('sz_yi_goods') . ' g on f.goodsid = g.id ' . ' left join ' . tablename('sz_yi_goods_option') . ' o on f.optionid = o.id left join '.tablename('sz_yi_perm_user') .' p on g.supplier_uid=p.uid where 1 ' . $condition . ' ORDER BY `id` DESC ';
		//echo $sql;
		...
		foreach ($list as $value){
			$lists[$value['username']][]=$value;
		}
		show_json(1, array('total' => $total, 'list' => $lists, 'totalprice' => $totalprice, 'difference' => $difference, 'ischannelpay' => $ischannelpay, 'verify_goods_ischannelpick' => $verify_goods_ischannelpick, 'virtual_currency' => $virtual_currency, 'yunbi_title' => $yunbi_title));
	}
	else {
						...
						if (($operation == 'remove') && $_W['ispost']) {
							...
							if (($operation == 'cart') && $_W['ispost']) {
								...
								if (!empty($cartids)) {
									...
									$data = pdo_fetchall('select * from ' . tablename('sz_yi_member_cart') . ' where openid=:openid and deleted=0 and id in (' . $cartids . ') and uniacid=:uniacid ', array(':uniacid' => $uniacid, ':openid' => $openid));
								}else{
