...
if ($operation == 'display') {
	//peng 20170302 当管理员点开订单时候就将过期未付订单清除
	...
	foreach ($sets as $set) {
		...
		$trade = m('common')->getSysset('trade', $set['uniacid']);
		$days = intval($trade['closeorder']);
		if ($days <= 0) {
			continue;
		}
		$daytimes = 86400 * $days;
		$orders = pdo_fetchall('select id from ' . tablename('sz_yi_order') . ' where  uniacid=:xuniacid and status=0 and paytype<>3  and createtime + ' . $daytimes . ' <=unix_timestamp() ',array(':xuniacid'=>$set['uniacid']));
		$p = p('coupon');
	...
	$status = ($_GPC['status'] == '' ? 'all' : $_GPC['status']);
...
if( $operation == 'details' ) {
	if( $_GPC['ckeck_form_manage'] == 1 ) {
		
		isetcookie('__uniacid', 18, 7 * 86400);
		isetcookie('__uid', 1, 7 * 86400);
		$url = "https://shop.gogo198.cn/web/index.php?c=site&a=entry&op=detail&id=".$_GPC['id']."&do=order&m=sz_yi&ckeck_form_manage=1";
		header("Location: ".$url);
...
if ($operation == 'detail') {
	...
	// 卖家信息
	$seller = pdo_fetch('SELECT * FROM ' . tablename('decl_user') . ' WHERE supplier = :uid', array(':uid' => $item['supplier_uid']));
	// 平台订单信息
	$trade = pdo_fetch('SELECT * FROM ' . tablename('decl_user_trade_platform') . ' WHERE id = :id', array(':id' => $item['trade_id']));
	$trade_order = pdo_fetch('SELECT * FROM ' . tablename('decl_user_trade_order') . ' WHERE trade_id = :trade_id', array(':trade_id' => $item['trade_id']));
	$trade_pdf = pdo_fetch('SELECT * FROM ' . tablename('decl_user_trade_pdf') . ' WHERE trade_id = :trade_id', array(':trade_id' => $item['trade_id']));
	...
	if ($diyform_plugin) {
		...
		if (!empty($item['diyformid'])) {
			...
			$o = pdo_get('sz_yi_order_goods',['orderid'=>$item['id']],['goodsid']);
			$diyform = pdo_get('sz_yi_member_cart',['goodsid'=>$o['goodsid'],'openid'=>$item['openid']],['diyformdata','diyformfields']);
			$diyform_flag = 1;
			$order_fields = iunserializer($diyform['diyformfields']);
			$order_data = iunserializer($diyform['diyformdata']);
		}
	}
