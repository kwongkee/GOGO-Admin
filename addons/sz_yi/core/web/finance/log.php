...
if ($op == 'display') {
	...
	$sql = 'select log.id,log.aging_id,m.id as mid, m.realname,m.diymemberdata,m.avatar,m.weixin,log.logno,log.type,log.status,log.rechargetype,m.nickname,m.mobile,g.groupname,log.money,log.pay_mode,log.poundage,log.createtime,l.levelname from ' . tablename('sz_yi_member_log') . ' log ' . ' left join ' . tablename('sz_yi_member') . ' m on m.openid=log.openid' . ' left join ' . tablename('sz_yi_member_group') . ' g on m.groupid=g.id' . ' left join ' . tablename('sz_yi_member_level') . ' l on m.level =l.id' . ' where 1 ' . $condition . ' ORDER BY log.createtime DESC ';
	...
	if ($_GPC['export'] == 1) {
		...
		$columns = array(
			...
			array('title' => empty($type) ? '赠送金额' : ' ', 'field' => 'gives', 'width' => 12),
	...
	//xj 170918 提现总额 s
	$sql = 'select SUM(log.money) AS tot,SUM(log.poundage) from ' . tablename('sz_yi_member_log') . ' log ' . ' left join ' . tablename('sz_yi_member') . ' m on m.openid=log.openid' . ' left join ' . tablename('sz_yi_member_group') . ' g on m.groupid=g.id' . ' left join ' . tablename('sz_yi_member_level') . ' l on m.level =l.id' . ' where 1 ' . $condition . ' ORDER BY log.createtime DESC';
	$withdrawInfo = pdo_fetch($sql,$params);
	//xj 170918 提现总额 e
	...
	if ($op == 'pay') {
		...
		if ($paytype == 'manual') {
			...
			$date = pdo_fetch('select * from '.tablename('sz_yi_member_log').'where id=:id ',array(':id'=>$id));
			$userinfo = pdo_fetch('select * from '.tablename('sz_yi_member').'where openid=:openid ',array(':openid'=>$date['openid']));
			$zhifubaozhanghao = $userinfo['alipay'];
			$zhifubaorealname = $userinfo['alipayname'];
			$money = $date['money'];
			$zhanghao = $date['logno'];
			$title = $date['title'];
			$aliname = $set['pay']['alipay_name'];
			$title = $_W['account']['name'].'-'.$title;
		
			
			date_default_timezone_set('PRC');
			//peng 20170324 支付宝提现 SZ_YI_INC . 'json/xml2json.php';  alipay_privatekey
			require_once  SZ_YI_INC .'zhifubao/aop/AopClient.php';
			require_once  SZ_YI_INC .'zhifubao/aop/request/AlipayFundTransToaccountTransferRequest.php';
			    
				
				$aop = new AopClient ();
				$aop->gatewayUrl = 'https://openapi.alipay.com/gateway.do';
				$aop->appId = $set['pay']['alipay_appid'];   //2088411997697889  	
				$aop->rsaPrivateKey = $set['pay']['alipay_privatekey'];
				$aop->alipayrsaPublicKey= $set['pay']['alipay_publickey'];
				$aop->apiVersion = '1.0';
				$aop->signType = 'RSA2';
				...
				"    \"payee_account\":\"$zhifubaozhanghao\"," .
				"    \"amount\":\"$money\"," .
				"    \"payer_real_name\":\"$aliname\"," .
				"    \"payer_show_name\":\"$aliname\"," .
				"    \"payee_real_name\":\"$zhifubaorealname\"," .
				"    \"remark\":\"转账备注\"," .
				"    \"ext_param\":\"{\\\"order_title\\\":\\\"$title\\\"}\"" .
				"  }");
