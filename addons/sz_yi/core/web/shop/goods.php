 ...
 *  读取敏感词关键字
 */
$keyWord = pdo_getall('keyword', array('id >' => 0),['keyword']);
// 把关键词变为一维数组
$KeyWords = [];
...
unset($keyWord);
// 键值互换
$badword = array_combine($KeyWords,array_fill(0,count($KeyWords),'*'));
unset($KeyWords);
// end 2019-07-31
...
if ($operation == 'change') {
	...
	unset($value);
	//xj 220170112 修复添加多规格不成功  $_POST['token']   !empty($_GPC['goodsname']
	if (checksubmit('submit') || $_POST['token'] || !empty($_GPC['goodsname'])) {
		...
		if (empty($id)) {
			...
			//xj 新增线下核销 s 20170815
			if($data['isverify'] == 2){
				//如果选择了门店 则更新门店的库存列表
				if($data['storeids']){
					$storeList = explode(',',$data['storeids']);

					foreach($storeList as $v){
						//查询是否已经存在核销门店库存里
						$sql ='SELECT id FROM '.tablename('sz_yi_store_goods').' WHERE storeid=:sid  AND goodsid=:goods_id AND uniacid=:uniacid';
						$paramsStroe = array(
							':sid'	=> $v,
							':uniacid'	=>  $_W['uniacid'],
							':goods_id'	=>	$id
						);
						$isStr = pdo_fetchcolumn($sql,$paramsStroe);
						...
						pdo_insert('sz_yi_store_goods',$dataStroe);
					}
				}else{
					//所有门店都添加这个商品
					$sql = 'SELECT id FROM '.tablename('sz_yi_store').' WHERE status=1 AND uniacid=:uniacid';
	...
	if ($operation == 'copygoods') {
		...
		$status=intval($_GPC['status']); //xj 20170504 已上架/已下架
		...
		//xj 新增 array('status'=>$status)
		message('商品复制成功！', $this->createWebUrl('shop/goods',array('status'=>$status)), 'success');
