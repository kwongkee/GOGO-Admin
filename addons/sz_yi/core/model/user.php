...
class Sz_DYi_User
{
	...
	public function isLogin()
	{
		...
		@session_start();
		$cookieid = '__cookie_sz_yi_userid_' . $_W['uniacid'];
		// 2020-02-20  修改前：$openid = base64_decode($_COOKIE[$cookieid]);
		$open = isset($_COOKIE[$cookieid]) ? $_COOKIE[$cookieid] : (isset($_SESSION[$cookieid]) ? $_SESSION[$cookieid] : '' );
		$openid = base64_decode($open);
	...
	public function getUserInfo()
	{
		...
		if( $_GPC['op'] == 'form_manage' )
		{

			$userinfo = array('openid' => '', 'headimgurl' => '');
			return $userinfo;
		}else{
	...
	public function getInfo($base64 = false, $debug = false)
	{
		...
		if (SZ_YI_DEBUG) {
			...
			if (empty($_GPC['directopenid'])) {
				...
				//lotodo 20170710 避免未关注用户重复授权登录
				if (empty(mc_openid2uid($_SESSION['openid']))) {
					$userinfo = mc_oauth_userinfo();
				}else{
					//lotodo 20170711
					$userinfo = mc_fetch($_SESSION['openid'], array('nickname', 'realname', 'gender', 'mobile', 'avatar', 'resideprovince', 'residecity', 'residedist'));
