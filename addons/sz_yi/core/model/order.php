...
class Sz_DYi_Order
{
		...
		if ($params['from'] == 'return') {
			...
			if ($order['status'] == 0) {
				...
				if (!empty($order['virtual']) && $pv) {
					...
					$params['trade_no'] = isset($params['trade_no']) && !empty($params['trade_no']) ? $params['trade_no'] : '0';
					if (p('channel')) {
						if ($params['ischannelpay'] == 1) {
							...
							pdo_query('update ' . tablename('sz_yi_order') . ' set status=1, trade_no ='. $params['trade_no'].' , paytime=' . time() . ', pay_ordersn=ordersn_general, ordersn_general=ordersn where ' . $order_update . ' and uniacid=\'' . $uniacid . '\' ');
						}
					}
					else {
						pdo_query('update ' . tablename('sz_yi_order') . ' set status=1, trade_no ='. $params['trade_no'].', paytime=' . time() . ', pay_ordersn=ordersn_general, ordersn_general=ordersn where ' . $order_update . ' and uniacid=\'' . $uniacid . '\' ');
	...
	public function sendSms($mobile, $code, $templateType = 'reg', $type = 'check', $name = '', $title = '', $total = '', $tel = '')
	{
		...
		if ($set['sms']['type'] == 3) { //20170911 han
			return send_sms_aliyun($mobile, $code, $templateType);
		}
