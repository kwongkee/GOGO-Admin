...
class Sz_DYi_Member
{
	...
	public function checkMember($openid = '')
	{
		...
		if (empty($member)) {
			...
			$member = array('uniacid' => $_W['uniacid'], 'uid' => $uid, 'openid' => $openid, 'realname' => !empty($mc['realname']) ? $mc['realname'] : '', 'mobile' => !empty($mc['mobile']) ? $mc['mobile'] : '', 'nickname' => !empty($mc['nickname']) ? filterNickname($mc['nickname']) : filterNickname($userinfo['nickname']), 'avatar' => !empty($mc['avatar']) ? $mc['avatar'] : $userinfo['avatar'], 'gender' => !empty($mc['gender']) ? $mc['gender'] : $userinfo['sex'], 'province' => !empty($mc['residecity']) ? $mc['resideprovince'] : $userinfo['province'], 'city' => !empty($mc['residecity']) ? $mc['residecity'] : $userinfo['city'], 'area' => !empty($mc['residedist']) ? $mc['residedist'] : '', 'createtime' => time(), 'status' => 0);
			$bindMobile = true;
			...
			if (!empty($userinfo['nickname']) && ($userinfo['nickname'] != $member['nickname'])) {
				//xj 修改
				$upgrade['nickname'] =  filterNickname($userinfo['nickname']);
			}
	...
	public function upgradeLevel($openid, $orderid = '')
	{
		...
		if ($isup) {
			//pdo_update('sz_yi_member', array('level' => $level['id'], 'upgradeleveltime' => time()), array('id' => $member['id'])); //lotodo 20170121
	...
	public function autoexec($uniacid = 0)
	{
		...
		if ($shopset['term']) {
			...
			//$level = pdo_fetch('UPDATE ' . tablename('sz_yi_member') . ' SET level = \'0\', upgradeleveltime = ' . $current_time . ' where uniacid=:uniacid and level > 0 and (' . $current_time . ' - upgradeleveltime ) >=  ' . $termtime, array(':uniacid' => $_W['uniacid'])); //lotodo 20170121
			$level = pdo_fetch('UPDATE ' . tablename('sz_yi_member') . ' SET level = \'0\' where uniacid=:uniacid and level > 0 ', array(':uniacid' => $_W['uniacid']));
		}
	}
}
