...
function file_upload_all($file,$type='image',$name=''){
    //校验上传类型
...
//    }
    // if(!in_array($ext,array('gif','jpg','jpeg','png','pdf','xlsx','xls','doc','docx')) || in_array($ext,$harmtype)){
    //     return error(-3, '不允许上传此类文件');
    // }
...
function file_upload_xin($file,$type='image',$name=''){
    //校验上传类型
    $harmtype = array('asp', 'php', 'jsp', 'js', 'css', 'php3', 'php4', 'php5', 'ashx', 'aspx', 'exe', 'cgi');
    if (empty($file)) {
        return error(-1, '没有上传内容');
