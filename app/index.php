...
if(!empty($user)){
    $user_company = pdo_fetch('select * from '.tablename('website_user_company').' where user_id=:user_id',[':user_id'=>$user['id']]);
    if(empty($user_company)){
        $user_company = pdo_fetch('select * from '.tablename('centralize_manage_person').' where gogo_id=:gogo_id and status=1',[':gogo_id'=>$user['id']]);
        if(empty($user_company)){
            #跳转到个人中心
            header('Location: https://boss.gogo198.cn/?s=index/member_login&uid='.base64_encode($user['id']));
        }
    }
}
else{
    #跳转到购购网登录
    header('Location: https://www.gogo198.net/?s=index/customer_login');
}
