...
$do = in_array($do, array('list', 'detail', 'handsel','adv_content')) ? $do : 'list';
load()->model('site');
load()->model('mc');
...
if($do == 'list') {
	...
	$sql1="SELECT * FROM ".tablename('foll_advertising_content').' WHERE `id` = '.$item_id;
	$adv_content=pdo_fetch($sql1);
...
}elseif($do=='adv_content'){
	$adv_id= $_GPC['item_id'];
	$view_times = $_GPC['times'];
	$uniacid=$_W['uniacid'];
	$openid="'".$_W['openid']."'";
	if($openid==''){
		$openid='web';
...
//	$result = pdo_insert('foll_advertising_view', $data);
//	if($result){
//		exit(json_encode('1','success'));
//	}
//添加到广告浏览表,新增记录 openid 不是微信获取不到
	$sql="INSERT INTO ".tablename('foll_advertising_view').'(`adv_id`,`view_times`,`uniacid`,`openid`,`create_time`) VALUES ('.$adv_id.','.$view_times.','.$uniacid.','.$openid.',"'.time().'")';
	$result_data=pdo_query($sql);
	if($result_data){
		exit(json_encode(array('1','success')));
	}
