...
function site_navs($params = array()) {
	...
	if(empty($navs)) {
	    //2022-04-06 通过openid查询自己的权限
	    $decl_user = pdo_fetch('select * from '.tablename('decl_user').' where openid=:openid',[':openid'=>$_W['openid']]);
	    if((empty($decl_user) || empty($decl_user['sys_app'])) && $multiid==45){
	        //不是商户时，只显示注册商户或凭证汇总或跨境政策
	        $is_accounting = pdo_fetch('select fanid from '.tablename('mc_mapping_fans').' where openid=:openid and is_accounting=1',[':openid'=>$_W['openid']]);
	        foreach ($navs as &$row) {
    			if(!$cid || !$ishomepage) {
    			...
    			if($is_accounting['fanid']>0){
    			    //记账代理    
    			    if($row['name']!='凭证汇总' && $row['name']!='商户系统' && $row['name']!='跨境政策'){
        			   unset($row);
        			}
    			}else{
    			    //普通粉丝
    			    #在总后台->海关申报系统->商户管理->公众号应用可设置
    			    $can_add = pdo_fetchcolumn('select id from '.tablename('offical_account_application').' where id=1 and find_in_set('.intval($row['id']).',can_add_app)');
    			 
    			    if(empty($can_add)){
    			        unset($row);
    			    }
    			 
    			 //   if($row['name']!='商户系统' && $row['name']!='跨境政策' && $row['name']!='审批管理'){
        // 			   unset($row);
        // 			}
    			}
    			//
    			$row['css']['name'] = "color:{$row['css']['name']['color']};";
    			$row['html'] = '<a href="'.$row['url'].'" class="box-item" style="width:120px !important;position:relative;">';
    			$row['html'] .= '<i '.(!empty($row['icon']) ? "style=\"background:url('".tomedia($row['icon'])."') no-repeat;background-size:100%;\" class=\"icon\"" : "class=\"fa {$row['css']['icon']['icon']} \" style=\"{$row['css']['icon']['style']}\"").'></i>';
	        ...
	        foreach ($navs as &$row) {
    			...
    			if($multiid==45){
    			 //   if($row['name']!='跨境政策'){
        			    ...
        			    if(empty($can_add)){
        			        #如果不是全部可见
        			        if(!in_array($row['id'],explode(',',$decl_user['sys_app']))){
        			            #如果不在商户可见列表
                			   unset($row);
                			}    
        			    }
	    ...
	    foreach ($navs as &$row) {
			...
			$row['css']['icon']['style'] = "color:{$row['css']['icon']['color']};font-size:{$row['css']['icon']['font-size']}px;";
			$row['css']['name'] = "color:{$row['css']['name']['color']};";
			if($row['name']=='购购网' || $row['name']=='集运网'){
			    #2023.10.30，微信打开该网址即可免登录（待做，因为还不知道该openid的手机号/邮箱号）
			    $row['url'] = str_replace('#wechat_redirect','?openid='.$_W['openid'],$row['url']);
			}
	        $row['html'] = '<a href="'.$row['url'].'" class="box-item" style="width:120px !important;position:relative;">';
			$row['html'] .= '<i '.(!empty($row['icon']) ? "style=\"background:url('".tomedia($row['icon'])."') no-repeat;background-size:100%;\" class=\"icon\"" : "class=\"fa {$row['css']['icon']['icon']} \" style=\"{$row['css']['icon']['style']}\"").'></i>';
			$row['html'] .= "<span style=\"{$row['css']['name']}\" title=\"{$row['name']}\">{$row['name']}</span></a>";
