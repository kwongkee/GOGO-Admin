 ...
 */
  ...
  * 	'token' => $receive['pay_type'],//支付类型如：wechat,alipay,...
		...
		'orderId'=>$_GPC['order'],//旧订单编号  用于未覆盖的订单编号
  * 
  * 判断表中是否存在旧订单编号，
...
if (!empty($_POST) && isset($_POST['token'])) {
	...
	if (in_array($_POST['token'], $type)) {//判断接收到的支付类型是否在数组中
		...
		$orderid = pdo_get('pay_order', array('ordersn' => $_POST['orderId']), array('id','ordersn','upOrderId'));
		/**
		 * 组装条件
		 */
		$updata = [
		...
		if(!empty($orderid)) {//如果订单表中存在该条订单
			...
			if (!empty($result)) {
			   //查询表中对应的订单编号是否有未支付订单
//			   	$field = 'uniacid', 'openid','ordersn','upOrderId','PayAmount','body','pay_type','pay_status','create_time','returnUrl'
				$field = 'uniacid,ordersn,upOrderId,pay_money,body,create_time';
				$parms = pdo_get('foll_advertising_order', array('ordersn' => $_POST['ordersn']), array($field));
				if(!$parms) {
...
//				$payconfig = unserialize($configs['config']);
				...
				$params = array(
					//支付金额  a.pay_account
					'payMoney'		=> $parms['pay_money'],
					//支付订单号  a.ordersn
					'ordersn'		=>$parms['ordersn'],
					//上游订单号  b.upOrderId
					'upOrderId'		=>$parms['upOrderId'],
					//支付类型	a.pay_type
					'payType'		=> $updata['payType'],
					//商品描述	a.body
					'body'			=>$parms['body'],
					//用户Openid  a.user_id  测试：om44I1X2wIlTWdN3pSsgoJkfMLtI
					'openid'		=> '',				
					//订单提交时间	a.create_time
					'create_time' 	=> $parms['create_time'],
					//商户号
					'account' 		=> $config['account'],
					//支付状态  0订单已提交，1支付成功，2支付失败
					'status' 		=> '0',
				);
