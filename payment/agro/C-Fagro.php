...
//	header("Content-Type:text/html;charset=GBK");
	...
	if(!empty($_POST) && isset($_POST['Token']))
	{
		...
		$address 	= '192.168.251.10';//农商行IP地址
		$port 		= '39031';
		$BkEntrNo   = '04000000050';//单位编号 11	char
		$BkAcctNo 	= '801101000974338949';//委托单位账号	32  char 801101000588712434	  801101000961488367
		...
		if(in_array($Token,$type)) {
			...
			switch($Token) {
				case 'FeeDeduction'://扣费  100015
					 ...
					 */
					$filed = 'a.uniacid,a.pay_account,a.ordersn,a.create_time,a.address,a.user_id,a.body,b.OthSeq,b.starttime,b.endtime,b.duration,a.total ';
...
//					file_put_contents('./log/OrdData.txt', print_r($OrdData,TRUE));
					...
					if(!empty($OrdData)) {
						...
						file_put_contents('./log/FeeResult.txt', print_r($Result,TRUE)."\r\n",FILE_APPEND);
						...
						if( ($Result['Result'] == '00000') && ($Result['Message'] == '成功完成') )
						{
							...
							try
							{
								pdo_begin();//开启事务
									...
									pdo_update('pay_old',array('status' => 1,'update_time'=>time(),'SeqNo'=>$Result['SeqNo'],'Msg'=>$Result['Message'],'PlatDate'=>$Result['PlatDate']), array('ordersn'=>$oldOrdersn));
									//支付成功修改parking_order 表中状态：支付成功  SeqNo：银行流水；
									pdo_update('parking_order', array('upOrderId'=>$Result['SeqNo'],'status'=>'已结算','PlatDate'=>$Result['PlatDate']),array('OthSeq' => $Result['OthSeq']));
							...
							try
							{
								pdo_begin();//开启事务
									...
									pdo_update('pay_old', array('status' => 2,'update_time'=>time(),'SeqNo'=>$Result['SeqNo'],'Msg'=>$Result['Message']), array('ordersn'=>$oldOrdersn));
									//支付成功修改parking_order 表中状态：支付成功  SeqNo：银行流水；
									pdo_update('parking_order', array('upOrderId'=>$Result['SeqNo'],'status'=>'未结算','PlatDate'=>$Result['PlatDate']), array('OthSeq' => $Result['OthSeq']));
...
//						echo json_encode(['status'=>102,'msg'=>'请检查订单编号']);
//						die;
//						return $res=['status'=>102,'msg'=>'请检查订单编号'];
						$res=json_encode(['status'=>102,'msg'=>'error']);
						echo $res;
						exit();
					}
				...
				case 'WriteOff'://冲销  100019
					...
					$res = $first->socket_client($address,$port,$strs);
					//返回结果；
					file_put_contents('./log/Write.txt', print_r($res,TRUE)."\r\n",FILE_APPEND);
				...
				case 'Surrender':
					...
					$res = $first->socket_client($address,$port,$strs);
					//返回结果；
					file_put_contents('./log/Surr.txt', print_r($res,TRUE)."\n",FILE_APPEND);
					$Res = $first->Analysis($res);
					file_put_contents('./log/SurrResult.txt', print_r($Res,TRUE));
//					if($Res['Result'] == '00000' && $Res['Message'] == '成功完成'){
//					if($Res['Result'] == '10000') {
					//返回json 数据；
					echo json_encode($Res);
...
//					}
				...
				case 'Reconciliation'://单笔对账文件   100018
					...
					$res = $first->socket_client($address,$port,$strs);
					//返回结果；
					file_put_contents('./log/Recon.txt', print_r($res,TRUE)."\r\n",FILE_APPEND);
					file_put_contents('../../crontab/sd/Recon'.date('Ymd',$OldDate).'.txt',$res,FILE_APPEND);
...
//				'BkTxAmt'=> '             '.$SurrData['BkTxAmt'],
			...
			file_put_contents('./log/FeeDeduction.txt', print_r($datas,TRUE),FILE_APPEND);
			//数组转字符串；以空格分割
			$str1 = implode('',$datas);
			//数据转GB2312
