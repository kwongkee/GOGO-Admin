...
file_put_contents('./log/ewei_shopv2/alipaynotify.txt', $input."\r\n",FILE_APPEND);
...
//			pdo_update('parking_order', array('pay_status' => 1,'paytime'=> time()), array('ordersn' => $receive['lowOrderId']));
				...
				$upArr = [
					...
					'paytype'=>'22',//1:余额支付，2：在线支付,3:货到付款,21:微信支付，22：支付宝支付
		...
		$sendArr['Reurl'] = 'http://shop.gogo198.cn/app/index.php?i='.$user['uniacid'].'&c=entry&do=member&m=sz_yi';
...
//				'paytype'=>'0',//1:余额支付，2：在线支付,3:货到付款
		...
		$sendArr['Reurl'] = 'http://shop.gogo198.cn/app/index.php?i='.$user['uniacid'].'&c=entry&m=ewei_shopv2&do=mobile&r=order&status=0';
...
//	$k = $key['tg']['key'];
	...
	file_put_contents('./log/ewei_shopv2/alipaynotifyJson.txt', serialize($answer),FILE_APPEND);
	//将数据转换成json数据返回
