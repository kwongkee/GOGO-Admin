...
if(!empty($_GET['Token']) && !empty($_GET['CarNo']))
{
	//车辆入场；
	$Token  = trim($_GET['Token']);//token
	$CarNo  = trim($_GET['CarNo']);//车牌号
	...
	if(in_array($Token,$type))//检测Token;
	{
		...
		switch($Token)
		{
			...
			case "Fee"://车辆离场扣费；
				...
				file_put_contents('./log/Fee.txt',json_encode($res)."\r\n",FILE_APPEND);
				if($res['retCode'] == 'SUCCESS') {
					echo json_encode(['status'=>1,'msg'=>$res['message']]);exit();
				} else {
					echo json_encode(['status'=>0,'msg'=>$res['message']]);exit();
	...
	echo json_encode(['status'=>0,'msg'=>'Token is Null']);
	die;
}
...
class Frx {
	...
	public function in_parking($inData = null , $inType = 'PARKING',$config) {
		...
		if($inType == 'PARKING') {
			...
			$inParking = [
				...
				'startTime'		=> date('YmdHis',time()),//'20180426151211',
				//车牌号。仅包括省份+车牌，不包括特殊字符
				'plateNumber'	=> $inData['CarNo'],//'粤YGB098',
				//停车场名称  false			 
				'parkingName'	=> '伦教停车',//'伦教停车',
				//免费时长，单位秒；
				'freeTime'		=>'1',
				//停车车辆的类型，可选值：大型车、小型车
				'carType'		=>'小型车',
	...
	public function FeeOk($params) {
		...
		//支付交易金额
		return $m = $m > 0 ? (round($m,2)*100):(round($sendArr['pay_account'],2)*100);
		//折扣计算结束
	}
