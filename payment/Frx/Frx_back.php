...
{
    $m = date('Ym',time());
    file_put_contents('./log/getDatas'.$m.'.txt', print_r($_POST,TRUE),FILE_APPEND);
    ...
    {
        //file_put_contents('./log/getToken.txt', print_r($_POST,TRUE));
        ...
        if(isset($orderNum) && ($Token == 'inPark' || $Token == 'Fee' || $Token == 'Query' || $Token == 'Refund'||$Token=='CheckCarNoSign'))
        {
            ...
            //file_put_contents('./log/inDatas1.txt', print_r($inData,TRUE),FILE_APPEND);
            ...
            if(!empty($inData) && is_array($inData)) {
                ...
                switch($Token)
                {
                    ...
                    case "Refund"://退款接口
                        ...
                        //exit(json_encode(['code'=>0,'codes'=>1,'msg'=>'success','data'=>$inData]));
...
class Frx {
    ...
    public function FeeDeduction($sendArr = null ,$parkType = 'PARKING SPACE',$config = null ) {
        ...
        file_put_contents('./log/FeeData'.date('Ym',time()).'.txt', print_r($feedata,TRUE),FILE_APPEND);
