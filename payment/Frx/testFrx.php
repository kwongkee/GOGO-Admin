			...
			if(!empty($inData) && is_array($inData)) {
				...
				switch($Token)
				{
					...
					case 'CheckCarNoSign':
						...
						echo json_encode(['status'=>1,'path'=>($res['path']?$res['path']:''),'userState'=>$res['userState'],'deductMode'=>$res['deductMode']]);exit();
						break;
					//车辆入场
            ...
            $res['ordersn'] = $inData['ordersn'];
              ...
              if($order)
              {
                pdo_update('foll_order',['deductMode'=>$res['deductMode']], array('id' => $order['id']));
							...
							echo json_encode(['status'=>1,'code'=>1,'msg'=>$res['message'],'deductMode'=>$res['deductMode']]);exit();
						} else {
							echo json_encode(['status'=>0,'code'=>0,'msg'=>$res['message'],'deductMode'=>$res['deductMode']]);exit();
						}
...
class Frx {
	...
	public $ip = 'http://jiekou-zsc.xiangfuba.cn/';
	...
	public function FeeDeduction($sendArr = null ,$parkType = 'PARKING SPACE',$config = null ) {
      ...
      if($sendArr['deductMode']=='PROACTIVE')
      {
        $feedata['openid'] = $sendArr['return_openid'];
      }
