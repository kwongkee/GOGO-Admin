 ...
 */
  ...
  * G99198101570223660201807052301
  * 
  */
  
if (!empty($_POST) && isset($_POST['token'])) {
	
	file_put_contents('./log/getTgpay'.date('Ym',time()).'.txt', print_r($_POST,TRUE),FILE_APPEND);
	/**
	 * 支付类型数组；
	 */
	...
	if (in_array($_POST['token'], $type)) {//判断接收到的支付类型是否在数组中
		...
		if(!empty($orderid)) {
			...
			file_put_contents('./log/pay_order'.date('Ym',time()).'.txt', print_r($orderid,TRUE));
			...
			if (!empty($result)) {
			    //查询表中对应的订单编号是否有未支付订单
			    //2018-04-13
			    $find = "a.uniacid,a.pay_account,a.total,a.ordersn,a.pay_type,a.body,a.user_id,a.create_time,a.returnUrl,b.upOrderId,b.starttime,b.endtime,b.duration";
				$parms = pdo_fetch("SELECT ". $find ." FROM ".tablename('foll_order')." a LEFT JOIN ".tablename('parking_order')." b ON a.ordersn = b.ordersn WHERE a.ordersn = :ordersn AND (a.pay_status = :pay_status OR a.pay_status = :paystatus) ORDER BY a.id LIMIT 1", array(':ordersn' => $updata['ordersn'], ':pay_status' => 0,':paystatus' => 2));
				if(!$parms) {
				...
				//定义key
				/*$key = $key.$parms['uniacid'];2018-08-01
				//不为空，取缓存数据；判断数据一致则直接退出
				if(!empty($cache = cache_load($key)))2018-08-01
				{
					$config = $cache;2018-08-01
				} else {*/
					//反序列获取数组；
					$config = json_decode($configs['config'],true);
					$config = array(
						'account'=> $config['tg']['mchid'],//商户号
						'key'    =>	$config['tg']['key'],//秘钥
					);
					//写入缓存
					//cache_write($key,$config);  2018-08-01
				//}
				...
				//折扣计算部分；2018-06-23
				/*$m = 0;
				$isFree = false;//,'status'=>2
				$opera = pdo_getall('parking_operate',['uniacid'=>$parms['uniacid'],'status'=>2],['discount','startDate','endDate']);
				//if(!empty($opera) && ($parms['total'] > 0))//1   2018-08-01
				if(!empty($opera) && ($parms['total'] > 0) && ($parms['user_id'] != 'oR-IB0t4Yc9zmV-K-_5NRB-u5k4U'))//1
				{
					...
					if($isFree) {
						...
						$sendArr = array(
							...
							'parkTime' 		=> $parms['duration'].'分钟',//停车时长
						...
						$arrs = ['payUrl'=>'','msg'=>'error','info'=>'缴费成功！'];
				...
				//支付交易金额
				$m = $m > 0 ? $m : $parms['pay_account'];*/
				
				$m = sprintf("%.2f",$parms['pay_account']);
				//组装数据  支付订单写入old订单流水表
				$params = array(
					'payMoney'	=>	$m,//$parms['pay_account'],//支付金额  a.pay_account
				...
				$params['returnUrl'] = $parms['returnUrl'];
			}

		}else {//如果 pay_order 表中不存在该订单编号
			
			//$find = "a.uniacid,a.pay_account,a.total,a.ordersn,a.pay_type,a.body,a.user_id,a.create_time,b.upOrderId,b.starttime,b.endtime,b.duration";
			$find = "a.uniacid,a.pay_account,a.total,a.ordersn,a.pay_type,a.body,a.user_id,a.create_time,a.returnUrl,a.upOrderId,b.starttime,b.endtime,b.duration";
			$parms = pdo_fetch("SELECT ".$find." FROM ".tablename('foll_order')." a LEFT JOIN ".tablename('parking_order')." b ON a.ordersn = b.ordersn WHERE a.ordersn = :ordersn AND (a.pay_status = :pay_status OR a.pay_status = :paystatus) ORDER BY a.id LIMIT 1", array(':ordersn' => $updata['ordersn'], ':pay_status' => 0,':paystatus' => 2));
...
//			if(!empty($opera) && ($parms['total'] > 0) && ($parms['user_id'] != 'oR-IB0t4Yc9zmV-K-_5NRB-u5k4U'))//1
			...
			$receive = array(
				...
				'body'		=>  $parms['body'],//商品描述		a.body
				'payMoney'	=>  $m,//$parms['pay_account'],//交易金额	a.pay_account
				...
				//'returnUrl' => $parms['returnUrl'],//前端同步回调地址		a.returnUrl
				'status' 	=> '0',
			);
			...
			case 'wechat'://微信公众号支付	
				...
				if(!empty($payurl) && ($payurl['status'] == '100') ) {
					$arrs = [
						...
						'pay_info'=> $payurl['pay_info'],// js 参数
						'msg'	=> 'success',
					];
				}else {
					$arrs = [
	...
	if(!empty($opera) && ($parms['total'] > 0))//1
	{
		...
		if($isFree) {
			...
			$sendArr = array(
				...
				'parkTime' 		=> $parms['duration'].'分钟',//停车时长
				'realTime' 		=> $parms['duration'],//实计时长		b
				'payableMoney' 	=> $parms['total'],//应付金额	a
				'deducMoney' 	=>sprintf('%.2f',($parms['total']- $m)),//抵扣金额	a
			...
			$arrs = ['status'=> 100,'msg'=>'缴费成功'];
			echo json_encode($arrs);			
			die();
		}
	}
	//支付交易金额
	return $m = $m > 0 ? (round($m,2)):(round($parms['pay_account'],2));
	//折扣计算结束
}
...
//	$url = 'http://tgjf.833006.biz/tgPosp/payApi/wxJspay'; //测试地址
    file_put_contents('./log/wechatResdata'.date('Ym',time()).'.txt', $data."\r\n",FILE_APPEND);
	...
	$response['ordersn'] = $params['ordersn'];

	file_put_contents('./log/wechatRes'.date('Ym',time()).'_1.txt', print_r($response,TRUE),FILE_APPEND);
	//直接返回支付URL地址
//	return $response->pay_url;
	//返回数组
	return $response;
...
//	$url = 'http://tgjf.833006.biz/tgPosp/services/payApi/unifiedorder';
    ...
    file_put_contents('./log/Tgalipay_scodeData.txt', $data."\r\n",FILE_APPEND);
