	...
	if (in_array($_POST['token'], $type)) {//判断接收到的支付类型是否在数组中
		...
		if(!empty($orderid)) {
			...
			if (!empty($result)) {
				 ...
				 * 试运营设置
				 */
				/*$opera = pdo_fetchall('parking_operate',['uniacid'=>$parms['uniacid'],['discount','startDate','endDate']]);
				if(!empty($opera) && $parms[''] > 0)
				{
					$m = 25;
					$j = '0.';
					$times = time();
					foreach($opera as $key=>$val) {
						if(($times >= $val['startDate']) && ($times <= $val['endDate'])) {
