...
class RealNameJob extends Command
{
    
    protected function configure ()
    {
        $this->setName('RealNameJob')->setDescription('阿里云身份验证');
    }
...
//      @file_put_contents(getcwd().'/runtime/log/test.txt',date('H:i:s',time())."\n",FILE_APPEND);
    ...
    protected function getRedis ()
    {
		...
		try {
			...
			while ($flag) {
				...
				$res = $this->redis->lRange($keys,0,-1);
				//print_r($res);
				if (empty($res)) {
                    $flag = false;
                    $lgo  = 'ok';
				...
				/*if(isset($res[1])) {
					//第二组数据导入  
					$dataArr1 = json_decode($res[1],true);
					$pici1    = isset($dataArr1['title'])?$dataArr1['title']:'no';//批次二
					if($pici != $pici1) {//第一批次不等于第二批次的时候就导出第一次批次的数据
				...
				if(!empty($mysqlData)&&!empty($mysqlData['resultCode']) && ($mysqlData['resultCode']=='1000') && ($mysqlData['resultMsg']=='一致')) {//判断认证代码不为空
					/*
					 * 有已经验证成功的数据
					 */
					//A表有数据    更新A表的订单
					...
					//B   删除B 表成功的数据；
					$this->db->table('foll_realname_error')->where(['orderId'=>$dataArr['orderId']])->delete();
					
					$this->rs->lpop($keys);
					
				} else if(!empty($mysqlData['resultCode']) && ($mysqlData['resultCode']!='1000') && ($mysqlData['resultMsg']!='一致')) {
					/**
					 *	有已经验证失败的数据 
					 */	
					$update1 = [
					...
					//删除A存在的失败数据     表成功数据表
					$this->db->table('foll_realname_general')->where(['orderId'=>$dataArr['orderId']])->delete();
					//出栈   
					$this->rs->lpop($keys);
					
				} else {//否则里面没有数据，请求去接口获取
    ...
    protected function Excelport($pici,$email) {
    	//如果该批次，数据有不等于1000（不成功的） 并且 不等于一致(不成功的)  就导出数据     否则提示全部验证通过
    	$order = $this->db->table('foll_realname_error')->where(['title'=>$pici,'resultCode'=>['neq','1000'],'resultMsg'=>['neq','一致']])->lists();
    	if(empty($order)) {
