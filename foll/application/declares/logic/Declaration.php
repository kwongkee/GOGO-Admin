...
class Declaration extends Model
{
    ...
    public function formatGoodsArray ( $data )
    {
        ...
        foreach ($data as $val) {
            $tmp = [];
            $tmp = ['Seq' => $val['Seq'], 'EntGoodsNo' => $val['EntGoodsNo'], 'ShelfGName' => $val['ShelfGName'], 'NcadCode' => $val['NcadCode'], 'HSCode' => $val['HSCode'], 'GoodsName' => $val['GoodsName'], 'GoodsStyle' => $val['GoodsStyle'], 'Brand' => $val['Brand'], 'GUnit' => $val['GUnit'], 'StdUnit' => $val['StdUnit'], 'SecUnit' => $val['SecUnit'], 'RegPrice' => $val['RegPrice'], 'GiftFlag' => $val['GiftFlag'], 'OriginCountry' => $val['OriginCountry'], 'QualityCertify' => $val['QualityCertify'], 'Manufactory' => $val['Manufactory'], 'NetWt' => $val['NetWt'], 'GrossWt' => $val['GrossWt'], 'EmsNo' => $val['EmsNo'], 'ItemNo' => $val['ItemNo'], 'BarCode' => $val['BarCode'], 'Quality' => $val['Quality'], 'Notes' => $val['Notes'],];
    ...
    protected function requestReport ( $data, $cli, $key )
    {
        ...
        $curl = curl_init();
        curl_setopt_array($curl, [CURLOPT_URL => $url, CURLOPT_RETURNTRANSFER => true, CURLOPT_ENCODING => "", CURLOPT_MAXREDIRS => 10, CURLOPT_TIMEOUT => 30, CURLOPT_CUSTOMREQUEST => "POST", CURLOPT_POSTFIELDS => $data, CURLOPT_SSL_VERIFYPEER => false, CURLOPT_SSL_VERIFYHOST => false, CURLOPT_HTTPHEADER => ["Content-Type:text/xml;charset=UTF-8", "Connection:Keep-Alive"],]);
     ...
     * 判断是否审核
    */
    protected function checkStatus ( $table, $where = [] )
    {
