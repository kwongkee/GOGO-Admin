...
class ReadElcOrderFile extends Model
{
    ...
    {
        $moveResult = $file->validate(['ext' => 'xls,xlsx'])->rule('unique')->move($this->savePath);
        if ( !$moveResult ) throw new \Exception('文件格式错错误');
...
//        @unlink($filePath);
    ...
    public function getTwoSheet ( $postData )
    {
        ...
        for ($i = 2; $i <= $row; $i++) {
            $orID = $postData['OrderDate']." ".mt_rand(0,23).':'.mt_rand(1,55).':'.mt_rand(1,55);
                $tmp = [];
                $tmp = [
                    'EntOrderNo' => 'GC' .Session('admin.order_prix'). date('YmdHis',strtotime($orID)) . mt_rand(111111, 999999),//电子订单编号
                    'OrderDate'  => $orID,//订单生成时间
                    'WaybillNo' =>  $this->ExcelObj->getActiveSheet()->getCell('A' . $i)->getValue(),//物流运单号码
                    'OrderGoodTotalCurr' => $this->ExcelObj->getActiveSheet()->getCell('B' . $i)->getValue(),//币制
                    'Freight'   => $this->ExcelObj->getActiveSheet()->getCell('C' . $i)->getValue(),//运费
                    'Tax'       => $this->ExcelObj->getActiveSheet()->getCell('D' . $i)->getValue(),//税费
                    'OrderGoodTotal' => $this->ExcelObj->getActiveSheet()->getCell('E' . $i)->getValue(),//贷款金额
                    'packageType'  => $this->ExcelObj->getActiveSheet()->getCell('F' . $i)->getValue(),//包装类型
                    'OrderDocAcount' => $this->ExcelObj->getActiveSheet()->getCell('G' . $i)->getValue(),//订购人用户名
                    'OrderDocName'  => $this->ExcelObj->getActiveSheet()->getCell('H' . $i)->getValue(),//订购人姓名
                    'OrderDocId'    => $this->ExcelObj->getActiveSheet()->getCell('I' . $i)->getValue(),//订购人证件号码
                    'OrderDocTel' => $this->ExcelObj->getActiveSheet()->getCell('J' . $i)->getValue(),//收货人电话
                    'Province'  => $this->ExcelObj->getActiveSheet()->getCell('K' . $i)->getValue(),//收货人省
                    'city'      => $this->ExcelObj->getActiveSheet()->getCell('L' . $i)->getValue(),//收货人城市
                    'county'    => $this->ExcelObj->getActiveSheet()->getCell('M' . $i)->getValue(),//收货人区/县
                    'RecipientAddr' => $this->ExcelObj->getActiveSheet()->getCell('N' . $i)->getValue(),//收货人地址
                    'RecipientCountry' =>$this->ExcelObj->getActiveSheet()->getCell('O' . $i)->getValue(),//收货人所在国
                    'RecipientProvincesCode' => $this->ExcelObj->getActiveSheet()->getCell('P' . $i)->getValue(),//收货人行政区diaim
                    'senderName'    => $this->ExcelObj->getActiveSheet()->getCell('Q' . $i)->getValue(),//发货人姓名
                    'senderTel' => $this->ExcelObj->getActiveSheet()->getCell('R' . $i)->getValue(),//发货人电话
                    'senderAddr' => $this->ExcelObj->getActiveSheet()->getCell('S' . $i)->getValue(),//发货人地址
                    'senderCountry' => $this->ExcelObj->getActiveSheet()->getCell('T' . $i)->getValue(),//发货人所在国
                    'senderProvincesCode'   => $this->ExcelObj->getActiveSheet()->getCell('U' . $i)->getValue(),//发货人省市区代码
                    'weight'    => $this->ExcelObj->getActiveSheet()->getCell('V' . $i)->getValue(),//净重
                    'grossWeight' => $this->ExcelObj->getActiveSheet()->getCell('W' . $i)->getValue(),//毛重
                    'GoodsType' => $this->ExcelObj->getActiveSheet()->getCell('X' . $i)->getValue(),//分类名称
                    'ActualAmountPaid'  => $this->ExcelObj->getActiveSheet()->getCell('E' . $i)->getValue(),
                    'RecipientName' =>$this->ExcelObj->getActiveSheet()->getCell('H' . $i)->getValue(),
                    'RecipientTel'  =>$this->ExcelObj->getActiveSheet()->getCell('J' . $i)->getValue(),
        ...
        for ($i=2;$i<=$row;$i++){
            $goodsInfo[$this->ExcelObj->getActiveSheet()->getCell('A'.$i)->getValue()][]=[
               'goodNo' =>$this->ExcelObj->getActiveSheet()->getCell('B'.$i)->getValue(),
                ...
                'price' => $this->ExcelObj->getActiveSheet()->getCell('D'.$i)->getValue()
            ];
        }
        return $goodsInfo;
    }
     ...
     *       'EntOrderNo' => 'GC' .Session('admin.order_prix'). date('YmdHis',time()) . mt_rand(111111, 999999),//电子订单编号
                    'OrderDate'  => $postData['OrderDate'],//订单生成时间
