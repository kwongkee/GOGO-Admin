...
class OrderReconcil extends CommonController
{
	...
	public function test() {
		...
		/*$where = ['pay_type'=>'wx','order_check'=>'no'];
		...
		/*$email = '198@gogo198.net';
        $name  = 'Admin';
        $subject = '';
        $content = '使用阿里云企业邮箱发送';

        $path = $_SERVER['DOCUMENT_ROOT'].'/foll/public/send/BOWX20200322.xlsx';// BOWX20200322.xlsx

        $url = 'https://www.baidu.com/';
        $send = $this->SendEmailali($url,$email,$content,$path);
		...
		// 发送邮件     http://shop.gogo198.cn/foll/public/?s=OrderReconcils/test
        $sendData['email'] = '597831209@qq.com';
        $sendData['sid']   = 2439;
        $sendData['types'] = 'aq';
        $sendData['dates'] = 20200424; //date('Ymd',strtotime('-1 day'));
        $url = 'http://shop.gogo198.cn/foll/public/index.php?s=Reconcils/emailok';
        $res[] = $this->PostData($url,$sendData);

        // 发送邮件  2020-03-11
        echo  '主动发邮件';

		// 修改了
		/*$data = Db::name('parking_mer_summary')->where('sid','>',1300)->field(['sid','date','pay_type'])->select();

        $res = [];
		foreach($data as $dv) {

            // 发送对账邮件   'http://shop.gogo198.cn/foll/public/?s=OrderReconcils/test
            $sendData['email'] = '353453825@qq.com';
            $sendData['sid']   = $dv['sid'];
            $sendData['types'] = $dv['pay_type'];
            $sendData['dates'] = $dv['date'];//date('Ymd',strtotime('-1 day'));
            $url = 'http://shop.gogo198.cn/foll/public/?s=Reconcils/emailok';
            $res[] = $this->PostData($url,$sendData);
    ...
    public function Analyaqs() {
        ...
        if(!empty($summaryOk)) {
            ...
            if($summaryOk['order_check'] == 'no') {
                ...
                $this->SendEmailali($url,'353453825@qq.com',$startTime.'您当天的订单对账已完成，请勿重复执行！');//179078286
                echo '执行时间：'.date('Y-m-d H:i:s',time());
				//发送微信通知
				//$this->SendMamage('aq',$startTime,'对账二次执行提示',$startTime.'您当天的订单对账已完成，请勿重复执行！','wx6d1af256d76896ba','pages/user/carbill/carbill?type=aq&status=Ok');
                die;
            }
        }
        ...
        if($numaq == $numsq) {
            ...
            if($flag == 1 ) {
				...
				//$this->SendMamage('aq',$startTime,'对账提示',$startTime.'您有聚合支付，对账！','wx6d1af256d76896ba','pages/user/carbill/carbill?type=aq');
...
//					print_r($inserArr);   parking_mer_mistake
	...
	public function Analywxs() {
		...
		// 2018-11-06
		$wh = ['date'=>$startTime,'pay_type'=>'wx'];
		$summaryOk = Db::name('parking_mer_summary')->field('sid,date,order_check')->where($wh)->order('sid asc')->find();
		if(!empty($summaryOk)) {

			if($summaryOk['order_check'] == 'no') {
				$sendData['email'] = '353453825@qq.com';//'353453825@qq.com';
				$sendData['sid']   = $summaryOk['sid'];
				$sendData['types'] = 'wx';
				$sendData['dates'] = $summaryOk['date'];
				$url = 'http://shop.gogo198.cn/foll/public/index.php?s=Reconcils/emailok';
				...
				$this->SendEmailali($url,'353453825@qq.com',$startTime.'您当天的订单对账已完成，请勿重复执行！');//179078286
				echo '执行时间：'.date('Y-m-d H:i:s',time());
				//$this->SendMamage('wx',$startTime,'对账二次执行提示',$startTime.'您当天的订单对账已完成，请勿重复执行！','wx6d1af256d76896ba','pages/user/carbill/carbill?type=wx&status=Ok');
				die;
			}
		}
		...
		if($numaq == $numsq) {
			...
			if($flag>0) {
				...
				$this->SendEmailali($url,'353453825@qq.com',$startTime.'您有微信免密，对账！请登录');
				//$this->SendMamage('wx',$startTime,'对账提示',$startTime.'您有微信免密，对账！','wx6d1af256d76896ba','pages/user/carbill/carbill?type=wx');
			} else {
				//发送平账通知
				/*$url = 'http://shop.gogo198.cn/foll/public/index.php?s=Reconcils/OkAnalyaqs&type=wx';
				$this->SendEmailali($url,'353453825@qq.com',$startTime.'您有微信免密，对账！请登录');*/
				//  需要导出昨日订单数据，加date条件 cklein
				$where = ['pay_type'=>'wx','order_check'=>'no','date'=>$startTime];
				$mer = Db::name('parking_mer_summary')->field('sid,date')->where($where)->order('sid asc')->find();
				if(!empty($mer)) {
					$sendData['email'] = '353453825@qq.com';
					$sendData['sid']   = $mer['sid'];
	...
	public function AnalyUnions() {
		...
		if(!empty($summaryOk)) {
			...
			if($summaryOk['order_check'] == 'no') {
				...
				$this->SendEmailali($url,'353453825@qq.com',$startTime.'您当天的订单对账已完成，请勿重复执行！');//179078286
				echo '执行时间：'.date('Y-m-d H:i:s',time());
				//$this->SendMamage('un',$startTime,'对账二次执行提示',$startTime.'您当天的订单对账已完成，请勿重复执行！','wx6d1af256d76896ba','pages/user/carbill/carbill?type=un&status=Ok');
				die;
			}
		}
		...
		if($numaq == $numsq) {
			...
			if($flag) {
				...
				$this->SendEmailali($url,'353453825@qq.com',$startTime.'您有银联无感，对账！请登录');
				//$this->SendMamage('un',$startTime,'对账提示',$startTime.'您有银联无感，对账！','wx6d1af256d76896ba','pages/user/carbill/carbill?type=un');
			} else {
				//发送平账通知
				/*$url = 'http://shop.gogo198.cn/foll/public/index.php?s=Reconcils/OkAnalyaqs&type=un';
				$this->SendEmailali($url,'353453825@qq.com',$startTime.'您有银联无感，对账！请登录');*/
				$where = ['pay_type'=>'un','order_check'=>'no'];
				$mer = Db::name('parking_mer_summary')->field('sid,date')->where($where)->order('sid asc')->find();
				if(!empty($mer)) {
					$sendData['email'] = '353453825@qq.com';
					$sendData['sid']   = $mer['sid'];
	...
	public function AnalySdes() {
		...
		if(!empty($summaryOk)) {
			...
			if($summaryOk['order_check'] == 'no') {
				...
				$this->SendEmailali($url,'353453825@qq.com',$startTime.'您当天的订单对账已完成，请勿重复执行！');//179078286
				echo '执行时间：'.date('Y-m-d H:i:s',time());
				//$this->SendMamage('sd',$startTime,'对账二次执行提示',$startTime.'您当天的订单对账已完成，请勿重复执行！','wx6d1af256d76896ba','pages/user/carbill/carbill?type=sd&status=Ok');
				die;
			}
		}
		...
		if($numaq == $numsq) {
			...
			if($flag) {
				...
				$this->SendEmailali($url,'353453825@qq.com',$startTime."您有农商代扣，对账！请登录");
				//$this->SendMamage('sd',$startTime,'对账提示',$startTime.'您有农商代扣，对账！','wx6d1af256d76896ba','pages/user/carbill/carbill?type=sd');
			} else {
				//发送平账通知
				/*$url = 'http://shop.gogo198.cn/foll/public/index.php?s=Reconcils/OkAnalyaqs&type=sd';
				$this->SendEmailali($url,'353453825@qq.com',$startTime.'您有农商代扣，对账！请登录');*/
				$where = ['pay_type'=>'sd','order_check'=>'no'];
				$mer = Db::name('parking_mer_summary')->field('sid,date')->where($where)->order('sid asc')->find();
				if(!empty($mer)) {
					$sendData['email'] = '353453825@qq.com';
					$sendData['sid']   = $mer['sid'];
	...
	public function PaymentSummary($day='')
	{
		...
		**/
		...
		if(!empty($NewArrs)) {
			...
			if($this->SendEmailali($msg,'198@gogo198.net',$path,$subject)) {
     ...
     *  2020-03-24
     */
    public function SendEmailali($url,$email='353453825@qq.com',$subject = '您有对账信息，请登录查看！',$path='true') {
        $name    = '系统管理员';
        $content = "提示：您有对账信息，请及时登录查看并确认！,您可请登录后台【<a href='".$url."'>点击前往查看</a>】";
        if($path == 'true'){//没有数据发送
