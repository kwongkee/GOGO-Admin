...
if($_W['ispost']) {
	...
	//通莞支付配置	2017-11-17
	$tgpay = array_elements(array('switch', 'key', 'mchid'), $_GPC['tgpay']);	
	$tgpay['switch'] = $tgpay['switch'] == 'true';	
	if($tgpay['switch'] && (empty($tgpay['key']) || empty($tgpay['mchid']))) 	{
		message('请输入完整的通莞支付接口信息.');	
	}
	//end 2017-11-17
