...
<script>
    layui.use(['layer', 'form', 'table', 'upload', 'laydate'], function () {
        ...
        table.render({
            elem: '#mainTable'
            ,url: "./index.php?i=3&c=entry&do=checkprice&p=quote&m=sz_yi&op=quote_list&pa=1"
            ,cellMinWidth: 200
            ,cols: [[
                ,{field:'ordersn', title: '询价订单编号',width:205}
                ,{field:'buss_name', title: '所属业务'}
                ,{field:'statusname', title: '状态'}
                ,{field:'createtime', title: '报价时间'}
                ,{align:'center',  title: '操作',fixed:'right',width:120, templet: function(d){
                        return [
                            '<a onclick="openWindow('+"'"+ d.id +"'" +','+"'"+ d.buss_name +"'" +','+"1"+ ');" class="layui-btn layui-btn-primary layui-btn-xs">查看详情</a>',
                        ].join('');
                    } }
            ]]
            ,page: true
    ...
    function openWindow(url,buss_name,typ){
        if(typ==1){
            window.location.href="./index.php?i=3&c=entry&do=checkprice&p=quote&m=sz_yi&op=quote_info&id="+url;
        }else if(typ==2){
