...
<script>
    $(function() {
        ...
        layui.use(['layer', 'form', 'table', 'upload', 'laydate'], function () {
            ...
            table.render({
                elem: '#mainTable'
                ,url: "./index.php?i=3&c=entry&do=approval&p=index&m=sz_yi&op=approval_set&pa=1"
                ,cellMinWidth: 200
                ,cols: [[
                    {field:'id', width: 80, title: '编号'}
                    ,{field:'name', title: '事项名称'}
                    ,{field:'role_id', title: '可发起角色'}
                    ,{align:'center',  title: '操作',fixed:'right',width:70, templet: function(d){
                            <?php  if($authority==2) { ?>
                                return ['暂无操作'].join();
                            <?php  } else { ?>
                                return [
                                    '<a onclick="openWindow('+"'"+ d.id +"'" +','+"1"+ ');" class="layui-btn layui-btn-normal layui-btn-xs">配置</a>',
    ...
    function openWindow(id,typ)
    {
        ...
        if(typ==1){
            //配置事务
            window.location.href="./index.php?i=3&c=entry&do=approval&p=index&m=sz_yi&op=save_setting&id="+id;
        }
    }
</script>