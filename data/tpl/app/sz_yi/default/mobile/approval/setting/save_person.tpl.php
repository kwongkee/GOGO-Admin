    ...
    <div class="layui-card">
        ...
        <div class="layui-card-body" style="padding: 0px;">
            <form class="layui-form" action="" lay-filter="component-form-demo1">
                ...
                <input type="number" name="type" value="<?php  echo $data['typ'];?>" style="display: none;">

                <?php  if($data['typ']==1) { ?>
                    <div class="layui-form-item">
                        <label class="layui-form-label">人员名称</label>
                        <div class="layui-input-block layui-select-fscon">
                            <input type="text" name="user_name" lay-verify="required" placeholder="请输入人员名称" autocomplete="off" class="layui-input" value="<?php  echo $info['name'];?>">
                        </div>
                    </div>
                    <div class="layui-form-item">
                        <label class="layui-form-label">联系电话</label>
                        <div class="layui-input-block layui-select-fscon">
                            <input type="text" name="user_tel" lay-verify="required" placeholder="请输入联系电话" autocomplete="off" class="layui-input" value="<?php  echo $info['tel'];?>">
                        </div>
                    </div>
                    <div class="layui-form-item">
                        <label class="layui-form-label">联系邮箱</label>
                        <div class="layui-input-block layui-select-fscon">
                            <input type="text" name="user_email" lay-verify="required" placeholder="请输入联系邮箱" autocomplete="off" class="layui-input" value="<?php  echo $info['email'];?>">
                        </div>
                    </div>
                <?php  } ?>
...
<script>
    layui.use(['layer', 'form', 'table', 'upload', 'laydate','jquery'], function () {
        ...
        var authList = xmSelect.render({
            ...
            layVerify: 'required',
            layVerType: 'msg',
            initValue: [<?php  echo $info['event_ids'];?>]
        });

        /* 监听提交 */
        form.on('submit(component-form-demo1)', function(data){
            $.ajax({
                url:"./index.php?i=3&c=entry&do=approval&p=index&m=sz_yi&op=save_person",
                method:'post',
                data:data.field,
                dataType:'JSON',
