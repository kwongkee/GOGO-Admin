...
<!--    </div>-->
<!--</div>-->
<?php  if($ishave_staff['id']>0) { ?>
    <p style="font-size:16px;margin-top:15px;text-align: center;">您已注册，无需重复操作！</p>
<?php  } else { ?>
    ...
    <!--            <?php  echo $title;?>-->
            ...
            <div class="layui-card-body" style="padding: 0px;">
                <form class="layui-form" action="" lay-filter="component-form-demo1">
                    <input type="number" name="pid" value="<?php  echo $data['pid'];?>" style="display: none;">
                    <input type="number" name="pa" value="1" style="display: none;">
                    <input type="number" name="decl_id" value="<?php  echo $data['decl_id'];?>" style="display: none;">

                    <p style="text-align: center;font-size:18px;"><?php  echo $info['inviter_name'];?>邀请您加入</p>
                    <p style="text-align: center;font-size:15px;margin:10px 0;"><?php  echo $info['company_name'];?></p>

                    <div class="layui-form-item">
                        <label class="layui-form-label">人员名称</label>
                        <div class="layui-input-block layui-select-fscon">
                            <input type="text" name="name" lay-verify="required" placeholder="请输入您的名称" autocomplete="off" class="layui-input" value="">
                        </div>
                    </div>
                    <div class="layui-form-item">
                        <label class="layui-form-label">联系电话</label>
                        <div class="layui-input-block layui-select-fscon">
                            <input type="text" name="tel" lay-verify="required" placeholder="请输入联系电话" autocomplete="off" class="layui-input" value="">
                        </div>
                    </div>
                    <div class="layui-form-item">
                        <label class="layui-form-label">联系邮箱</label>
                        <div class="layui-input-block layui-select-fscon">
                            <input type="text" name="email" lay-verify="required" placeholder="请输入联系邮箱" autocomplete="off" class="layui-input" value="">
                        </div>
                    </div>
...
<script>
    layui.use(['layer', 'form', 'table', 'upload', 'laydate','jquery'], function () {
        ...
        form.on('submit(component-form-demo1)', function(data){
            $.ajax({
                url:"./index.php?i=3&c=entry&do=approval&p=index&m=sz_yi&op=add_person",
                method:'post',
                data:data.field,
                dataType:'JSON',
