            ...
            <div class="layui-col-md12">
                <div class="layui-card">
                    ...
                    <div class="layui-card-header">
                        <div class="layui-btn layui-btn-normal layui-btn-sm" onclick="openWindow(0,2)">新增表单</div>
                    </div>
                    <?php  } ?>
...
<script>
    $(function() {
        ...
        layui.use(['layer', 'form', 'table', 'upload', 'laydate'], function () {
            ...
            table.render({
                elem: '#mainTable'
                ,url: "./index.php?i=3&c=entry&do=approval&p=index&m=sz_yi&op=diyForm_set&pa=1"
                ,cellMinWidth: 200
                ,cols: [[
                    ...
                    ,{align:'center',  title: '操作',fixed:'right',width:120, templet: function(d){
                            ...
                            <?php  } else { ?>
                                if(d.decl_id==0){
                                    return [
                                        '<a onclick="openWindow('+"'"+ d.id +"'" +','+"1"+ ','+"1"+');" class="layui-btn layui-btn-normal layui-btn-xs">查看</a>',
                                    ]
                                }else{
                                    return [
    ...
    function openWindow(id,typ,issee=0)
    {
        var $ = layui.$
            , layer = layui.layer;
        ...
        if(typ==1 || typ==2){
            //保存表单
            window.location.href="./index.php?i=3&c=entry&do=approval&p=index&m=sz_yi&op=save_diyForm&id="+id+"&issee="+issee;
        }else if(typ==3){
            //删除表单
            layer.confirm('确认删除吗？',function(){
                $.ajax({
                    url:"./index.php?i=3&c=entry&do=approval&p=index&m=sz_yi&op=del_diyForm",
                    method:'post',
                    data:{'id':id},
                    dataType:'JSON',
