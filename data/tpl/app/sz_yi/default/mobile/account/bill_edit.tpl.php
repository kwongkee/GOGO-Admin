    ...
    <div class="info_main">
        ...
        <div class="bill_info">
            <div class="line"><div class="title" style="background:#fff;z-index:99;">账户类型</div><div class="info"><div class="inner" style="z-index:98;">
                <span class="bill_account_type" data-val="1"><i class="fa <?php echo $data['bill_account_type']==1?'fa-check-circle-o':'fa-circle-o'?>"></i> 企业账户</span>&nbsp;&nbsp;
                <span class="bill_account_type" data-val="2"><i class="fa  <?php echo $data['bill_account_type']==2?'fa-check-circle-o':'fa-circle-o'?>"></i> 个人账户</span>&nbsp;&nbsp;
                <span class="bill_account_type" data-val="3"><i class="fa  <?php echo $data['bill_account_type']==3?'fa-check-circle-o':'fa-circle-o'?>"></i> 支付账户</span>
                <input type="text" name="bill_account_type" id="bill_account_type" value="<?php  echo $data['bill_account_type'];?>" style="display: none;">
            </div></div></div>
            <div class="line bill_account_id">
                ...
                <select name="bill_account_id" id="bill_account_id">
                    <?php  if(is_array($data['account_info'])) { foreach($data['account_info'] as $v) { ?>
                        <option value="<?php  echo $v['id'];?>" <?php echo $v['id']==$data['bill_account_id']?'selected':''?>><?php  echo $v['bank_account'];?>-<?php  echo $v['bank_name'];?>-<?php  echo $v['name'];?></option>
                    <?php  } } ?>
                </select>
            </div></div>
            </div>
            <div class="line"><div class="title">对账日期</div><div class='info'><div class='inner'>
                <div class="disf" style="align-items: center;">
                    <input type="text" id="bill_startDate" placeholder="启始日期" readonly="" value="<?php  echo $data['bill_startDate'];?>" style="text-align: center;height:38px;">—<input type="text" id="bill_endDate" placeholder="截止日期" readonly="" value="<?php  echo $data['bill_endDate'];?>" style="text-align: center;height:38px;">
                </div>
            </div></div></div>
            <div class="line"><div class="title">账单余额</div><div class='info'><div class='inner'>
                <div class="disf" style="align-items: center;">
                    <select name="currency3" id="currency3" style="width:32%;margin-right:5px;">
                        ...
                        <option value="<?php  echo $v['code_value'];?>" <?php echo $v['code_value']==$data['currency3']?'selected':''?>><?php  echo $v['code_name'];?></option>
                        <?php  } } ?>
                    </select>
                    <input type="number" id='bill_price' placeholder="账单余额"  style="width:32%;" value="<?php  echo $data['bill_price'];?>" onkeyup="value=value.replace(/^\D*(\d*(?:\.\d{0,2})?).*$/g, '$1')"/>
                ...
                <div class="pic img_info" data-ogid='0' data-max='99' style="height:38px;overflow-x: scroll;display: flex;align-items: center;">
                    <div class="images">
                        <?php  if(!empty($data['bill_file'])) { ?>
                            <?php  if(is_array($data['bill_file'])) { foreach($data['bill_file'] as $v2) { ?>
    ...
    $(function(){
        $('.info_sub').click(function(){
            ...
            $.ajax({
                ...
                type:'POST',
                dataType:'json',
                data:{'op':'bill_edit','id':<?php  echo $data['id'];?>,'status':<?php  echo $data['status'];?>,'type':<?php  echo $data['type'];?>,'bill_file':bill_file,'bill_account_type':bill_account_type,'bill_account_id':bill_account_id,'bill_startDate':bill_startDate,'bill_endDate':bill_endDate,'currency3':currency3,'bill_price':bill_price,'submit_method':submit_method},
                success:function(json) {
                    if(json.status==-1){
