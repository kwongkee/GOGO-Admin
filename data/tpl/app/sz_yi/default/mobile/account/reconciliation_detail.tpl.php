...
<div id="container">
        ...
        <div class="layui-row layui-col-space15">
            <div class="layui-col-md12">
                <div class="layui-card">
                    <div class="layui-card-header" style="padding:0 6px;height:auto;line-height:28px;">
                        <div id="toolbar" class="btn-group" style="display:flex;align-items:center;justify-content: space-evenly;">
                            <input type="text" name="ie_date" class="layui-input" style="width:65%;" id="ie_date" placeholder="请选择月份搜索" value="<?php  echo $_GPC['date'];?>">
                            <div class="induce" onClick="induce(<?php  echo $my_merchant['id'];?>)">对账归纳</div>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="hui-segment" id="cate" style="margin-top:14px;">
        <a href="javascript:changeDom(1);" data-i="1" class="hui-segment-active">待对账</a>
        <a href="javascript:changeDom(2);" data-i="2">已对账</a>
    </div>
    <!--对账归纳-->
    <div id="domsIn" style="padding-top:10px; text-align:center;">
        <div class="typ1" style="display:flex;align-items: center;">
...
<div class="sign_box">
      ...
      <div class="info_main">
        <div class="line"><div class="title">应收款</div><div class='info'>
            <div class='inner'>
                <input type="text" name="col_1" id="col_1" value="" placeholder="已开票，应收款"/>
        </div></div></div>
        <div class="line"><div class="title">未收款</div><div class='info'>
            <div class='inner'>
                <input type="text" name="col_2" id="col_2" value="" placeholder="已开票，未收款"/>
        </div></div></div>
        <div class="line"><div class="title">预收款</div><div class='info'>
            <div class='inner'>
                <input type="text" name="col_3" id="col_3" value="" placeholder="未开票，预收款"/>
        </div></div></div>
        <div class="line"><div class="title">应付款</div><div class='info'>
            <div class='inner'>
                <input type="text" name="pay_1" id="pay_1" value="" placeholder="已收票，应付款"/>
        </div></div></div>
        <div class="line"><div class="title">未付款</div><div class='info'>
            <div class='inner'>
                <input type="text" name="pay_2" id="pay_2" value="" placeholder="已收票，未付款"/>
        </div></div></div>
        <div class="line"><div class="title">预付款</div><div class='info'>
            <div class='inner'>
                <input type="text" name="pay_3" id="pay_3" value="" placeholder="未收票，预付款"/>
        </div></div></div>
      </div>
      <div class="confirm">
...
<script language="javascript">
        ...
        layui.use(['layer', 'form', 'table', 'upload', 'laydate'], function () {
            ...
            laydate.render({
                ...
                ,done: function(value, date, endDate){
                    ...
                    window.location.replace('./index.php?i=3&c=entry&do=account&m=sz_yi&p=bookkeeping&op=reconciliation_detail&id=<?php  echo $id;?>&date='+value);
                }
            });
        });
    });
    ...
    function opera_queren(t,id){
        ...
        if(selected==1){
            ...
            //查看批次
            watch_voucher("./index.php?i=3&c=entry&do=account&m=sz_yi&p=bookkeeping&op=watch_batch&uid=<?php  echo $id;?>&id="+id);
            // watch_voucher("./index.php?i=3&c=entry&do=account&m=sz_yi&p=bookkeeping&op=watch_voucher&id="+id);
        }else if(selected==4){
            //查看账单
            watch_voucher("./index.php?i=3&c=entry&do=account&m=sz_yi&p=bookkeeping&op=watch_bill&uid=<?php  echo $id;?>&id="+id);
        }
    }
    ...
    function changeDom(index,date){
        ...
        $.ajax({
            ...
            type:'POST',
            dataType:'json',
            data:{'op':'reconciliation_detail','typ':index+1,'id':<?php  echo $id;?>,'date':date},
            success:function(json) {
                if(json.status==1){
                    var dat = json.result.data.batch_info;
                    var html = '';
                    ...
                    if(dat==0){
                        ...
                        if(index+1==2 && dat.reconciliation_status2==1){
                            html += '<tr>\n' +
                                '<td  style="width:100%; overflow:hidden; white-space:nowrap; text-overflow:ellipsis;">'+json.result.data.user_name+'</td>\n'+
                                '<td>'+dat.reconciliation_date+'</td>\n'+
                                '<td>\n'+
                                '            <select name="opera_queren" id="opera_queren" onchange="opera_queren(this,\''+dat['id']+'\')" style="margin-right:10px;">\n' +
                                '                        <option>选择操作</option>\n' +
                                '                        <option value="3">查看批次</option>\n' +
                                '                        <option value="4">查看账单</option>\n' +
                                '                    </select>\n'+
                                '</td>\n'+
                                '</tr>\n';
                        }else if(index+1==1 && dat.reconciliation_status2!=1){
                            html += '<tr>\n' +
                                '<td  style="width:100%; overflow:hidden; white-space:nowrap; text-overflow:ellipsis;">'+json.result.data.user_name+'</td>\n'+
                                '<td>'+dat.reconciliation_date+'</td>\n'+
                                '<td>\n'+
    ...
    function induce(merch_id){
        ...
        let ie_date = $('#ie_date').val();
        if(ie_date!='' || typeof(ie_date)!='undefined'){
            date = ie_date;
        }
        window.location.href="./index.php?i=3&c=entry&do=account&m=sz_yi&p=bookkeeping&op=reconciliation_induce&id="+merch_id+"&date="+date;
    }
</script>
