...
<style type="text/css">
    ...
    .table_title{width: 100%;background: #3c763d;color: #fff;font-size: 15px;padding: 5px;text-align: center;margin-top: 5px;}
    .buss_add{padding:6px 14px;background:#1E9FFF;color:#fff;width:fit-content;font-size:15px;margin: 0 auto;}
    .buss_del{padding:6px 14px;background:#ff5555;color:#fff;width:fit-content;font-size:15px;margin: 0 auto;}
...
<div id="container">
    <div class="page_topbar">
        <div class="title">记账配置</div>
    </div>
    <div class="info_main">
        <div class="line"><div class="title" style="background:#fff;z-index:99;">选择商户</div><div class="info"><div class="inner" style="z-index:98;">
            <select name="user_id" id="user_id">
                <option value="">请选择商户</option>
                <?php  if(is_array($decl_user)) { foreach($decl_user as $v) { ?>
                <option value="<?php  echo $v['id'];?>"><?php  echo $v['user_name'];?></option>
                <?php  } } ?>
            </select>
        </div></div></div>
        <div class="line bill_account_id">
            <div class="title">记账代理</div><div class='info'><div class='inner'>
            <select name="accounting_id" id="accounting_id">
                <option value="">请选择记账代理</option>
                <?php  if(is_array($accounting)) { foreach($accounting as $v) { ?>
                <option value="<?php  echo $v['fanid'];?>"><?php  echo $v['nickname'];?></option>
                <?php  } } ?>
            </select>
        </div></div>
        </div>
        <div class="table_title">核定税种</div>
        <table class="table table-striped" style="margin-bottom:0;table-layout:fixed;word-break: break-all;text-align:center;">
            ...
            <tr>
                <td style="text-align: center;">
                    <select name="taxes_id" id="taxes_id" style="width:100%;">
                        <?php  if(is_array($tax_category)) { foreach($tax_category as $v) { ?>
                        <option value="<?php  echo $v['id'];?>"><?php  echo $v['name'];?></option>
                        <?php  } } ?>
                    </select>
                </td>
                <td style="text-align: center;">
                    <div class="buss_del" onclick="buss_del(this)">删除</div>
                </td>
            </tr>
            <tr>
                <td></td>
                <td><div class="buss_add"  onclick="buss_add(this)">增加</div></td>
            </tr>
        </table>

    </div>
    <div class="info_sub" style="margin-bottom:10px;">确定</div>
...
<script language="javascript">
    ...
    function buss_del(t){
        let len = $(t).parent().parent().parent().children().length;
        if(len==3){
            alert('最少保留一个！');return;
        }
    ...
    function buss_add(t){
        let html = $(t).parent().parent().prev().html();
        $(t).parent().parent().prev().after('<tr>'+html+'</tr>');
    }
    ...
    $(function(){
        $('.info_sub').click(function(){
            ...
            let accounting_id = $('#accounting_id').val();
            //税种核定
            let taxes_id = '';
            ...
            if($('#accounting_id').isEmpty()){
                alert('请选择记账代理');return;
            }
            if(taxes_id==',' || taxes_id=='' || typeof(taxes_id)=='undefined'){
                alert('请添加核定税种');return;
            }
            $.ajax({
                url:"<?php  echo $this->createMobileUrl('account/manage');?>",
                type:'POST',
                dataType:'json',
                data:{'op':'accounting_config','user_id':user_id,'accounting_id':accounting_id,'taxes_id':taxes_id},
