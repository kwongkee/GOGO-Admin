...
<div id="container">
    <div class="hui-segment" id="cate" style="margin-top: 15px;">
        <a id="dom1" href="javascript:changeDom(1);" data-i="1" class="hui-segment-active">待确认列表</a>
        <a id="dom2" href="javascript:changeDom(2);" data-i="2">已确认列表</a>
    </div>
...
<script language="javascript">
    ...
    function changeDom(index,date){
        ...
        $.ajax({
            ...
            type: 'POST',
            dataType: 'json',
            data: {'op': 'taxes', 'typ': index + 1,'date':date},
            success: function (json) {
                if (json.status == 1) {
                    var dat = json.result.data;
                    var html = '';
                    html += '<table class="table table-striped" style="margin-bottom:0;table-layout:fixed;word-break: break-all;">'+
                                '<tr>'+
                                    '<td>商户名称</td>'+
                                    '<td style="width:110px;">税费月份</td>'+
                                    '<td style="width:70px;">操作</td>'+
                                '</tr>';
                    var isshow=0;
                    for(var i2=0;i2<dat['tax_info'].length;i2++){
                        if(dat['tax_info'] != ''){
                            isshow=1;
                            html +='<tr>'+
                                    '  <td>'+dat['tax_info'][i2]['user_name']+'</td>'+
                                    '  <td>'+dat['tax_info'][i2]['decl_date']+'</td>'+
                                    '  <td><div class="see" onclick="opera_queren(this,'+dat['tax_info'][i2]['id']+',3)">查看</div></td>'+
                                    '</tr>';
                        }
                    }
                    if(isshow==0){
    ...
    function opera_queren(t,tax_id,selected){
    ...
    function watch_info(id){
        window.location.href='./index.php?i=3&c=entry&do=account&m=sz_yi&p=manage&op=watch_taxInfo&id='+id;
    }
