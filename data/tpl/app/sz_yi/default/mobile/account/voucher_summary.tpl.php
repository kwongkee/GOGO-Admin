...
<style type="text/css">
    ...
    .submit{display:block;font-size:14px;padding:7px 12px;background:#1E9FFF;border-radius:5px;width:fit-content;margin-left:20px;color:#fff;}
    ...
    <div class="hui-wrap">
        <div style="padding-top:10px;">
            <!--<div class="hui-segment" id="cate">-->
                <!--<a href="javascript:changeDom(1);" class="hui-segment-active">已登记未汇总</a>-->
            <!--</div>-->
            <div id="domsIn" style="padding-top:0px; text-align:center;">
                <div class="typ1" style="display:flex;align-items: center;justify-content:space-evenly;margin-top:0px;">
                ...
                <!--<div class="typ1" style="display:flex;align-items: center;justify-content:flex-end;margin-top:10px;">-->
                <!--    <div class="submit" onClick="submit()" style="margin-right:20px;margin-left:0;">提交（0）</div>-->
                <!--</div>-->
            </div>
        </div>
        ...
        layui.use(['layer', 'form', 'table', 'upload', 'laydate'], function () {
            ...
            laydate.render({
                ...
                ,done: function(value, date, endDate){
                    changeDom(1,value);
                    window.location.replace('./index.php?i=3&c=entry&do=account&m=sz_yi&p=register&op=voucher_summary&date='+value);
                }
            });
        });
                    ...
                    if (index == 0) {
                        ...
                        for (var i2 = 0; i2 < dat.length; i2++) {
                            ...
                            html += '                  <tr class="voucherID' + dat[i2]['id'] + '">\n' +
                                ...
                                // '                                <div class="see" onclick=\'see('+dat[i2]['id']+','+dat[i2]['type']+')\'>编辑</div>\n' +
    ...
    function submitMethod(typ){
        if(typ==1){
            ...
            if(selectMethod==1){
                ...
                //单独该凭证的汇总提交
                $('.tab1').find('.voucherID'+voucher_id).siblings().find('.summary').removeClass('noAdd').css('border-color','#009933');
                $('.tab1').find('.voucherID'+voucher_id).find('.summary').addClass('noAdd').css('border-color','rgb(255, 85, 85)');
                window.times=1;
    ...
    function submit(){
        ...
        if(confirm('确认提交？')){
            $.ajax({
                ...
                type: 'POST',
                dataType: 'json',
                data: {'op': 'voucher_manage', 'type': 5,'ids':window.ids,'submit_method':window.submit_method,'date':$('#ie_date').val()},
                success: function (json) {
                    if (json.status == 1) {
                        alert(json.result.msg);
                        changeDom(1,"<?php  echo $_GPC['date'];?>");
