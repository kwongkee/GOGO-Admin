...
<script language="javascript">
        ...
        layui.use(['layer', 'form', 'table', 'upload', 'laydate'], function () {
            ...
            laydate.render({
                ...
                ,done: function(value, date, endDate){
                    ...
                    window.location.replace('./index.php?i=3&c=entry&do=account&m=sz_yi&p=manage&op=reconciliation&date='+value);
                }
            });
        });
    })
    ...
    function opera_queren(t,uid,id){
        let selected = $(t).val();
        if(selected==1){
            //对账无误
            operation(id,1);
            ...
            //查看凭证
            watch_voucher("./index.php?i=3&c=entry&do=account&m=sz_yi&p=bookkeeping&op=watch_batch&uid="+uid+"&id="+id);
            // watch_voucher("./index.php?i=3&c=entry&do=account&m=sz_yi&p=bookkeeping&op=watch_voucher&id="+id);
        }else if(selected==4){
            //查看账单
            // watch_voucher("./index.php?i=3&c=entry&do=account&m=sz_yi&p=bookkeeping&op=watch_bill&opera=1&id="+id);
            watch_voucher("./index.php?i=3&c=entry&do=account&m=sz_yi&p=bookkeeping&op=watch_bill&uid="+uid+"&id="+id);
        }
    }
    ...
    function changeDom(index,date){
        ...
        $.ajax({
            ...
            type:'POST',
            dataType:'json',
            data:{'op':'reconciliation','typ':index+1,'date':date},
                    ...
                    var html = '';
                        html += '<div class="psd" style="margin-top:10px;">\n' +
                            ...
                            for(var i=0;i<dat.length;i++){
                                if(index+1==2 && dat[i].reconciliation_status2==1){
                                    html += '<tr>\n' +
                                        '<td  style="width:100%; overflow:hidden; white-space:nowrap; text-overflow:ellipsis;">'+dat[i]['user_name']+'</td>\n'+
                                        '<td>'+dat[i].reconciliation_date+'</td>\n'+
                                        '<td>\n'+

                                        '            <select name="opera_queren" id="opera_queren" onchange="opera_queren(this,\''+dat[i]['uid']+'\',\''+dat[i]['id']+'\')">\n' +
                                        '                        <option>选择操作</option>\n' +
                                        ...
                                        '</td>\n';
                                    '</tr>\n';
                                }else if(index+1==1 && dat[i].reconciliation_status2!=1){
                                    html += '<tr>\n' +
                                        '<td  style="width:100%; overflow:hidden; white-space:nowrap; text-overflow:ellipsis;">'+dat[i]['user_name']+'</td>\n'+
                                        '<td>'+dat[i].reconciliation_date+'</td>\n'+
                                        '<td>\n'+
    ...
    function operation(id,typ,manager='2'){
        if(typ==1){
            //正确无误
            if(confirm('确定对账无误？')){
                $.ajax({
