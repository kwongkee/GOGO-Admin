...
<script>
    layui.use(['layer', 'form', 'table', 'upload', 'laydate'], function () {
        ...
        table.render({
            elem: '#mainTable'
            ,url: "./index.php?i=3&c=entry&do=behalf&p=merchant&m=sz_yi&op=order_manage&pa=1"
            ,cellMinWidth: 200
            ,cols: [[
                {field:'id', width: 80, title: '编号'}
                ,{field:'ordersn', title: '订单编号'}
                ,{field:'realname', title: '买家名称'}
                ,{field:'totalprice', title: '订单金额'}
                ,{field:'status_name', title: '订单状态'}
                ,{field:'createtime', title: '下单日期'}
                ,{align:'center',  title: '操作',fixed:'right',width:80, templet: function(d){
                        if(d.status==0){
                            return [
                                '<a onclick="openWindow('+"'"+ d.id +"'" +','+"1"+ ');" class="layui-btn layui-btn-normal layui-btn-xs">订单确认</a>',
                                '<a onclick="openWindow('+"'"+ d.id +"'" +','+"4"+ ');" class="layui-btn layui-btn-primary layui-btn-xs">订单详情</a>',
                            ].join('');
                        }else{
                            return [
                                '<a onclick="openWindow('+"'"+ d.id +"'" +','+"2"+ ');" class="layui-btn layui-btn-normal layui-btn-xs">小票制作</a>',
                                '<a onclick="openWindow('+"'"+ d.id +"'" +','+"3"+ ');" class="layui-btn layui-btn-normal layui-btn-xs">运单制作</a>',
                                '<a onclick="openWindow('+"'"+ d.id +"'" +','+"4"+ ');" class="layui-btn layui-btn-primary layui-btn-xs">订单详情</a>',
                            ].join('');
                        }
                    } }
    ...
    function openWindow(id,typ)
    {
        if(typ==1){
            //订单确认
            layer.confirm('订单是否确认？', {
                btn: ['确认','取消'] //按钮
            }, function() {
                $.ajax({
                    url:"./index.php?i=3&c=entry&do=behalf&p=merchant&m=sz_yi&op=sure_order",
                    method:'post',
                    data:{'id':id},
                    dataType:'JSON',
            ...
            //运单制作
            window.location.href="./index.php?i=3&c=entry&do=behalf&m=sz_yi&p=merchant&op=make_express&id="+id;
        }else if(typ==4){
            //订单详情
            window.location.href="./index.php?i=3&c=entry&do=behalf&m=sz_yi&p=merchant&op=order_detail&id="+id;
        }
    }
</script>