...
<script id='tpl_index' type='text/html'>
    ...
    <div class="top">
        ...
        <div class="bgimg">
            <?php  if($_GPC['i'] == 3) { ?>
            <img src="../addons/sz_yi/template/mobile/default/static/images/bg.jpg">
            <?php  } else { ?>
            <img src="../addons/sz_yi/template/mobile/default/static/images/h-bg1.png">
            <?php  } ?>
        </div>

        <!-- <div class="bgimg"><img src="<?php  if($yzShopSet['img']) { ?><?php  echo '/attachment/'.$yzShopSet['img']?><?php  } else { ?>../addons/sz_yi/template/mobile/default/static/images/bg.jpg<?php  } ?>"></div>-->

        <!--        <div class="list" onclick="location.href='<?php  echo $this->createMobileUrl('shop/category')?>'"><i class="fa fa-list"></i> 分类</div>-->
        <div class="topbar">

            <div class="logo"><img src="/attachment/images/3/2017/04/YFpv9zTCFrP8fl51UyYPTPT5tccc9P.jpg"></div>


            <div class="shop_name">购购网</div>
        </div>
    </div>
    <div class='search'><i class="fa fa-search"></i> 在商城内搜索</div>
    ...
    <div class="search1">
        <div class="topbar1">
            ...
            <div class='left_wrap'>
                <div class='left'>
                    <input type="text" id='keywords' class="input1" placeholder='搜索: 输入货品关键词'/>
                </div>
            </div>
        </div>
...
<script id='tpl_goods_list' type='text/html'>
    ...
    <div class="good" data-goodsid='<%g.id%>'>
        <div class='img'><img src="<%g.main_img%>" onerror="javascript:this.src='/attachment/images/3/default.png'"></div>
...
<script id='tpl_search_list' type='text/html'>
    <ul>
        <%each list as value%>
        <li><i class="fa fa-angle-right"></i> <a href="<?php  echo $this->createMobileUrl('behalf/buyer')?>&op=detail&id=<%value.id%>"><%value.name%></a></li>
        <%/each%>
    </ul>
</script>
    ...
    require(['core', 'tpl'], function (core, tpl) {
        ...
        core.json('shop/index', {}, function (json) {
            ...
            function getGoods(type) {
                ...
                core.json('behalf/buyer', {'op': 'get_goods', page: page}, function (gjson) {
                    ...
                    $('.good').unbind('click').click(function(){
                        location.href = core.getUrl('behalf/buyer',{op:'detail',id:$(this).data('goodsid') });
                    })

                    if (result.goods.length < result.pagesize && scrolling) {

                        $('#goods_container').append('<div id="index_loading">已经加载全部货品</div>');
                        loaded = true;
                        $(window).scroll = null;
                        return;
                    }
                    ...
                    $(window).scroll(function () {
                        ...
                        if ($(document).height() <= totalheight) {
                            if (stop == true) {
                                ...
                                $('#goods_container').append('<div id="index_loading"><i class="fa fa-spinner fa-spin"></i> 正在加载更多货品</div>');
                                page++;
                                getGoods('display');
                            }
                        }
            ...
            $('.search').click(function(){
                ...
                $('#keywords').unbind('keyup').keyup(function(){
                    ...
                    core.json('behalf/buyer',{op:'search_goods',keywords:keywords }, function (json) {
                        var result = json.result;
                ...
                //     var url = core.getUrl('behalf/buyer',{op:'search_goods',keywords:keywords});
                //     location.href=  url;
                // });
