...
<style type="text/css">
    ...
    .good_bottom {height:50px; width:100%;position:fixed; bottom:0px; left:0px;display: block;}
...
<script id='goods_info'  type='text/html'>
    ...
    <div class="good_choose">
        <div class="info">
            <div class="left">
                <img id="chooser_img" src='<%goods.main_img%>' onerror="javascript:this.src='/attachment/images/3/default.png'"/>
            </div>
            <div class="right">
                <div class="price">￥<span id='option_price'><%goods.price%></span></div>
                <div class="option" style="display: none;">请选择规格</div>
            </div>
        ...
        <div class="sub confirm_option" onClick="sure_buy()">确认下单</div>
    </div>
    ...
    <div class="good_img">
        ...
        <div class="main_image">
            <ul>
                <%each pics as p %>
                <li> <img src='<%p%>' onerror="javascript:this.src='/attachment/images/3/default.png'"/></li>
                <%/each%>
            </ul>
        </div>
    </div>
    <div class="good_info1">
        ...
        <div class="info1">
            <div class="name"><%if goods.issendfree==1%><span class='label label-free'>包邮</span>&nbsp;<%/if%><%goods.name%></div>
        ...
        <%else%>
        <div class="info2"><%if goods.issendfree==1%><span class='label label-free' >包邮</span><%/if%>&nbsp;<%goods.name%></div>
        <%/if%>
        <div class="price">￥<d id='marketprice'>
            ...
            <span style='float:right;display:none;'><d id="stockcontainer">库存:<d id="stock"><%goods.total%></d></d> 销量:<%goods.sales%><%goods.unit%></d> 浏览:<%goods.viewcount%></span>
        </div>
    ...
    <div class="good_info2">
        <div class="menu">
            <div id="nav_1" class="nav navon" onClick="tab(1)">详情描述</div>
            <div id="nav_2" class="nav" onClick="tab(2)">货品参数</div>
            <div id="nav_3" class="nav" onClick="tab(3)">备注说明</div>
            <div id="nav_4" class="nav" onClick="tab(4)" style="border-right:0px; width:24%;display: block;">货品图片</div>
        </div>
        <div class="tab_con">
            <a onclick="openDetail();" style="color:red;text-align: center;line-height: 17px;font-size: 16px;">点击查看详情</a>

            <div class="con" id="con_1"  style='display:none'><%=goods.desc%></div>
            ...
            <div class="con" id="con_3" ><div id='comment_container' style="display: none;"></div><%goods.remark%></div>
            <div class="con" id="con_4"><div id='recommand_container' style="display: none;"></div>
                <%each pics as value%>
                    <img src="<%value%>" style="width:100%;margin:5px 0;">
                <%/each%>
            </div>
        </div>
    </div>
    ...
    <div class="good_bottom">
        ...
        <!--            </div>-->
        <div class="cart" onclick="location.href='<?php  echo $this->createMobileUrl('shop/easy_deliver_cart')?>'"><i class="fa iconfont icon-cart" style="font-size: 18px"></i><br><span>购物车</span>
            <%if cartcount>0%><b><%cartcount%></b><%/if%>
        </div>
    ...
    <div class="buy" onclick="choose5();"  <%if shopset.cservice &&  goods.canaddcart%>style="width:27%"<%else%>style="width:70%"<%/if%>>前往购物车</div>
...
<script language="javascript">
    ...
    */
    ...
    require(['tpl', 'core','jquery.touchslider','swipe'], function(tpl, core) {
        ...
        core.json('behalf/buyer',{op:'detail',pa:1,id:'<?php  echo $_GPC['id'];?>',mid:"<?php  echo $mid;?>",is_admin:'<?php  echo $_GPC['is_admin'];?>'},function(json){

            tip = core.tip;
            if(json.status<=0){
                core.message('没有找到您想要的宝贝哦~',"<?php  echo $this->createMobileUrl('behalf/buyer')?>&op=buy_goods",'error');
                return;
            }
    ...
    function choose(act){
        ...
        <?php  } ?>
            ...
            function sure_buy(){
                ...
                require(['core'],function(core){
                    core.json('behalf/buyer',{op:'check_buy', id:'<?php  echo $_GPC['id'];?>','total_num':total_num},function(res){
                        if(res.status==1){
                            core.json('behalf/buyer',{op:'sure_buy', id:'<?php  echo $_GPC['id'];?>','total_num':total_num,'is_same':0},function(cret){
                                if(cret.status==0){
                                    if(confirm(cret.result.msg)){
                                        location.href = "<?php  echo $this->createMobileUrl('behalf/buyer')?>&op=order_manage";
                                    }else{
                                        window.location.reload();
                                    }
                                } else{
                                    core.tip.show(cret.result.msg);
                                    return;
                                }
                            },true,true);
                        }else if(res.status==2){
                            if (window.confirm('系统检测到您已买过此货品，确定是同一批货品？')) {
                                //确定
                                core.json('behalf/buyer',{op:'sure_buy', id:'<?php  echo $_GPC['id'];?>','total_num':total_num,'is_same':1},function(cret){
                                    if(cret.status==0){
                                        if(confirm(cret.result.msg)){
                                            location.href = "<?php  echo $this->createMobileUrl('behalf/buyer')?>&op=order_manage";
                                        }else{
