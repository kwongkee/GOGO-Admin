        ...
        <div class="layui-col-md12">
            <div class="layui-card">
                <div class="layui-card-header">
                    <p>已通知待确认列表</p>
<!--                    <div class="layui-btn layui-btn-normal layui-btn-sm" onclick="openWindow('',1)">新增货品</div>-->
                </div>
...
<script>
    layui.use(['layer', 'form', 'table', 'upload', 'laydate'], function () {
        ...
        table.render({
            elem: '#mainTable'
            ,url: "./index.php?i=3&c=entry&do=onlinepay&p=index&m=sz_yi&op=status1&pa=1"
            ,cellMinWidth: 200
            ,cols: [[
                {field:'ordersn_flow', title: '支付单号'}
                ,{field:'totalmoney', title: '待付金额'}
                ,{field:'statusname', title: '支付单状态'}
                ,{field:'createtime', title: '创建时间'}
                ,{align:'center',  title: '操作',fixed:'right',width:120, templet: function(d){
                        return [
                            '<a onclick="openWindow('+"'"+ d.id +"'" +','+"3"+ ');" class="layui-btn layui-btn-success layui-btn-xs">确认</a>',
    ...
    function openWindow(id,typ)
    {
        if(typ==1){
            //新增
            window.location.href="./index.php?i=3&c=entry&do=behalf&m=sz_yi&p=merchant&op=save_product";
        }else if(typ==2){
            //确认
