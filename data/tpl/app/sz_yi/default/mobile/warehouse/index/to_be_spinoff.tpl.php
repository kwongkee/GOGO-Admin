...
<div class="layui-col-xs12">
    ...
    <div class="info_search">
        ...
        <div class="info_box info_search_box">
            ...
            <div class="line layui-col-xs12"></div>
            <div class="layui-col-xs12 disf">
                分拆状态：<span style="color:#ff2222;"><?php  echo $v['spin_status'];?></span>
            </div>
            <div class="line layui-col-xs12"></div>
            <div class="layui-col-xs12 disf">
                申请分拆时间：<?php  echo date('Y-m-d H:i',$v['createtime'])?>
            </div>
            <div class="line layui-col-xs12"></div>
            <div class="layui-col-xs12" style="padding:15px 10px;box-sizing: border-box;text-align: right;">
                <div class="layui-btn layui-btn-primary" style="border:1px solid #1790FF;color:#1790FF;" onclick="chayan(<?php  echo $v['orderid'];?>,'<?php  echo $v['express_no'];?>','<?php  echo $v['ordersn'];?>',<?php  echo $v['status2'];?>)">立即分拆
                </div>
            </div>
        </div>
        <?php  } } ?>
...
<script>
    ...
    function chayan(id, express_no,ordersn,status2) {
        ...
        layer.open({
            ...
            title:"立即分拆["+ordersn+"]",
            area: ['100%', '100%'], //宽高
            content: './index.php?i=3&c=entry&p=index&do=warehouse&m=sz_yi&op=show_spinoff_box&orderid='+id+'&express_no='+express_no+'&ordersn='+ordersn+'&status2='+status2
        });
...
<script>
    ...
    function express_info(express_no) {
        var layer = layui.layer
            ...
            , form = layui.form;
        $.ajax({
            url: "./index.php?i=3&c=entry&p=index&do=warehouse&m=sz_yi&op=to_be_spinoff",
            method: 'post',
                ...
                layer.msg(res.result.msg, {time: 3000}, function () {
                    if (res.status == 0) {
                        ...
                        var html = '<div class="info_box info_search_box">\n' +
                            ...
                            '                <div class="line layui-col-xs12"></div>\n' +
                            '                <div class="layui-col-xs12 disf">\n' +
                            '                    分拆状态：<span style="color:#ff2222;">' + res.result.data.spin_status + '</span>\n' +
                            '                </div>\n' +
                            '                <div class="line layui-col-xs12"></div>\n' +
                            '                <div class="layui-col-xs12 disf">\n' +
                            '                    申请分拆时间：' + res.result.data.createtime + '\n' +
                            '                </div>\n' +
                            '                <div class="line layui-col-xs12"></div>\n' +
                            '                <div class="layui-col-xs12" style="padding:15px 10px;box-sizing: border-box;text-align: right;">\n' +
                            '                    <div class="layui-btn layui-btn-primary" style="border:1px solid #1790FF;color:#1790FF;" onclick="chayan(' + res.result.data.orderid + "," + "'" + res.result.data.express_no + "'"+ "," + res.result.data.ordersn + "," + res.result.data.status2 +')">立即分拆</div>\n' +
                            '                </div>\n' +
                            '            </div>';
