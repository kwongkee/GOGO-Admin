                ...
                <div class="layui-col-xs12">
                    <table class="layui-table" style="margin-top:0;">
                        ...
                        <tbody>
                            <?php  if(is_array($order['content']['event_date'])) { foreach($order['content']['event_date'] as $k => $v) { ?>
                                <tr>
                                    ...
                                    <td><?php  echo $order['content']['event_currency'][$k];?> <?php  echo sprintf('%.2f',substr(sprintf("%.4f", $order['content']['event_totalprice'][$k]), 0, -2));?></td>
                                    <td>
                                        <div class="layui-btn layui-btn-xs" onclick="info(<?php  echo $k;?>,<?php  echo $order['id'];?>)">查看详情</div>
                                    </td>
                                </tr>
                            <?php  } } ?>
                        <tr style="background: rgb(189,215,238);color:#000;font-weight: bold;">
                            ...
                            <td rowspan="3">
                                <div class="layui-btn layui-btn-xs" onclick="moneyInfo(<?php  echo $order['id'];?>)">合计详情</div>
                            </td>
                        </tr>
                        <tr style="background: rgb(189,215,238);color:#000;font-weight: bold;">
                            <td>开票税费</td>
...
<script>
    layui.use(['layer','jquery','element','laydate','form'], function() {
        ...
        form.on('submit(component-form-element)', function(data){
            ...
            $.ajax({
                url:"https://shop.gogo198.cn/app/index.php?i=3&c=entry&do=warehouse&p=warehouse&op=sureOrderList&m=sz_yi&id=<?php  echo $id;?>&sub=1",
                type:'post',
                dataType:'JSON',
                data:data.field,
    ...
    function add_line(){
        var layer = layui.layer
            ,form = layui.form
            ,laydate = layui.laydate;
        let dateNum = $('#dateNum').val();

        let html = '<div class="event layui-col-xs12">\n' +
            '                            <div class="layui-col-xs12">\n' +
            '                                <div class="layui-col-xs3 title">日期时间</div>\n' +
            '                                <div class="layui-col-xs9 val">\n' +
            '                                    <input type="text" class="layui-input" name="event_date[]" id="event_date'+dateNum+'" value="" placeholder="yymmddhhii" lay-verify="required">\n' +
            '                                </div>\n' +
            '                            </div>\n' +
            '                            <div class="line layui-col-xs12"></div>\n' +
            '                            <div class="layui-col-xs12">\n' +
            '                                <div class="layui-col-xs3 title">订单事项</div>\n' +
            '                                <div class="layui-col-xs9 val">\n' +
            '                                    <input type="text" lay-verify="required" name="event_name[]" value="" placeholder="" autocomplete="off" class="layui-input">\n' +
            '                                </div>\n' +
            '                            </div>\n' +
            '                            <div class="line layui-col-xs12"></div>\n' +
            '                            <div class="layui-col-xs12">\n' +
            ...
            <?php  if(is_array($unit)) { foreach($unit as $vo) { ?>
            html += '                                            <option value="'+'<?php  echo sprintf("%03d",$vo["code_value"]);?>'+'"><?php  echo $vo['code_name'];?></option>\n';
            <?php  } } ?>
            html += '                                    </select>\n' +
            '                                </div>\n' +
            '                            </div>\n' +
            '                            <div class="line layui-col-xs12"></div>\n' +
            '                            <div class="layui-col-xs12">\n' +
            '                                <div class="layui-col-xs3 title">订单单价</div>\n' +
            '                                <div class="layui-col-xs9 val">\n' +
            '                                    <input type="number" class="layui-input" name="event_price[]" placeholder="" value="">\n' +
            '                                </div>\n' +
            '                            </div>\n' +
            '                            <div class="line layui-col-xs12"></div>\n' +
            '                            <div class="layui-col-xs12">\n' +
            ...
            '                                <div class="layui-col-xs9 val">\n' +
            '                                    <input type="number" class="layui-input" name="event_num[]" placeholder="" value="">\n' +
            '                                </div>\n' +
            '                            </div>\n' +
            '                            <div class="line layui-col-xs12"></div>\n' +
            '                            <div class="layui-col-xs12" style="text-align: center;">\n' +
            '                                <div class="layui-btn layui-btn-danger layui-btn-md remove" onclick="del_line(this)">删除</div>\n' +
            '                            </div>\n' +
            '                        </div>';


        $('#dateNum').val(parseInt(dateNum) + 1);
        $('.btm_add').before(html);
        laydate.render({
            elem: '#event_date'+dateNum
            ,type: 'datetime'
    ...
    function del_line(t){
        ...
        layer.confirm('确定删除该事项吗？', function(index){
    ...
    function info(key,id){
        ...
        window.location.href='https://shop.gogo198.cn/app/index.php?i=3&c=entry&do=warehouse&p=warehouse&op=order_detail&m=sz_yi&key='+key+'&id='+id;
        // layer.open({
        //     type: 2,
        //     title: '账单详情',
    ...
    function moneyInfo(id){
        ...
        window.location.href='https://shop.gogo198.cn/app/index.php?i=3&c=entry&do=warehouse&p=warehouse&op=money_detail&m=sz_yi&id='+id;
    }

    function chat(){
        var layer = layui.layer;

        window.location.href='https://shop.gogo198.cn/app/index.php?i=3&c=entry&do=warehouse&p=warehouse&op=chat_page&m=sz_yi&id='+<?php  echo $order['id'];?>;
    }
</script>
