...
<script>
    $(function() {
        layui.use(['layer', 'form', 'table', 'upload', 'laydate'], function () {
            ...
            table.render({
                elem: '#mainTable'
                ,url: "<?php  echo $this->createMobileUrl('policy/index');?>&op=column_list&pa=1&cate_id=<?php  echo $cate_id;?>"
                ,cellMinWidth: 200
                ,cols: [[
    ...
    function openWindow(id,typ)
    {
        if(typ==1){
            //编辑
            window.location.href="<?php  echo $this->createMobileUrl('policy/index');?>&op=save_column&id="+id;
        }else if(typ==2){
            //新增栏目
            window.location.href="<?php  echo $this->createMobileUrl('policy/index');?>&op=save_column&cate_id="+id;
        }else if(typ==3){
            //删除栏目
            ...
            layer.confirm('确认删除？', {
                ...
                $.ajax({
                    url:"<?php  echo $this->createMobileUrl('policy/index');?>&op=del_column&id="+id,
                    method:'post',
                    data:'',
                    dataType:'JSON',
