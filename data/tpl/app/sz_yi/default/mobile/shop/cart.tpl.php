...
<style type="text/css">
    ...
    .cart_top .title .icon-chevron-left {
        position: absolute;
        left: 10px;
        top: 0px
    }
    ...
    .cart_good .info .inner .optionsel {
        height: 30px;
        width: 50%;
        float: left;
        line-height: 30px;
    ...
    .cart_good .info .inner .optionsel .content {
        float: left;
        width: 100%;
        overflow: hidden;
        height: 30px;
    ...
    .cart_good .info .inner .optionsel .cico {
        float: right;
        width: 20px;
        text-align: center;
        margin-left: -20px;
...
<script id='option_info' type='text/html'>
    ...
    <div class="good_choose">
        ...
        <div class="other">
            ...
            <%each specs as spec%>
            <input type='hidden' name="optionid[]" class='optionid optionid_<%spec.id%>' value=""
                   title="<%spec.title%>">
            <div class="spec"><%spec.title%></div>
            <div class="spec_items options_<%spec.id%>" title="<%spec.title%>">
                <%each spec.items as o%>
                <div class="option option_<%spec.id%>" specid='<%spec.id%>' oid="<%o.id%>" sel='false'
                     title='<%o.title%>' thumb='<%o.thumb%>'><%o.title%>
                </div>
                <%/each%>
            </div>
            <%/each%>
...
<script id='cart_empty' type='text/html'>
    ...
    <div class="card_no"><i class="fa fa-shopping-cart" style="font-size:100px;"></i><br><span
            style="line-height:18px; font-size:16px;">购物车快饿瘪了</span><br>主人快去给我找点东西吧
    </div>
...
<script id='cart_list' type='text/html'>
    ...
    <div class="cart_main">
        ...
        <div style="background-color: white;line-height: 35px;">
            <div class="ico singleBuss" style="float: left;"><i class="iconfont icon-check-circle-o"
                                                                style="color:#f15353;font-size: 23px;margin-left: 4px;"></i>
            </div>
            <p style="margin-bottom: 0px;margin-left: 31px;font-size: 12px;"><%index%><i></i></p>
        </div>
        <%each val as value %>
        ...
        >
            ...
            <div class="img"
                 onclick="location.href='<?php  echo $this->createMobileUrl('shop/detail')?>&id=<%value.goodsid%>'"><img
                    style="border-radius: 10%;" src="<%value.thumb%>"/></div>
            <div class="info">
                <div class='inner'>
                    <div class="name"
                         onclick="location.href='<?php  echo $this->createMobileUrl('shop/detail')?>&id=<%value.goodsid%>'">
            ...
            <div class="other">
                ...
                <div class="num">
                    ...
                    <input type="text" class="shownum" value="<%value.total%>"/>
        ...
        <%/each%>
        <%/each%>
        <div class="cart_count">共 <span class='total'><%total%></span> 件商品 合计: ￥<span
                class='totalprice'><%totalprice%></span></div>
    </div>
    <div class="cart_pay">
        <div class="all"><i class="iconfont icon-check-circle-o" style="color:#f15353;"></i> <span class='t'>全选</span></div>
        <div class="paysub <%if total<=0%>disabled<%/if%>">结算(<span class='total'><%total%></span>)</div>
        <div class="text">合计：<span>￥<span class='totalprice'><%totalprice%></span></span><br><span
                style="color:#999; font-size:14px;">不含运费</span></div>
    </div>
</script>
<script language="javascript">
    ...
    require(['tpl', 'core'], function (tpl, core) {
        ...
        core.json('shop/cart', {
            ischannelpick: "<?php  echo $_GPC['ischannelpick'];?>",
            ischannelpay: "<?php  echo $_GPC['ischannelpay'];?>",
            ...
            $('.paysub').click(function () {
                ...
                if(isjiesuan){
                    //core.tip.show("不能同其他商家商品一起结算");
                    alert('请选择同一商家的商品进行付款！');
                    return ;
                }

                core.json('order/split_order', {
                    cartids: ids.join(',')
                },function (json) {
                    console.log(json);
                    if (json.status==0){
                        location.href = core.getUrl('order/confirm', {
                            cartids: ids.join(','),
                            ischannelpay: "<?php  echo $_GPC['ischannelpay'];?>",
                            ischannelpick: "<?php  echo $_GPC['ischannelpick'];?>",
                            isyunbipay: isyunbipay
                        });
                    } else {
            ...
            $('.shownum').blur(function () {
                ...
                core.json('shop/cart', {
                    'op': 'updatenum',
                    id: $(this).closest('.cart_good').data('cartid'),
                    goodsid: $(this).closest('.cart_good').data('goodsid'),
                    total: num
                }, null, false, true);
                calcprice();
            ...
            $(".singleBuss").click(function () {

                var $this = $(this);
                var sel = $this.attr('sel');
                bussList.push($this.next().text());
                $this.parent().nextAll().each(function () {

                    if ($(this).attr('class') != 'cart_good') {
                        return false;
                    }
                    setSelect($(this).find('.ico'), sel)
                });
                setSelect($(this), sel, true);
