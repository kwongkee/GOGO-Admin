...
<script src="../addons/sz_yi/static/js/layui/layui.js"></script>
<style>
    .disf{display:flex;align-items: center;}
    /**聊天模板**/
    .conatact_info{padding:10px;box-sizing: border-box;}
    .chat_content{border:1px solid #eee;height:60%;min-height:400px;max-height:400px;margin-bottom:0px;overflow-y:scroll;overflow-x:hidden;background:rgb(240,230,221);border:1px solid #000;border:0;}
    ...
    /**左**/
    .conatact_info .left .other_people_name{background:#0078d3;color: #fff;border-radius: 50%;font-size: 12px;text-align: center;height: 30px;width: 30px;line-height: 30px;}
    ...
    .conatact_info .left .more_sel_box{display:none;position: absolute;font-weight: 100;background: #fff;box-shadow: 0px 0px 8px #999;top: 30px;left: 15px;z-index: 9;padding: 5px 10px;font-size: 13px;box-sizing: border-box;width: 70px;text-align: center;cursor:pointer;}
    ...
    .chatBox_con .chatContent .leftBtm .quoteBox .quote_info{background: #ddd;padding: 0px 20px 0 10px;max-width: 150px;text-overflow: ellipsis;white-space: nowrap;overflow:hidden;color: #000;font-size: 12px;}
    ...
    .shareBox .shareBtn .left{background:#1f5188;max-width: 40%;}
    .shareBox .shareBtn .right{background:#009688;max-width: 40%;}
    /**聊天样式结束**/
    ...
    .submit-btn{background:#0078d3 !important;}
    @media screen and (max-width: 768px){
        ...
        .shareBox .shareBtn .left,.shareBox .shareBtn .right{min-width: 40%;}
    }
</style>
        ...
        <div class="layui-card">
            <div class="layui-card-body" style="padding: 0px;background:#fff;">
                <form class="layui-form" action="" lay-filter="component-form-group">
                    ...
                    <!--聊天输入框-->
                    <div class="layui-form-item" style="margin-bottom:0;padding-top: 0px;background: rgb(240,230,221);">
                        <div class="chatContent">
                            <textarea class="layui-textarea" name="content" id="content" placeholder="开始聊天~~"></textarea>
                            <div class="disf rightBtm" style="align-items: flex-end;">
                                <div class="chatIcon">
                                    <img src="//boss.gogo198.cn/img/chat_upload.png" alt="" id="fileUpload">
                                </div>

                                <div class="chatIcon" style="display: none;">
                                    <img src="//boss.gogo198.cn/img/chat_share.png" alt="" style="width: 26px;height:26px;" id="shareBox" onclick="shareInfo()">
                                </div>
...
<script type="text/javascript">
    layui.use(['layer','form','laydate','upload','element'],function() {
        ...
        form.on('submit(component-form-demo1)', function (data) {
            ...
            $.ajax({
                url: "<?php  echo $this->createMobileUrl('customer/chat');?>",
                method: 'post',
                data: data.field,
                dataType: 'JSON',
        ...
        upload.render({
            ...
            ,done: function(res){
                ...
                if(res.code == 1)
                {
                    let content = '<a href="//shop.gogo198.cn/'+ res.file_path +'" target="_blank"><img src="//shop.gogo198.cn/'+ res.file_path +'" onerror=src="//shop.gogo198.cn/attachment/images/default_file.png" /></a>';
                    let pid = $('#pid').val();
                    let origin_page = '';
                    <?php  if($isframe==1) { ?>
                    origin_page = window.parent[0].location.href;
                    <?php  } else { ?>
                    origin_page = window.parent.location.href;
                    <?php  } ?>

                        $.ajax({
                            url: "/?s=customer/customer_online",
                            method: 'post',
                            data: {'pid':pid,'content_type':2,'pa':1,'who_send':"<?php  echo $who_send;?>",'content':content,'origin_page':origin_page,'company_id':"<?php  echo $company_id;?>"},
                            dataType: 'JSON',
                            success: function (res) {
                                layer.msg(res.msg, {time: 2000}, function () {
                                    if (res.code == 0) {
    ...
    function sendLink(){
        ...
        $.ajax({
            url: "/?s=customer/customer_online",
            method: 'post',
            data: {'pid':pid,'content_type':2,'pa':1,'content':content,'origin_page':upLink},
            dataType: 'JSON',
            success: function (res) {
                layer.msg(res.msg, {time: 2000}, function () {
                    if (res.code == 0) {
                        ...
                        for(let i2=0;i2<res.data[i].info.length;i2++) {
                            if (res.data[i].info[i2]['pid'] == 0) {
                                ...
                                // }else {
                                    ...
                                    //         '                              </div>';
                                    // }
                                    html += '                             <div class="chatlog">' + res.data[i].info[i2]['text'] + '</div>';

                                    html += '                            <div class="toolbar disf">\n' +
                                        '                                    <div class="more_sel toolbarImg" onclick="more_sel(this,'+res.data[i].info[i2]['id']+')"><img src="//boss.gogo198.cn/img/more_sel.png"></div>';
                            ...
                            else if (res.data[i].info[i2]['pid'] > 0) {
                                html += '                       <div class="conatact_info layui-col-xs12">\n';
                                // if(res.data[i].info[i2]['is_withdraw']==1){
                                ...
                                // }else {
                                    ...
                                    html += '                              <div class="chatlog">' + res.data[i].info[i2]['text'] + '</div>\n' +
                                        ...
                                        '                                      <div class="copy_sel toolbar_info" onclick="copyInfo(this,'+res.data[i].info[i2]['id']+')">复制</div>\n'+
                                        '                                    </div>\n'+
                                        // '                                         <div class="quote toolbarImg" onclick="quoteInfo(this,' + res.data[i].info[i2]['id'] + ')"><img src="https://www.gogo198.cn/assets/d2eace91/images/newhome/chat_quote.png"></div>\n' +
                                        '                                  </div>\n'+
                                        '                                </div>\n'+
