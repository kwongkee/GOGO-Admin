...
.order_sub12 {height:44px; margin:14px 5px; background:#19F95E; border-radius:4px; text-align:center; font-size:16px; line-height:44px; color:#fff;}
.order_sub13 {height:44px; margin:14px 5px; background:#2e78d0; border-radius:4px; text-align:center; font-size:16px; line-height:44px; color:#fff;}
...
<script id='tpl_order_info' type='text/html'>
    ...
    <%if order.price>0 %>
	    ...
	    <%if tgwechat.success%><div class="button order_sub12" >微信支付</div><%/if%>
	    <%if tgalipay.success%><div class="button order_sub13" >支付宝支付</div><%/if%>	
	    <!--2017-11-01-->
   	<!-- 余额支付  -->
    <%if credit.success %>
        ...
        core.json('order/pay',{orderid:'<?php  echo $_GPC['orderid'];?>',openid:"<?php  echo $openid;?>"},function(json){
            ...
            if(result.tgwechat.success){ 
                 $('.order_sub12').click(function() {
                    ...
                    core.json('order/pay', {op: 'pay',type: 'tgwechat', orderid:'<?php  echo $_GPC['orderid'];?>',openid:"<?php  echo $openid;?>",deduct:deduct}, function (rjson) {
                                ...
                                //if(confirm(str)) {
                                    ...
                                    $.ajax({
                                        ...
                                        data:{
                                            ...
                                            <!-- 请求支付 -->
                                            if(rjson.result.tgwechat['success']) {
                                                //2018-08-21
                                                var tgw = rjson.result.tgwechat;
                        ...
                        /*if(rjson.result.tgwechat['success']) {
                        	//2018-08-21
                        	var tgw = rjson.result.tgwechat;
                        	$('#wecpay').attr('action','http://shop.gogo198.cn/addons/sz_yi/payment/tgwechat/Paymentess.php')
                        	$('#tid').val(tgw.tid);
                        	$('#opid').val(tgw.openid);
                        	$('#title').val(tgw.title);
                        	$('#fee').val(tgw.fee);
                        	...
                        	//location.href = rjson.result.tgwechat.payurl;
                        	/*var wechat = rjson.result.tgwechat.pay_info;                        	
		                  	WeixinJSBridge.invoke('getBrandWCPayRequest', {
		                        ...
		                        if (res.errMsg == 'get_brand_wcpay_request:ok') {
		                          	core.json('order/pay',{op:'pay',orderid:'<?php  echo $_GPC['orderid'];?>',type:'tgwechat',deduct:deduct},function(pay_json){ 
            ...
            if(result.tgalipay.success){
                 ...
                 $('.order_sub13').click(function(){
                    ...
                    core.json('order/pay', {op: 'pay',type: 'tgalipay', orderid:'<?php  echo $_GPC['orderid'];?>',openid:"<?php  echo $openid;?>",deduct:deduct}, function (rjson) {
                        if(rjson.status!=1) {
                            $('.button').removeAttr('submitting');
                            core.tip.show(rjson.result);
                            return;
           ...
           if(result.wechat.success){
                ...
                if (result.wechat.weixin) { 
                        if($(this).attr('submitting')=='1'){
                         ...
                         $('.button').attr('submitting',1);
                         var deduct = ($('#deductmoney').length>0 &&$('#deductmoney').attr('on')=='1' )?1:0 ;
                         //ajax 数据请求order/pay($option = pay & type = weixin)  传送：orderid 订单号ID，
