                ...
                <div class="layui-card-body" style="padding:20px 0 0;">
                    <form class="layui-form" action="" method="post" lay-filter="component-form-element">
                        ...
                        <div class="layui-form-item">
                            ...
                            <div class="layui-inline" style="margin-bottom:5px;width:100%;">
                                <label class="layui-form-label" style="padding:9px;width:60px;">补缺方式</label>
                                <div class="layui-input-block disf">
                                    <select name="method" id="method" lay-verify="required" lay-filter="method">
                                        <option value="1">系统补缺</option>
                                        <option value="2">人工补缺</option>
                                    </select>
                                </div>
                            </div>
                        </div>
        ...
        layui.use(['layer', 'form', 'table', 'upload', 'laydate'], function () {
            ...
            form.on('submit(component-form-element)', function(data){
                ...
                if(data.field['method']==1){
                    //系统补缺
                    timer = load();
                    var dataList = {batch_num:"<?php  echo $batch_num;?>",adjust_method:data.field['adjust_method']};
                    $.post("<?php  echo $this->createMobileUrl('declare/report_risk');?>&op=system_fill",dataList,function(res){
                        res = $.parseJSON(res);
                        if(res.status==-1){
                            hide_load(timer);
                            ...
                            layer.msg(res.result.msg,{time:3000}, function () {
                                layer.open({
                                    type: 2,
                                    title: '人工补缺',
                                    content: "<?php  echo $this->createMobileUrl('declare/report_risk');?>&op=manual_fill_page&pre_batch_num=<?php  echo $batch_num;?>&ids="+res.result.need_fill_ids,
                                    area:['80%','250px']
                                });
                            });
                        }else{
                    ...
                    return false;
                }else if(data.field['method']==2){
                    //人工补缺
                    layer.open({
                        type: 2,
                        title: '人工补缺',
                        content: "<?php  echo $this->createMobileUrl('declare/report_risk');?>&op=manual_fill_page&ids=0&pre_batch_num=<?php  echo $batch_num;?>&adjust_method="+data.field['adjust_method'],
                        area:['80%','250px']
                    });
                }
            });
