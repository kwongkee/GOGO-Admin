...
<script>
    $(function() {
        layui.use(['layer', 'form', 'table', 'upload', 'laydate'], function () {
            ...
            table.render({
                elem: '#mainTable'
                ,url: "<?php  echo $this->createMobileUrl('declare/report_risk');?>&op=display&pa=1"
                ,cellMinWidth: 200
                ,cols: [[
                    {field:'pre_batch_num', title: '预报编号'}
                    ,{field:'createtime', title: '预报时间'}
                    ,{align:'center', width:80,fixed:'right', title: '操作', templet: function(d){
                            return [
                                '<a onclick="openWindow('+ "'"+d.pre_batch_num+"商品归类'" +',' + "'"+ d.logisticsNo +"'" +','+"'"+d.pre_batch_num+"'"+ ','+1+');" class="layui-btn layui-btn-normal layui-btn-xs">商品归类</a>',
                            ].join('');
                        }}
                ]]
                ,page: 1
    ...
    function openWindow(title,sys_batch_num,pre_batch_num,typ)
    {
        var layer = layui.layer;
        var url = '';
        // if(typ==1){
        //     //风控列表
        //     url="<?php  echo $this->createMobileUrl('declare/report_risk');?>&op=risk_list&sys_batch_num="+sys_batch_num+"&pre_batch_num="+pre_batch_num;
        // }

        if(typ==1){
            //商品归类
            layer.confirm('若商品编码为空，请选择归类缺省。',{
                btn:['归类缺省','商品列表']
                ...
                layer.open({
                    'type':2,
                    'title':'归类缺省',
                    'content':"<?php  echo $this->createMobileUrl('declare/report_risk');?>&op=goods_cateFill&batch_num="+pre_batch_num,
                    'area':['100%','100%']
                });
                return false;
                        ...
                        if(res.status==-1){
                            ...
                            layer.msg(res.result.msg,{time:3000}, function () {
                                layer.open({
                                    type: 2,
                                    title: '人工补缺',
                                    content: "<?php  echo $this->createMobileUrl('declare/report_risk');?>&op=manual_fill_page&ids="+res.result.need_fill_ids,
                                    area:['80%','250px']
                                });
                            });
                        }else{
                ...
                function(){
                    ...
                    layer.open({
                        type: 2,
                        title: '人工补缺',
                        content: "<?php  echo $this->createMobileUrl('declare/report_risk');?>&op=manual_fill_page&ids=0&pre_batch_num="+pre_batch_num,
                        area:['80%','250px']
                    });
                });
            }, function(index){
                //商品列表
                url="<?php  echo $this->createMobileUrl('declare/report_risk');?>&op=goods_list&pre_batch_num="+pre_batch_num;
