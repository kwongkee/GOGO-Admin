    ...
    <div class="layui-row layui-col-space15">
        <div class="layui-col-md12">
            <div class="layui-card">
                <div class="layui-card-header">上传涉检文件</div>
                            ...
                            <div class="layui-upload" style="text-align:left;">
                                ...
                                <blockquote class="layui-elem-quote layui-quote-nm yulan" style="margin-top: 10px;">
                                    ...
                                    <div class="layui-upload-list" id="file-upload-list">
                                        <?php  if(!empty($list['file2'])) { ?>
                                        <?php  if(is_array($list['file2'])) { foreach($list['file2'] as $k => $v) { ?>
        ...
        layui.use(['layer', 'form', 'table', 'upload', 'laydate'], function () {
            ...
            form.on('submit(component-form-element)', function(data){
                if($('#file-upload-list').children().length == 0){
                    layer.msg('请上传涉检文件！',{time:2000});
                    return false;
                }

                layer.load(); //上传loading
                $.ajax({
                    url:"<?php  echo $this->createMobileUrl('declare/risk_manage');?>&op=insp_file&itemNo=<?php  echo $itemNo;?>&pre_batch_num=<?php  echo $batch_num;?>&pa=1&hscode=<?php  echo $hscode;?>",
                    type:'post',
                    dataType:'JSON',
                    data:data.field,
                    success:function(res){
                        ...
                        if(res.result.is_synchronization==1){
                            ...
                            layer.confirm('系统检测到还有其它相同编码，需要全部同步为相同文件吗？',{
                                ...
                                let dataList = {op:'synchronization',typ:2,pre_batch_num:"<?php  echo $batch_num;?>",hscode:"<?php  echo $hscode;?>"};
