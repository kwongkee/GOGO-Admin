...
<script>
    $(function() {
        layui.use(['layer', 'form', 'table', 'upload', 'laydate','element'], function () {
            ...
            table.render({
                elem: '#mainTable'
                ,url: "<?php  echo $this->createMobileUrl('declare/risk_manage');?>&op=threeInvolveGoods&pre_batch_num=<?php  echo $batch_num;?>&pa=1"
                ,cellMinWidth: 200
                ,cols: [[
            ...
            table.render({
                elem: '#mainTable2'
                ,url: "<?php  echo $this->createMobileUrl('declare/risk_manage');?>&op=threeInvolveGoods&pre_batch_num=<?php  echo $batch_num;?>&pa=2"
                ,cellMinWidth: 200
                ,cols: [[
                    {field:'gname', title: '品名'}
                    ,{field:'gcode', title: '编码'}
                    ,{field:'reason', title: '原因'}
                    ,{align:'center', title: '操作',fixed:'right',width:150, templet: function(d){
                        var content='<a onclick="openWindow('+ "'"+d.gcode+"的信息'" +',' + "'"+ d.gcode +"'" +','+"1"+ ');" class="layui-btn layui-btn-normal layui-btn-xs">查看详情</a>';
                            if(d.file!='' && typeof(d.file)=='string'){
                                content+='<a onclick="openWindow('+"'"+d.gname+"'"+','+"'"+d.gcode+"'"+','+"2"+','+"'"+d.itemNo+"'"+');" class="layui-btn layui-btn-xs layui-btn-normal" style="background-color:#297911;">上传文件</a>';
                            }else{
                                content+='<a onclick="openWindow('+"'"+d.gname+"'"+','+"'"+d.gcode+"'"+','+"2"+','+"'"+d.itemNo+"'"+');" class="layui-btn layui-btn-xs layui-btn-danger">上传文件</a>'
                            }
                            return content;
                            // return [
                            //
            ...
            table.render({
                elem: '#mainTable3'
                ,url: "<?php  echo $this->createMobileUrl('declare/risk_manage');?>&op=threeInvolveGoods&pre_batch_num=<?php  echo $batch_num;?>&pa=3"
                ,cellMinWidth: 200
                ,cols: [[
                    {field:'gname', title: '品名'}
                    ,{field:'gcode', title: '编码'}
                    ...
                    ,{align:'center', title: '操作',fixed:'right',width:150, templet: function(d){
                        ...
                        var content='<a onclick="openWindow('+ "'"+d.gcode+"的信息'" +',' + "'"+ d.gcode +"'" +','+"1"+ ');" class="layui-btn layui-btn-normal layui-btn-xs">查看详情</a>';
                            if(d.file2!='' && typeof(d.file2)=='string'){
                                content+='<a onclick="openWindow('+"'"+d.gname+"'"+','+"'"+d.gcode+"'"+','+"3"+','+"'"+d.itemNo+"'"+');" class="layui-btn layui-btn-xs layui-btn-normal" style="background-color:#297911;">上传文件</a>';
                            }else{
                                content+='<a onclick="openWindow('+"'"+d.gname+"'"+','+"'"+d.gcode+"'"+','+"3"+','+"'"+d.itemNo+"'"+');" class="layui-btn layui-btn-xs layui-btn-danger">上传文件</a>';
                            }
                            return content;
                        } }
                ]]
