...
<script>
    $(function() {
        layui.use(['layer', 'form', 'table', 'upload', 'laydate'], function () {
            ...
            table.render({
                elem: '#mainTable'
                ,url: "<?php  echo $this->createMobileUrl('declare/risk');?>&op=value_default&pa=1&logisticsNo=<?php  echo $logisticsNo;?>&batch_num=<?php  echo $batch_num;?>"
                ,cellMinWidth: 200
                ,cols: [[
                    {field:'itemNo', title: '商品货号'}
                    ,{field:'itemName', title: '商品名称'}
                    ,{field:'price', title: 'FOB单价'}
                    ,{field:'totalPrice', title: 'FOB总价'}
                    ,{field:'charge', title: '收款金额'}
                    ,{field:'chargeDate', title: '到账时间'}
                    ,{field:'unit', title: '申报计量单位'}
                    ,{field:'unit1', title: '法定计量单位'}
                    ,{align:'center', fixed:'right', width:70, title: '操作', templet: function(d){
                        // && d.onebound_gid==0typeof(d.type)!='undefined' &&
                        if(d.type==2){
                            return [
                                '<a onclick="operation('+ "'"+d.itemName+"'" +',' + "'"+ d.logisticsNo +"'" +','+"'"+d.id+"'"+ ','+1+');" class="layui-btn layui-btn-normal layui-btn-xs">同品异价</a>',
                                // '<a onclick="operation('+ "'"+d.itemName+"'" +',' + "'"+ d.logisticsNo +"'" +','+"'"+d.id+"'"+ ','+2+');" class="layui-btn layui-btn-normal layui-btn-xs">异品异价</a>',
                            ].join('');
                        }else{
                            return [
                                '-'
    ...
    function fillVacancy(typ){
        var layer = layui.layer;
        var table = layui.table;
        let ids = $('#ids').val();
        if(ids=='' || ids==',' || typeof(ids)=='undefined'){
            layer.msg('暂无货值缺省记录！');return false;
        }
        ...
        layer.confirm('请选择补缺方式', {
            ...
            $.ajax({
                ...
                type:'POST',
                dataType:'json',
                data:{'op':'system_fill','ids':ids,'batch_num':"<?php  echo $batch_num;?>"},
                        ...
                        layer.msg(json.result.msg,{time:3000}, function () {
                            if (json.status == -1)
                            {
                                layer.open({
                                    type: 2,
                                    title: '人工补缺',
                                    content: "<?php  echo $this->createMobileUrl('declare/risk');?>&op=manual_fill_page&batch_num=<?php  echo $batch_num;?>&ids="+json.result.need_fill_ids,
                                    area:['80%','250px']
                                });
            ...
            var index = layer.open({
                type: 2,
                title: '人工补缺',
                content: "<?php  echo $this->createMobileUrl('declare/risk');?>&op=manual_fill_page&ids="+ids+"&batch_num=<?php  echo $batch_num;?>",
                area:['80%','250px']
            });
            // layer.full(index);
        });
    ...
    function operation(itemName,logisticsNo,id,typ){
        var layer = layui.layer;
        var table = layui.table;

        layer.confirm('当前品名存在同品异价，请在以下按钮作出选择。', {
            btn: ['同品同价','异品异价'] //按钮
        }, function() {
            layer.load(); //上传loading
            $('.layui-layer-shade').show();
            ...
            $.ajax({
                ...
                type: 'POST',
                dataType: 'json',
                data: {'op': 'tongPinYiPrice', 'itemName': itemName,'logisticsNo':logisticsNo,'typ':1,'batch_num':"<?php  echo $batch_num;?>"},
                success: function (json) {
                    layer.closeAll('loading'); //关闭loading
                    $('.layui-layer-shade').hide();
                    layer.msg(json.result.msg, {time: 3000}, function () {
            ...
            $.ajax({
                ...
                type: 'POST',
                dataType: 'json',
                data: {'op': 'tongPinYiPrice', 'itemName': itemName,'logisticsNo':logisticsNo,'typ':2,'batch_num':"<?php  echo $batch_num;?>"},
                success: function (json) {
                    layer.closeAll('loading'); //关闭loading
                    $('.layui-layer-shade').hide();
                    layer.msg(json.result.msg, {time: 3000}, function () {
