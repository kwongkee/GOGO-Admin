...
<style type="text/css">
    ...
    .addorder_topbar {height:64px; background:#5f6e8b; padding:15px;}
    .addorder_topbar .ico {height:34px; width:30px; line-height:34px; float:left; font-size:26px; text-align:center; color:#fff;}
    .addorder_topbar .ico .fa{color: #fff}
    .addorder_topbar .tips {height:34px;  margin-left:10px; font-size:13px; color:#fff; line-height:17px;}
    
    .addorder_nav { height:26px; padding:10px;box-sizing: content-box; background: #fff;margin: 10px 0;}
    .addorder_nav .nav { padding:2px 5px 2px 5px;; border:1px solid #5f6e8b; color:#5f6e8b; background:#fff; float:left; margin-left:10px;cursor:pointer;}
    .addorder_nav .selected { border:1px solid #ff6600; color:#ff6600;cursor:pointer; }
    ...
    .addorder_user .info .info1 .inner { margin-left:30px;margin-right:30px;overflow:hidden;width: auto;}
    .addorder_user .info .info1 .inner .user {box-sizing: content-box;height:30px; width:100%; font-size:16px; color:#333; line-height:35px;overflow:hidden;}
    .addorder_user .info .info1 .inner .address {height:20px; width:100%; font-size:14px; color:#999; line-height:20px;overflow:hidden;border: 0}
    .addorder_user .info .ico2 {height:50px;  width:30px; line-height:65px; float:right; font-size:16px; text-align:center; color:#999;cursor: pointer;}
    ...
    .addorder_good {height:auto;padding:10px;background:#fff;
    margin: 0 auto; margin-top:16px; border-bottom:1px solid #eaeaea; border-top:1px solid #eaeaea;}
    .addorder_good .ico {height:6px; /*width:10%; */line-height:36px; float:left; text-align:center;}
    .addorder_good .shop {height:36px; width:100%; padding-left:26px; border-bottom:1px solid #eaeaea; line-height:36px; font-size:18px; color:#333;}
    .addorder_good .good {height:70px; width:100%; padding:10px 0px; border-bottom:1px solid #eaeaea;}
    ...
    .addorder_pay {height:60px; width:100%; background:#fff; padding:0px 1%; margin-top:30px; border-top:1px solid #eaeaea;}
    ...
    .chooser .address {box-sizing: content-box;height:50px; background:#fff; padding:10px;; border-bottom:1px solid #eaeaea;width: auto;}
    ...
    .chooser .address .edit {cursor: pointer;height:50px; width:30px; float:right;margin-left:-30px;text-align:center;line-height:50px;color:#666;}

    .chooser .add_address {box-sizing: content-box;height:44px; padding:5px; background:#fff; border-bottom:1px solid #eaeaea; line-height:44px; font-size:16px; color:#666;cursor: pointer;}

   

    .address_main {width: 500px;min-height: 280px;left: 50%;top: 20%;margin-left: -250px;z-index: 999;background:#fff;  position: fixed;border-radius: 5px;    border: 4px solid rgba(0,0,0,.2);}
    ...
    .address_main .line select  { border:none;height:42px;width:100%;color:#666;font-size:16px;}
...
.select { -webkit-appearance: none }
    ...
    .carrier_input_info .row { margin:0;padding:0 10px; height:40px; background:#fff; border-bottom:1px solid #e8e8e8; line-height:40px; color:#999;}
    .carrier_input_info .row .title {height:40px; width:85px; line-height:40px; color:#444; float:left; font-size:14px;}
    .carrier_input_info .row .info { width:100%;float:right;margin-left:-85px; }
    .carrier_input_info .row .inner { margin-left:85px;margin-bottom: 0 }
    .carrier_input_info .row .inner input {height:30px; color:#666;background:transparent;margin-top:0px; width:100%;border-radius:0;padding:0px; margin:0px; border:0px; float:left; font-size:14px;}
...
.cnum {height:22px; width:81px; border:1px solid #e2e2e2; }
...
<script id='tpl_address_list' type='text/html'>
    <div class="chooser choose_address" >
        ...
        <%/each%>
        <div class="add_address"><i class="fa fa-plus-circle" style="margin-left:3%; margin-right:10px; line-height:44px; font-size:20px;"></i><?php  if($_W['uniacid'] == 18) { ?>New shipping address<?php  } else { ?>新增收货地址<?php  } ?></div>
    </div>
</script>
<script id='tpl_address_list_isverify' type='text/html'>
    <div class="chooser choose_address" >
        <%each list as address%>
        <div class="address" 
             data-addressid='<%address.id%>'
             ...
             data-address='<%address.address%>'
             >
            <div class="ico" ><%if address.id==selectedAdressID%><i class="fa fa-check" style="color:#0c9"></i><%/if%></div>
            <div class="info">
                <div class='inner'>
                    <div class="name"><%address.realname%>(<%address.mobile%>)</div>
                    <div class="addr"><%address.address%></div>
...
<script id='tpl_address_new' type='text/html'>
    ...
    <div class="address_main" >
        ...
        <div class="line"><input type="text" placeholder="<?php  if($_W['uniacid'] == 18) { ?>Mobile<?php  } else { ?>联系电话<?php  } ?>"  id="mobile" value="<?php  if(address.mobile) { ?><%address.mobile%><?php  } ?>"/></div>
        ...
        <div class="line"><input type="text" placeholder="<?php  if($_W['uniacid'] == 18) { ?>Detailed address (excluding provinces and cities)<?php  } else { ?>详细地址(不包含省份城市区域)<?php  } ?>"  id="address" value="<?php  if(address.address) { ?><%address.address%><?php  } ?>"/></div>
...
<script id='tpl_confirm_info' type='text/html'>
    ...
    <div class="addorder_topbar">
        ...
        <div class="tips"><?php  if($_W['uniacid'] == 18) { ?>Please pay as soon as possible after confirming the order,<br>outdated orders will be automatically cancelled.<?php  } else { ?>确认订单后请尽快付款，<br>过时订单将自动取消。<?php  } ?></div>
    </div>
    <input type="hidden"  id="isverify" value="<%if isverify || isvirtual || goods.type==2%>true<%else%>false<%/if%>" />
	 ...
	 <?php  if($show==1) { ?>
        ...
        <div class="addorder_nav">
            ...
            <div class="nav <%if carrier_list.length>0%>selected<%/if%>" data-nav='1'><?php  if($_W['uniacid'] == 18) { ?>Pickup<?php  } else { ?>上门自提<?php  } ?></div>
            <%/if%>
            <%if isverifysend && carrier_send%>
            <div class="nav <%if carrier_list.length<=0%>selected<%/if%>"  data-nav='0'><?php  if($_W['uniacid'] == 18) { ?>Distribution verification<?php  } else { ?>配送核销<?php  } ?></div>
            <%/if%>
            <%if dispatchsend && carrier_send%>
            <div class="nav <%if carrier_list.length<=0 && !isverifysend%>selected<%/if%>"  data-nav='2'><?php  if($_W['uniacid'] == 18) { ?>Express delivery<?php  } else { ?>快递配送<?php  } ?></div>
            <%/if%>
        </div>
        ...
        <div class='info1'>
            <div class='inner'>
                <div class="user"><?php  if($_W['uniacid'] == 18) { ?>Recipient<?php  } else { ?>收件人<?php  } ?>：<span id='address_realname'><%address.realname%></span>(<span id='address_mobile'><%address.mobile%></span>)</div>
        ...
        <div class='info1'>
            <div class='inner'>
                <div class="user" style='padding-top:9px;'><?php  if($_W['uniacid'] == 18) { ?>Add address<?php  } else { ?>新增地址<?php  } ?></div>
            </div>
        </div>
        <div class="ico2"><i class='fa fa-angle-right fa-2x'></i></div>
        </div>
        ...
        <div class="info" id='carrier_select_send'>
            ...
            <div class='info1'>
                <div class='inner'>
                    ...
                    <div class="address"><span id='carrier_address_send'><%carrier_send.address%></span></div>
                </div>
            </div>
            <div class="ico2"><i class='fa fa-angle-right fa-2x'></i></div>
        </div>
        ...
        <%/if%>
        <%if carrier%>
              <div class="addorder_user addorder_user_1" <%if isverifysend%>style='display:none'<%/if%>>
              ...
              <div id='address_container'></div>
              <div class="carrier_input_info" <%if isverifysend%>style='display:none'<%/if%> >
                  <div class="row">
...
<!--订单区分开始-->
    ...
    <div class="addorder_good">
        ...
        <div class="text">共 <span id="goodscount"><%order.total%></span> 件商品 合计：<span style="color:#ff771b;">￥<span class='goodsprice' data-id="<%order.supplier_uid%>" style="color:#ff771b;"><%order.goodsprice%></span></span></div>
    </div>
    <?php  if(!empty($order_formInfo)) { ?>
    	 <div class="carrier_input_info" >
    ...
    <div class="addorder_price" >
        ...
        <div class="price" style="border:none;">
            ...
            <%/if%>
  		      <%if order.saleset%>
            <div class="deductenough"  data-id='<%order.supplier_uid%>' class="line" style="line-height:26px;<%if !order.saleset.showenough %>display:none<%/if%>"  >-单笔满 <d style='color:#ff6600' class="deductenough_enough" data-id='<%order.supplier_uid%>'><%saleset.enoughmoney%></d> 元立减  <span>-￥<span class="deductenough_money"  data-id='<%order.supplier_uid%>'><%saleset.enoughdeduct%></span><span></div>
            <%/if%>
				   <?php  if($hascouponplugin) { ?>
			       <div class="coupondeduct_div"  data-id='<%order.supplier_uid%>' class="line" style="line-height:26px;display:none"><d class='coupondeduct_text'></d><span>-￥<span class="coupondeduct_money">0</span><span></div>
				   <?php  } ?>
            
        </div>
    </div>
	<input type="hidden" data-id="<%order.supplier_uid%>" data-name="couponid" value="" />
     <div class="addorder_price coupondiv"  data-id="<%order.supplier_uid%>" style="margin-top:5px;<%if !order.hascoupon%>display:none<%/if%>" >
        <div class="price" style="border:none;">
            <div class="line selectcoupon" style="line-height:32px;" data-id="<%order.supplier_uid%>">
			           <d id="couponselect" data-id="<%order.supplier_uid%>">我要使用优惠券</d>
    ...
    <div class="addorder_price coupondiv"  data-id="<%order.supplier_uid%>" style="margin-top:5px;<%if order.hascoupon%>display:none<%/if%>" >
        <div class="price" style="border:none;">
            <div class="line selectcoupon1" style="line-height:32px;" data-id="<%order.supplier_uid%>">
                 <d class="couponselect" data-id="<%order.supplier_uid%>">没有符合使用条件的优惠券~</d>
                <div class="ico2" style="float:right;color:#999;margin-top:2px;"><i class='fa fa-angle-right fa-2x'></i></div>
                <div class="couponcount"><%order.couponcount%></div>
            </div>
        </div>
        ...
        <div class="price" style="border:none;">
            <%if order.deductyunbi>0 %>
              <div class="line" style="line-height:26px;"><d class="deductyunbi_info" data-id="<%order.supplier_uid%>"><%order.deductyunbi%></d> <?php  echo $yunbiset['yunbi_title'];?>可抵扣 <d class="deductyunbi_money" data-id="<%order.supplier_uid%>" style='color:#f15353'><%order.deductyunbimoney%></d> 元
                  <div class="nav deductyunbi" data-id="<%order.supplier_uid%>" on="0" credit="<%order.deductyunbi%>" money='<%order.deductyunbimoney%>'><nav></nav></div>
              </div>
            <%/if%>
        </div>
    </div>
   <%/if%>
    ...
    <%if order.deductcredit>0 || order.deductcredit2>0  %>
     <div class="addorder_price" style="margin-top:5px;">
            ...
            <div class="line" style="line-height:26px;"><d class="deductcredit_info" data-id="<%order.supplier_uid%>"><%order.deductcredit%></d> <?php  if($shopset['credit1']) { ?><?php  echo $shopset['credit1'];?><?php  } else { ?>积分<?php  } ?>可抵扣 <d class="deductcredit_money" data-id="<%order.supplier_uid%>" style='color:#ff6600'><%order.deductmoney%></d> 元
            ...
            <div class="line" style="line-height:26px;"><?php  if($shopset['credit']) { ?><?php  echo $shopset['credit'];?><?php  } else { ?>余额<?php  } ?>可抵扣 <d class="deductcredit2_money"  data-id="<%order.supplier_uid%>" style='color:#ff6600'><%order.deductcredit2%></d> 元
...
<%/each%>
    <!--这个按钮用于不需要支付商品，即可生成订单，2021/07/22-->
...
<div class="qrs">
    <!--<div id="qrcode"><img src="" width="200px" height="200px" id="imgd" alt=""></div>-->
    <div id="qrcode"></div>
    <p id="xd">请使用微信扫码，确认订单</p>
    <!-- <p style="color:red;" id="xd">首次扫码完成后，请重新登录</p> -->
    <div class="addorder_topbar" style="color: white; padding: 12px 190px; margin-top: 20px;">
                        	<div style="float: left; margin-right: 10px;"><img src="https://shop.gogo198.cn/addons/sz_yi/template/pc/default/static/images/tip.png" alt="" width="32px"></div>
                        	<div style="float: left;">阁下现正使用跨境电商购物，基于跨境电商海关监管要求，敬请扫码支付前确保已经成为我司注册会员并已经实名认证！<br/>如需客服，请致电：0757-86329911。</div>
                        </div>
</div>
</div>

<script type="text/javascript" src="../addons/sz_yi/static/js/dist/area/jquery.min.js"></script>
            ...
            $('.leftnav').click(function(){
                ...
                $('.goodsprice').html( marketprice.toFixed(2) );
                    ...
                    for (var i = 0; i < supplierarr.length; i++) {
                        discount = $('input[data-id=discount'+i+']').val();
                        discountway = $('input[data-id=discountway'+i+']').val();
                        if (discountway == 1) {
                            if( $('input[data-id=discount'+i+']').length>0){
     ...
     $('.shownum').blur(function(){
        ...
        if (json.result.carrier_list.length > 0) {
            ...
            $('#carrier_select').click(function() {
                ...
                $('#address_container').html("");
                var arrlist = $('#address_container').html();
                $('#address_container').html(arrlist + tpl('tpl_carrier_list', json.result));
                ...
                $('.carrier').click(function() {
                    ...
                    $('#carrier_select').click();//pc版选择地址后ico没有变换 因此加入
                })
            })
        }
        //地址编辑
        $(document).on("click", '.address .edit', function() {
            ...
            core.json('shop/address', {op: 'get', id: addressid}, function(getaddress_json) {
                var arrlist = $('#address_container').html();
                $('#address_container').html(arrlist + tpl('tpl_address_new', getaddress_json.result));
                //$(".chooser").height($(document.body).height());
                ...
                $('.address_sub2').click(function() {
                    ...
                    //$(".good_choose_layer").class("display", "");
                    //$('.mark_layer').show();
                    //$(".mark_layer").removeClass("hide").show();
                    //$(".address_main").removeClass("hide").show();
                    $(".mark_layer").hide();
                    $(".address_main").hide();
        ...
        $('#address_select').click(function() {
            ...
            core.json('shop/address', {}, function(addresslist_json) {
                ...
                $('.address .ico,.address .info').click(function() {
                    ...
                    //重新获取运费
                    getDispatchPrice();
                    $('#address_select').click();//pc版选择地址后ico没有变换 因此加入
                });
            }, true);
        ...
        function getDispatchPrice(zt) {
            ...
            for (var i = 0; i < supplierarr.length; i++) {
              ...
              core.json('order/confirm', {
                  ...
                  dflag: zt,
                  goods: $("input[data-name='goods'][data-id="+supplier_uid+"]").val()
              }, function(getjson) {
                  if (getjson.status == 1) {
                      if(zt){
                          $('.dispatchprice[data-id='+supplier_uid+']').html('0.00');
                      } else {
                          $('.dispatchprice[data-id='+supplier_uid+']').html(getjson.result.price);
                      }

                      if(getjson.result.deductcredit){
                          $('.deductcredit_money[data-id='+supplier_uid+']').html( getjson.result.deductmoney);
                          $('.deductcredit_info[data-id='+supplier_uid+']').html( getjson.result.deductcredit);
                          $('.deductcredit[data-id='+supplier_uid+']').attr('credit',getjson.result.deductcredit);
                          $('.deductcredit[data-id='+supplier_uid+']').attr('money',getjson.result.deductmoney);
                      }
                      if(getjson.result.deductyunbi){
                          $('.deductyunbi_money[data-id='+supplier_uid+']').html( getjson.result.deductyunbimoney);
                          $('.deductyunbi_info[data-id='+supplier_uid+']').html( getjson.result.deductyunbi);
                          $('.deductyunbi[data-id='+supplier_uid+']').attr('credit',getjson.result.deductyunbi);
                          $('.deductyunbi[data-id='+supplier_uid+']').attr('money',getjson.result.deductyunbimoney);
                      }
                      if(getjson.result.deductcredit2){
                          $('.deductcredit2_money[data-id='+supplier_uid+']').html( getjson.result.deductcredit2);
                          $('.deductcredit2[data-id='+supplier_uid+']').attr('credit2',getjson.result.deductcredit2);
                      }
                      ...
                      if(getjson.result.deductenough_money>0){
                        $('.deductenough[data-id='+supplier_uid+']').show();
                        $('.deductenough_money[data-id='+supplier_uid+']').html( getjson.result.deductenough_money);
                        $('.deductenough_enough[data-id='+supplier_uid+']').html( getjson.result.deductenough_enough);
                      } else{
                        $('.deductenough[data-id='+supplier_uid+']').hide();
                        $('.deductenough_money[data-id='+supplier_uid+']').html( '0');
                        $('.deductenough_enough[data-id='+supplier_uid+']').html( '0');
                      }
                      calctotalprice(supplier_uid);
                      if( $('.shownum').length>0){
                        var goodsinfo = $("input[data-name='goods'][data-id="+supplier_uid+"]").val().split('|')[0];
                        var goods = goodsinfo.split(',');
                        var goodsid = goods[0];
                        var optionid = goods[1];
                        var num = parseInt( $('.shownum').val());
...
<script type="text/javascript">
    ...
    $(function(){
        ...
        if( type == 'buy')
        {
            var url = "<?php  echo $this->createMobileUrl('order/confirm',array('isyunbipay'=>1,'cartids'=>$_GPC['cartids'],'id'=>$_GPC['id'],'optionid'=>$_GPC['optionid'],'total'=>$_GPC['total'],'gdid'=>$_GPC['gdid'],'openid'=>$openid))?>";

            
        }else{
            var url = "<?php  echo $this->createMobileUrl('order/cart',array('id'=>$_GPC['id'],'optionid'=>$_GPC['optionid'],'total'=>$_GPC['total'],'gdid'=>$_GPC['gdid'],'cartids'=>$_GPC['cartids'],'openid'=>$openid))?>";
        }
