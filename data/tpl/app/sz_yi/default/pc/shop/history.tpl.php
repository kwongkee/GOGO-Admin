...
.history_main {height:auto; background:#fff; border-bottom:1px solid #e3e3e3;overflow: hidden;}
.history_good {height:100px;  padding:10px 0px; border-bottom:1px solid #e3e3e3;overflow: hidden;}
.history_good .ico {margin-left:10px;height:20px; width:30px;  float:left ; font-size:24px;margin-top:22px;color:#666;z-index:2;position: relative;}
.history_good .img {height:80px; width:80px; background:#f99; float:left;z-index:2;position: relative; /*margin-top:-10px;*/}
...
.history_good .info .inner .other {height:30px; width:100%;}
...
<script id='history_empty' type='text/html'>
     <div class="history_no"><i class="fa fa-history" style="font-size:100px;"></i><br><span style="line-height:18px; font-size:16px;"><?php  if($_W['uniacid'] == 18) { ?>You have not viewed any products<?php  } else { ?>您没有浏览过任何商品<?php  } ?></span><br><?php  if($_W['uniacid'] == 18) { ?>Master, go find me something<?php  } else { ?>主人快去给我找点东西吧<?php  } ?></div>
<div class="history_no_menu">
   <div class="history_no_nav" onclick="location.href='<?php  echo $this->createMobileUrl('member')?>'"><?php  if($_W['uniacid'] == 18) { ?>Membership Zone<?php  } else { ?>个人中心<?php  } ?></div>
        <div class="history_no_nav"  onclick="location.href='<?php  echo $this->getUrl()?>'"><?php  if($_W['uniacid'] == 18) { ?>Go shopping<?php  } else { ?>去逛逛<?php  } ?></div>
 </div>
</script>
<script id="history_main" type="text/html">
      <div class="history_top">
        <div class="title" onclick='history.back()'><!--<i class='fa fa-chevron-left'></i>--> <?php  if($_W['uniacid'] == 18) { ?>My footprint<?php  } else { ?>我的足迹<?php  } ?>(<%total%>)</div>
        <div class="nav" id="removehistory"><?php  if($_W['uniacid'] == 18) { ?>Delete<?php  } else { ?>删除<?php  } ?></div>
    </div>
        ...
        function bindEvents(){
                     ...
                     function removeHistory(ids){
                        if(ids.length<=0){
                              ...
                              core.tip.confirm('<?php  if($_W['uniacid'] == 18) { ?>Confirm to delete these items from my collection<?php  } else { ?>确认从我的足迹删除这些商品<?php  } ?>?',function(){
                      ...
                      $(window).scroll(function(){ 
                          if(loaded){
                            ...
                            if($(document).height() <= totalheight){ 
                                ...
                                if(stop==true){ 
                                    stop=false; 
                                    $('.history_main').append('<div id="history_loading"><i class="fa fa-spinner fa-spin"></i> <?php  if($_W['uniacid'] == 18) { ?>Loading<?php  } else { ?>正在加载<?php  } ?>...</div>');
                                    page++;
                                    core.json('shop/history', {page:page}, function(morejson) {  
                                        ...
                                        if (morejson.result.list.length <morejson.result.pagesize) {
                                            ...
                                            $('.history_main').append('<div id="history_loading"><?php  if($_W['uniacid'] == 18) { ?>All collections have been loaded<?php  } else { ?>已经加载完全部足迹<?php  } ?></div>');
                                            loaded = true;
                                            $(window).scroll = null;
                                            return;
                                        }
