...
<script id='favorite_empty' type='text/html'>
     <div class="favorite_no"><i class="fa fa-favorite" style="font-size:100px;"></i><br><span style="line-height:18px; font-size:16px;"><?php  if($_W['uniacid'] == 18) { ?>You have not viewed any products<?php  } else { ?>您没有浏览过任何商品<?php  } ?></span><br><?php  if($_W['uniacid'] == 18) { ?>Master, go find me something<?php  } else { ?>主人快去给我找点东西吧<?php  } ?></div>
<div class="favorite_no_menu">
   <div class="favorite_no_nav" onclick="location.href='<?php  echo $this->createMobileUrl('member')?>'"><?php  if($_W['uniacid'] == 18) { ?>Membership Zone<?php  } else { ?>个人中心<?php  } ?></div>
        <div class="favorite_no_nav"  onclick="location.href='<?php  echo $this->getUrl()?>'"><?php  if($_W['uniacid'] == 18) { ?>Go shopping<?php  } else { ?>去逛逛<?php  } ?></div>
 </div>
</script>
<script id="favorite_main" type="text/html">
      <div class="favorite_top">
        <div class="title" onclick='history.back()'> <?php  if($_W['uniacid'] == 18) { ?>My collection<?php  } else { ?>我的收藏<?php  } ?>(<%total%>)</div>
        <div class="nav" id="removefavorite"><?php  if($_W['uniacid'] == 18) { ?>Delete<?php  } else { ?>删除<?php  } ?></div>
    </div>
        ...
        function bindEvents(){
                     ...
                     function removeFavorite(ids){
                        if(ids.length<=0){
                              ...
                              core.tip.confirm('<?php  if($_W['uniacid'] == 18) { ?>Confirm to delete these items from my collection<?php  } else { ?>确认从我的收藏删除这些商品<?php  } ?>?',function(){
                      ...
                      $(window).scroll(function(){ 
                          if(loaded){
                            ...
                            if($(document).height() <= totalheight){ 
                                ...
                                if(stop==true){ 
                                    stop=false; 
                                    $('.favorite_main').append('<div id="favorite_loading"><i class="fa fa-spinner fa-spin"></i> <?php  if($_W['uniacid'] == 18) { ?>Loading<?php  } else { ?>正在加载<?php  } ?>...</div>');
                                    page++;
                                    core.json('shop/favorite', {page:page}, function(morejson) {  
                                        ...
                                        if (morejson.result.list.length <morejson.result.pagesize) {
                                            ...
                                            $('.favorite_main').append('<div id="favorite_loading"><?php  if($_W['uniacid'] == 18) { ?>All collections have been loaded<?php  } else { ?>已经加载完全部收藏<?php  } ?></div>');
                                            loaded = true;
                                            $(window).scroll = null;
                                            return;
                                        }
