    ...
    <div class="good_choose">
        <div class="info">
             ...
             <div class="right">
                   ...
                   <div class="stock"><?php  if($_W['uniacid'] == 18) { ?>in stock<?php  } else { ?>库存<?php  } ?>:<span id='option_stock'><%goods.total%></span>件</span> </div>
                   <div class="option"><?php  if($_W['uniacid'] == 18) { ?>Choose specifications<?php  } else { ?>请选择规格<?php  } ?></div>
             </div>
        ...
        <div class="sub <%if specs.length>0%>disabled<%/if%>" onClick="choose2()"><?php  if($_W['uniacid'] == 18) { ?>Confirm<?php  } else { ?>确认<?php  } ?></div>
    </div>
 </script>   
 <script id='cart_empty' type='text/html'>
     <!--<div class="cart_top">
    <div class="title" onclick='history.back()'><i class='fa fa-chevron-left'></i> <?php  if($_W['uniacid'] == 18) { ?>cart<?php  } else { ?>购物车(0)<?php  } ?></div>
</div>-->
 <div class="card_no"><i class="fa fa-shopping-cart" style="font-size:100px;"></i><br><span style="line-height:18px; font-size:16px;"><?php  if($_W['uniacid'] == 18) { ?>The shopping cart is almost flat<?php  } else { ?>购物车快饿瘪了<?php  } ?></span><br><?php  if($_W['uniacid'] == 18) { ?>Master, go find me something<?php  } else { ?>主人快去给我找点东西吧<?php  } ?></div>
<div class="card_no_menu">
   	<div class="card_no_nav" onclick="location.href='<?php  echo $this->createMobileUrl('member')?>'"><?php  if($_W['uniacid'] == 18) { ?>Membership Zone<?php  } else { ?>个人中心<?php  } ?></div>
        <div class="card_no_nav"  onclick="location.href='<?php  echo $this->getUrl()?>'"><?php  if($_W['uniacid'] == 18) { ?>Go shopping<?php  } else { ?>去逛逛<?php  } ?></div>
 </div>
...
<div class="cart_top">
    <div class="title" onclick='history.back()'><i class='fa fa-chevron-left'></i> <?php  if($_W['uniacid'] == 18) { ?>cart<?php  } else { ?>购物车<?php  } ?>(<%total%>)</div>
        <div class="nav" id='tofavorite'><?php  if($_W['uniacid'] == 18) { ?>Move to favorites<?php  } else { ?>移至收藏夹<?php  } ?></div>
        <div class="nav" id='removecart'><?php  if($_W['uniacid'] == 18) { ?>Delete<?php  } else { ?>删除<?php  } ?></div>
</div>
    ...
    <div class="cart_main">
                     ...
                     <div class='inner' >
            	      <div class="name" onclick="location.href='<?php  echo $this->createMobileUrl('shop/detail')?>&id=<%value.goodsid%>'"><%value.title%></div>
                        <%if value.optionid!='0'%>
                         <div class="optionsel">
                            <div class='content'>
                                <div class="cinner"><%if optiontitle!=''%><%value.optiontitle%><%else%><?php  if($_W['uniacid'] == 18) { ?>Unselected specifications<?php  } else { ?>未选规格<?php  } ?><%/if%></div>
                            </div>
        ...
        <%/each%>
        <%/each%>
        <div class="cart_count"><?php  if($_W['uniacid'] == 18) { ?>Total<?php  } else { ?>共<?php  } ?> <span class='total'><%total%></span> <?php  if($_W['uniacid'] == 18) { ?>items<?php  } else { ?>件商品<?php  } ?> <?php  if($_W['uniacid'] == 18) { ?>Total<?php  } else { ?>合计<?php  } ?>: ￥<span class='totalprice'><%totalprice%></span></div>
    </div>
    <div class="cart_pay">
        <div class="all" sel="1"><i class="fa fa-check-circle-o" style="color:#0C9;"></i> <span class='t'><?php  if($_W['uniacid'] == 18) { ?>All<?php  } else { ?>全选<?php  } ?></span></div>
        <div class="paysub <%if total<=0%>disabled<%/if%>"><?php  if($_W['uniacid'] == 18) { ?>Settlement<?php  } else { ?>结算<?php  } ?>(<span class='total'><%total%></span>)</div>
        <div class="text"><?php  if($_W['uniacid'] == 18) { ?>Total<?php  } else { ?>合计<?php  } ?>：<span>￥<span  class='totalprice'><%totalprice%></span></span><br><span style="color:#313131; font-size:14px;"><?php  if($_W['uniacid'] == 18) { ?>Not included shipping<?php  } else { ?>不含运费<?php  } ?></span></div>
    </div>
</script>
<script language="javascript">
    ...
    require(['tpl', 'core'], function(tpl, core) {
        ...
        core.json('shop/cart',{},function(json){
          ...
          $('.good_choose .other').css('max-height',bh-175);
           ...
           $('.shownum').blur(function(){
               ...
               if(num>maxbuy && maxbuy>0){
                    num=maxbuy;
                    core.tip.show("<?php  if($_W['uniacid'] == 18) { ?>You buy most<?php  } else { ?>您最多购买 <?php  } ?> " + maxbuy + "<?php  if($_W['uniacid'] == 18) { ?>pieces<?php  } else { ?>件<?php  } ?>");
                }
                else if(stock!='-1' && stock!='' && num>stock){
                    num=stock;
                    core.tip.show("<?php  if($_W['uniacid'] == 18) { ?>You buy most<?php  } else { ?>您最多购买 <?php  } ?> " + stock + "<?php  if($_W['uniacid'] == 18) { ?>pieces<?php  } else { ?>件<?php  } ?>");
                }
                input.val(num);
			...
			var productprice = 0;
				...
				for(var i=0;i<len;i++) {
					...
					if(stock==0){
					   $('.sub').addClass('disabled').html('<?php  if($_W['uniacid'] == 18) { ?>Insufficient inventory to purchase<?php  } else { ?>库存不足,无法购买<?php  } ?>');
					}
					else{
					     $('.sub').removeClass('disabled').html('<?php  if($_W['uniacid'] == 18) { ?>Confirm<?php  } else { ?>确认<?php  } ?>');
					}
	      ...
	      var titles='<?php  if($_W['uniacid'] == 18) { ?>selected<?php  } else { ?>已选<?php  } ?>: ';
         ...
         if(thumb!=''){
			...
			if(ret.no==''){
			   ...
			   $("#optionid").val(optionid); 
				...
				if(stock!="-1"){
				      $("#stockcontainer").html("<?php  if($_W['uniacid'] == 18) { ?>in stock<?php  } else { ?>库存<?php  } ?>:<span id='stock'>" + stock + "</span>");
				}
				else{
				...
				if(ret.no==''){
		     	...
		     	$("#option_stock").html(stock);	
                        ...
                        $("#tofavorite").click(function(){
                            ...
                            if(ids.length<=0){
                                core.tip.show('<?php  if($_W['uniacid'] == 18) { ?>No product selected<?php  } else { ?>未选择商品<?php  } ?>');
                                return;
                            }
                           core.tip.confirm('<?php  if($_W['uniacid'] == 18) { ?>Confirm to move these items to favorites<?php  } else { ?>确认将这些商品移至收藏夹<?php  } ?>?',function(){
                              $('.cart_good').attr('del',0);
                               core.json('shop/cart',{'op':'tofavorite',ids:ids},function(json){
                                    if(json.status==1)  {
                                        ...
                                        core.tip.show('<?php  if($_W['uniacid'] == 18) { ?>Move failed<?php  } else { ?>移动失败<?php  } ?>');
                                    }
                               },true,true);
                           });
                        })
                        ...
                        $('#removecart').click(function(){
                           ...
                           core.tip.confirm('<?php  if($_W['uniacid'] == 18) { ?>Confirm to move these items to favorites<?php  } else { ?>确认从购物车删除这些商品<?php  } ?>?',function(){
                               $('.cart_good').attr('del',0);
