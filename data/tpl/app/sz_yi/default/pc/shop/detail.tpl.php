...
.good_info1 {height:auto; width:100%; width:810px !important; padding-left:30px;/* border-bottom:1px solid #e2e2e2;*/}
...
.good_shop {height:157px; width:100%; padding:0 8px; background:#fff; padding:0px 1%; margin-top:-1px; margin-bottom:10px; border:1px solid #ededed;}
...
/*详情标题改*/
.good_info2 .menu .nav{height:37px; width:20%;  float:left; font-size:14px; color:#666; text-align:center; line-height:37px;cursor: pointer;}
...
.goods-side-slides-nav .prev, .goods-side-slides-nav .next { width: 50%; height: 25px; display: inline-block; *display: inline; *zoom: 1; cursor: pointer; }
/* background: url("../images/hdside-nav.gif") no-repeat 50% 0*/
...
/*新增css（商品提供商）*/
.provider{margin:0; padding:0; border-bottom:1px solid #e8e8e8; height: auto; float: left; width: 100%;}
.goods_provider{height: 45px; width: 90px; line-height: 45px; float: left; font-size: 14px; text-align: left; margin-left: 10px;} 
.company_provider button{font-size: 14px; width: 75%; line-height: 45px; color: #999; border: 0; background: #fff; text-align: left;}
#popBox{position: absolute;display:none;width:700px;height:auto;min-height: 300px; left:28%;top:5%;z-index:11; background: #fff; border-radius: 3px;}
#popLayer{position: absolute;display:none;left:0;top:0;z-index:10;background:#000;-moz-opacity: 0.6;opacity:.60;filter: alpha(opacity=60);/* 只支持IE6、7、8、9 */}
.popcon{font-size: 14px; color: #666; line-height: 24px; width: 95%; margin: 10px auto;}
.popcon p{padding: 10px 0; border-bottom: 1px solid #e8e8e8;}
.closed{text-align: right; width: 100%; border-bottom: 1px solid #ccc; background: #eee; border-top-left-radius:3px; border-top-right-radius:3px;}
.closed a{color: #666; line-height: 40px; margin-right: 10px;}
.closed span{position: absolute;left: 10px;line-height: 40px;}

/*消费权益保障*/
.l_nav1 {height: 40px;padding-top: 8px; width: 98%; margin: 15px 1% 0;}
.l_nav1 div{text-decoration: none;padding: 7px 14px 5px; float: left; font-size: 16px; font-weight: bold; margin-left: 35px; margin-right: 30px; background: #ddd;
...
color: #1a5486; letter-spacing: 1px; text-transform: uppercase; font-weight: bold; text-align: center; text-shadow: 1px 1px 1px rgba(255,255,255,0.3);
border-radius: 3px /*3px 0 0*/; box-shadow: 2px 0 2px rgba(0,0,0,0.1), -2px 0 2px rgba(0,0,0,0.1);}
.l_nav1 .l_nav1_no1 {color: #1a5486;text-decoration: none;background: #fff; border: 1px solid #ccc; border-bottom: 8px solid #fff; box-shadow:none;}
.l_nav1 img{width:40px; height: 40px;}
.l_nav1 p{width: 100%;}
.l_no2 {margin:50px 1% 0; padding:10px; background: #fff; width: 98%; z-index: 5; /*box-shadow: 0 -2px 3px -2px rgba(0,0,0,0.2), 0 2px 2px rgba(0,0,0,0.1); border-radius: 0 3px 3px 3px;*/ border: 1px solid #ccc;}

/*购买须知及确认*/
.alert-modal{left:0;top:0;width:100%;height:100%;opacity: 0.6;filter: alpha(opacity=60);}
.alert-container{display: inline-block;border-radius: 5px;-webkit-border-radius: 5px;-moz-border-radius: 5px;-o-border-radius: 5px;background: #fff;border:1px solid #f1f1f1;z-index: 99999;overflow: hidden;}
.alert-outside{position: fixed; left: 50%; top: 50%; z-index: 1011; width: 500px; height: auto; margin-left: -225px; margin-top: -316px; display: none;}
.alert-title{font-size: 16px;text-align: center;line-height: 42px;color:#fff;background: #c30000;position: absolute;left:0;top:0;z-index: 999;width:100%;text-align: center; border-bottom:1px solid #ccc;}
.alert-content{font-size: 14px;padding:0px 20px;color:#555;height:100%;overflow: auto; margin-top: 50px;}
.alert-content-scrollbar{}
.alert-btn-box{border-top:1px solid #ccc;text-align: right;position: absolute;left:0;background: #fff;bottom: 0;z-index: 999;width:100%;}
.alert-btn-p{display: inline-block;text-align: center;border-right:1px solid #f1f1f1;font-size:14px;line-height: 35px;cursor: pointer;color:#fff;}
.btn_blue{background: #1b7ab6;}
.btn_red{background: #ff4a4e;}
...
.pcAlert .alert-btn-p{border:1px solid #f1f1f1;border-radius: 3px;-webkit-border-radius: 3px;-moz-border-radius: 3px;-o-border-radius: 3px;box-sizing: border-box;-webkit-box-sizing: border-box;
-moz-box-sizing: border-box;-o-box-sizing: border-box;line-height: 30px;margin:7px 0;padding:0 10px;margin-right:10px;}
...
.protocol-title{text-align: center; font-size: 18px; padding: 10px 0 20px; font-weight: bold;}
.protocol{line-height: 24px;}
...
<script async src="../addons/sz_yi/template/mobile/default/static/js/star-rating.js" type="text/javascript"></script>
<script async src="../addons/sz_yi/static/js/dist/ajaxfileupload.js" type="text/javascript"></script>
<script async  type="text/javascript" src="../addons/sz_yi/static/js/dist/area/cascade.js"></script>
...
<div id="popLayer" ></div>
<div id="popBox" >
	<div class="closed"><span><?php  if($_W['uniacid'] == 18) { ?>E-commerce operator information<?php  } else { ?>电商经营者信息<?php  } ?></span><a href="javascript:void(0)" onclick="closeBox()">关闭</a></div>
	<div class="popcon">
		<p><!--<img src='../addons/sz_yi/static/images/card01.png'  />--></p>
		<p style="border: none;"><?php  if($_W['uniacid'] == 18) { ?>company profile<?php  } else { ?>企业简介<?php  } ?>：</p>
	</div>
</div>

<!--购买须知及确认-->
<input type="hidden" id="params" value="" />
<div class="alert-modal" id="alert-modal" style="position: fixed; z-index: 0; display: none;"></div>
<div class="alert-container alert-outside" id="nu1">
	<div class="alert-title"><?php  if($_W['uniacid'] == 18) { ?>Notice for quality differences<?php  } else { ?>质量存在差异须知<?php  } ?></div>
	<div class="alert-content" style="text-align: left; padding-bottom: 46px;">
		<div class="protocol">	    
			<p>	    </p>
			<h4 style="padding: 8% 0; line-height: 25px;"><?php  if($_W['uniacid'] == 18) { ?>The cross-border import and export commodities you purchased from Gogo.com comply with the standards or technical specifications of the place of origin on quality, safety, hygiene, environmental protection, and labeling, and may be different from the relevant standards in mainland China, which may arise during use. The harm, loss or other risks incurred by you will be borne by you.<?php  } else { ?>您购买的购购网跨境进口商品符合原产地关于质量、安全、卫生、环保、标识等标准或技术规范要求，可能与我国大陆地区相关标准有所不同，在使用过程中由此可能产生的危害、损失或者其他风险，将由您自行承担。<?php  } ?></h4>	
			
		</div>
	</div>
	<div class="alert-btn-box">
		
		<p class="alert-btn-p btn_blue" style="width: 246px;" onclick="agreement(1)"><?php  if($_W['uniacid'] == 18) { ?>Agree<?php  } else { ?>同意<?php  } ?></p>
		<p class="alert-btn-p btn_red" style="width: 247px;" onclick="closese(1)"><?php  if($_W['uniacid'] == 18) { ?>Disagree<?php  } else { ?>不同意<?php  } ?></p> 
	</div>
</div>

<div class="alert-container alert-outside" id="nu2">
	<div class="alert-title"><?php  if($_W['uniacid'] == 18) { ?>No Chinese label notice<?php  } else { ?>没有中文标签须知<?php  } ?></div>
	<div class="alert-content" style="text-align: left; padding-bottom: 46px;">
		<div class="protocol">	     
			<p>	    </p>
			<h4 style="padding: 8% 0; line-height: 25px;"><?php  if($_W['uniacid'] == 18) { ?>Your purchase of goods on the GOGO shopping network (www.gogo198.cn) is regarded as your purchase at home or abroad, so the goods themselves may not have Chinese labels and instructions. You can view the Chinese label and description through the product details page. The Chinese label and description are directly translated from English. If you have any questions about the translated Chinese label, you can contact the merchant for confirmation.<?php  } else { ?>您在GOGO购购网（www.gogo198.cn）购买商品视同您于境外购买，故商品本身可能无中文标签及说明书。您可以通过商品详情页查看中文标签及说明，中文标签及说明是由英文直接翻译，若您对翻译后的中文标签有疑问，可联系商家确认。<?php  } ?></h4>	
		</div>
	</div>
	<div class="alert-btn-box">
		<p class="alert-btn-p btn_blue" style="width: 246px;" onclick="agreement(2)"><?php  if($_W['uniacid'] == 18) { ?>Agree<?php  } else { ?>同意<?php  } ?></p>
		<p class="alert-btn-p btn_red" style="width: 247px;" onclick="closese(2)"><?php  if($_W['uniacid'] == 18) { ?>Disagree<?php  } else { ?>不同意<?php  } ?></p> 
	</div>
</div>

<div class="alert-container alert-outside" id="nu3">
	<div class="alert-title"><?php  if($_W['uniacid'] == 18) { ?>Notice on prohibiting resale of goods<?php  } else { ?>商品禁止转售须知<?php  } ?></div>
	<div class="alert-content" style="text-align: left; padding-bottom: 46px;">
		<div class="protocol">	   
			<p>	    </p>
			<h4 style="padding: 8% 0; line-height: 25px;"><?php  if($_W['uniacid'] == 18) { ?>In accordance with relevant laws and policies, the cross-border import and export commodities you purchase on Gogo.com are for personal use only and cannot be resold.<?php  } else { ?>根据相关法律政策，您选购的购购网跨境进口商品仅限于个人自用，不得进行再次销售。<?php  } ?></h4>	
		</div>
	</div>
	<div class="alert-btn-box">
		<p class="alert-btn-p btn_blue" style="width: 246px;" onclick="agreement(3)"><?php  if($_W['uniacid'] == 18) { ?>Agree<?php  } else { ?>同意<?php  } ?></p>
		<p class="alert-btn-p btn_red" style="width: 247px;" onclick="closese(3)"><?php  if($_W['uniacid'] == 18) { ?>Disagree<?php  } else { ?>不同意<?php  } ?></p> 
	</div>
</div>
...
<div class="crumb" style=" line-height:50px;">
    <span class="t"><?php  if($_W['uniacid'] == 18) { ?>Location<?php  } else { ?>您的位置<?php  } ?>：</span>
    <a class="c" href="<?php  echo $this->createMobileUrl('shop')?>"><?php  if($_W['uniacid'] == 18) { ?>Home<?php  } else { ?>首页<?php  } ?></a>
    <?php  if($pcate) { ?>
...
<div class="detail_lc">
    <div class="good_img" style="height: 389px !important;width: 389px !important;" >
        <%if pics.length>1%>
        ...
        <div class="main_image">
            <ul>
                <%each pics as p %>
                <li> <img  rel="preload" src='<%p%>' onerror="javascript:this.src='/attachment/images/3/default.png'"/></li>
                <%/each%>
            </ul>
        </div>
    </div>
    <div class="good_info1">
        ...
        <%/if%>
            ...
            <!--div class="price">購購會員价：￥<d id='marketprice'-->
            <div class="price"><?php  if($_W['uniacid'] == 18) { ?>Price: <?php  } else { ?><?php  echo $_W['account']['name'];?>會員价：<?php  } ?>￥<d id='marketprice'>
            ...
            <span style='float:right'><d id="stockcontainer"><?php  if($_W['uniacid'] == 18) { ?>In stock<?php  } else { ?>库存<?php  } ?>:<d id="stock"><%goods.total%></d></d> 
<?php  if($_W['uniacid'] == 18) { ?>Sales<?php  } else { ?>销量<?php  } ?>:<%goods.sales%><%goods.unit%> <?php  if($_W['uniacid'] == 18) { ?>Browse<?php  } else { ?>浏览<?php  } ?>:<%goods.viewcount%></d></span>
                    <input type="hidden" id="marketpricess" value="<%goods.marketprice%>">
                    <select id="seewaibi" onchange="seewaibi(this)" style="border-radius: 5px;border: 1px solid #1b7ab6;font-size: 14px;line-height: 34px;color:#333;">
                        <option value="">查看外币</option>
  ...
  <!--  <div class="good_choose_layer"></div>-->
        ...
        <div class="other">
            ...
            <!--<span>diyform</span>-->
             ...
             <style type='text/css'>
				...
				<!--商品提供商-->
				<div class="provider">
					<div class="goods_provider"><?php  if($_W['uniacid'] == 18) { ?>Supplier<?php  } else { ?>电商经营者<?php  } ?></div>
            		<div class="company_provider">
                        <?php  if($_GPC['id'] == '79107') { ?>
                        
                        <button id="Button1" onclick="popBox()">佛山市信念行精工科技有限公司</button>
                        <?php  } else { ?>
                        <button id="Button1" onclick="popBox()"><?php  echo $merchant['username'];?></button>
                        <?php  } ?>
                ...
                <div class='number'>
                  <div class='label'><?php  if($_W['uniacid'] == 18) { ?>Number<?php  } else { ?>购买数量<?php  } ?></div>
                  <div class='num'>
    ...
    <%if goods.canbuy && goods.total!=0 && goods.timebuy=='0' && goods.userbuy=='1' && goods.groupbuy=='1'  && goods.levelbuy=='1'%>
        <div class="good_bottom">
                ...
                <%if goods.canaddcart%><div class="add" onclick="cart('cart')"><?php  if($_W['uniacid'] == 18) { ?>Add to cart<?php  } else { ?>加入购物车<?php  } ?></div><%/if%>
                <div class="buy" onclick="cart('buy')" <%if !goods.canaddcart%>style='width:50%'<%/if%>><?php  if($_W['uniacid'] == 18) { ?>Buy now<?php  } else { ?>立即购买<?php  } ?></div>
				
				<div class="like" >
                <i class="fa <%if isfavorite%>fa-star<%else%>fa-star-o<%/if%>" <%if isfavorite==1%>style="color:#f90"<%/if%>></i>
                <br><span><?php  if($_W['uniacid'] == 18) { ?>Favorites<?php  } else { ?>收藏<?php  } ?></span>
            </div>
...
<div class="detail_lcol"> 
	...
	<div class="detailshop_hd">
	 <?php  if($_W['uniacid'] == 18) { ?>Store Information<?php  } else { ?>店铺信息<?php  } ?>
	</div>
	
	<div class="good_shop">
        <div class="shop1" onclick="location.href='<%detail.btnurl2%>'">
        <img src='<%detail.logo%>'/>
        <div class="shop_info"><%detail.shopname%><br><span><%if detail.totaltitle!=''%><%detail.totaltitle%><%else%><?php  if($_W['uniacid'] == 18) { ?>All Products<?php  } else { ?>全部宝贝<?php  } ?> <%goodscount%> <?php  if($_W['uniacid'] == 18) { ?> <?php  } else { ?>个 <?php  } ?><%/if%></span></div></div>
        <div class="shop2">
            <div class="sub1" onclick="location.href='<%detail.btnurl1%>'"><i class="fa fa-list"></i> <%if detail.btntext1!=''%><%detail.btntext1%><%else%><?php  if($_W['uniacid'] == 18) { ?>Products<?php  } else { ?>全部商品 <?php  } ?><%/if%></div>
            <div class="sub2" onclick="location.href='<%detail.btnurl2%>'"> <i class="fa fa-gift" o></i> <%if detail.btntext2!=''%><%detail.btntext2%><%else%><?php  if($_W['uniacid'] == 18) { ?>Store<?php  } else { ?>进店逛逛<?php  } ?><%/if%></div>
        </div>
    </div>
	
	<div class="detail_hd">
	 <?php  if($_W['uniacid'] == 18) { ?>Categories<?php  } else { ?>商品分类<?php  } ?>
	</div>
...
<dl>
        <dd class="level1"><a class="category_item" href="<?php  echo $this->createMobileUrl('shop/list')?>" style="background-color:#fff;color:#333;"><?php  if($_W['uniacid'] == 18) { ?>All Products<?php  } else { ?>全部商品<?php  } ?></a></dd>
    </dl>
...
<%each category as value%>
	...
	<div class="detail_hd">
	 <?php  if($_W['uniacid'] == 18) { ?>Best Sellers<?php  } else { ?>热销商品<?php  } ?>
	</div>
		...
		<li>
			<a href="<?php  echo $this->createMobileUrl('shop/detail')?>&id=<%item.id%>">									
				...
				<div class="item_price clearfix">
					<span class="price">¥<%item.marketprice%></span> <span class="sales"><?php  if($_W['uniacid'] == 18) { ?>Sell<?php  } else { ?>已售<?php  } ?>:<em><%item.sales%></em></span>
				</div>
...
<div class="detail_cc">       
    <div class="good_info2">
        <div class="menu">
            <div id="nav_1" class="nav navon" onClick="tab(1)"><?php  if($_W['uniacid'] == 18) { ?>Product details<?php  } else { ?>图文详情<?php  } ?></div>
            <div id="nav_2" class="nav" onClick="tab(2)"><?php  if($_W['uniacid'] == 18) { ?>Specifications<?php  } else { ?>产品参数<?php  } ?></div>
            <div id="nav_5" class="nav" onClick="tab(5)" ><?php  if($_W['uniacid'] == 18) { ?>Rights Protection<?php  } else { ?>权益保障<?php  } ?></div>
            <div id="nav_3" class="nav" onClick="tab(3)"><?php  if($_W['uniacid'] == 18) { ?>Comment<?php  } else { ?>用户评价<?php  } ?></div>
            <div id="nav_4" class="nav" onClick="tab(4)"style="border-right:0px; width:20%"><?php  if($_W['uniacid'] == 18) { ?>Recommend<?php  } else { ?>同店推荐<?php  } ?></div>
        ...
        <div class="tab_con">
            ...
            <div class="con" id="con_5">

			<!--消费权益保障-->
				<?php  if($_W['uniacid'] == 18) { ?>
				<div id="nav9" class="l_nav1">None</div>
				<?php  } else { ?>
            	<div id="nav9" class="l_nav1">
				    <div onclick="tabSwitch(this,1)" class="l_nav1_no1">
				    	<p><img src="../addons/sz_yi/static/images/icon_01.png"></p>
            			<p style="font-weight:bold; margin-top:5px;">商品退换机制</p>
            		</div>
				    <div onclick="tabSwitch(this,2)">
				    	<p><img src="../addons/sz_yi/static/images/icon_02.png"></p>
            			<p style="font-weight:bold; margin-top:5px;">商品召回机制</p>
				    </div>
				    <div onclick="tabSwitch(this,3)">
				    	<p><img src="../addons/sz_yi/static/images/icon_03.png"></p>
            			<p style="font-weight:bold; margin-top:5px;">商品赔付机制</p>
				    </div>
				    <div onclick="tabSwitch(this,4)">
				    	<p><img src="../addons/sz_yi/static/images/icon_04.png"></p>
            			<p style="font-weight:bold; margin-top:5px;">商品安全机制</p>
				    </div>
    			</div>
			    <div class="l_no2">
    ...
    function seewaibi(obj) {
        ...
        if( value == '')
        {
            ...
            require(['core'],function(core){
                core.json('shop/huilv',{ to:value,price:$("#marketpricess").val()},function(json){
                    console.log(json)
                    if(json.status == 1)
                    {
                        $("#waibi").text(json.result.to + ' ' + json.result.camount);
                    }else{
                    ...
                    page = 1;
                     core.json('shop/util',{op:'comment', page:page,goodsid:'<?php  echo $_GPC['id'];?>'},function(json){
                         if(json.result.list.length<=0){
                             $('#comment_container').html(<?php  if($_W['uniacid'] == 18) { ?>'No comment'<?php  } else { ?>'暂时没有任何评价'<?php  } ?>);
                             return;
                         }
            ...
            if(direct=='cart'){
                ...
                <?php  if(!empty($formInfo)) { ?>
                    ...
                    require(['core'],function(core){
                        core.json('shop/detail', data, function(create_json) {
                            if (create_json.status == 1) {
                                ...
                                window.location.href = "<?php  echo $this->createMobileUrl('order/confirm')?>&id=<?php  echo $_GPC['id'];?>&optionid=" +$("#optionid").val() + "&total=" + $("#total").val()+ "&gdid="+create_json.result.goods_data_id+"&type=buy";
                            } else {
                                core.tip.show("提交失败!")
                            }
                    ...
                    <?php  } else { ?>
                        ...
                        window.location.href = "<?php  echo $this->createMobileUrl('order/confirm')?>&id=<?php  echo $_GPC['id'];?>&optionid=" +$("#optionid").val() + "&total=" + $("#total").val();
                <?php  } ?>
...
function reduceNum(){
    ...
    function tabSwitch(_this,num) {
      ...
      var number = tag.getElementsByTagName("div");   //获取导航栏元素个数(getElementsByTagName是返回元素素组)
    ...
    function cart(param) {
     ...
     function agreement(id) {
     	...
     	switch(id){
     		...
     		case 3:
     			$("#nu3").hide();
     			$("#alert-modal").css('z-index','0');
        		$("#alert-modal").css('background','none');
     		...
     		break;
     		default:
     			$("#alert-modal").css('z-index','1011');
     			$("#alert-modal").css('background','#000');
