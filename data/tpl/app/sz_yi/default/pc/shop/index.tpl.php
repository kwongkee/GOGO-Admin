...
<div class="cover-page-index fl wfs">
  <div class="cover-page-wrapper"> <?php  if(is_array($index_name)) { foreach($index_name as $key => $val) { ?>
    <?php  if($this->yzShopSet['index']!=""&&in_array($key, $this->yzShopSet['index'])) { ?>
    <?php  if($ads_pc[$key]) { ?>
...
<div class="tab_goods_warp" style="margin-bottom:20px;">
  <div class="tab_left">
    <div class="tab_title"><span><%value1.name%></span></div>
    <a href=""><img src="https://shop.gogo198.cn/attachment/<%value1.advimg_pc%>"/></a>
  </div>

  <div class="tab_goods">
    <ul class="tab_menu">        
      <a class="more" href="<?php  echo $this->createMobileUrl('shop/list')?>&pcate=<%value.id%>"><%if value.uniacid==18%>More<%else%>更多<%/if%> <span>></span></a>
    </ul>
    
    <div class="tab_box<%value.id%>">
        <div class="goods_tab_list" style="display:block;">
            <ul class="items" style="margin:0">
    <%each value1.ch_goods as g%>
              <li class="items-li"> <a target="_blank" href="<?php  echo $this->createMobileUrl('shop/detail')?>&id=<%g.id%>" style="position:relative;"><img src="<%g.thumb%>" title=""></a>
                ...
                <p class="price-attente"> <span class="price">￥<%g.marketprice%>元</span> <span class="attente" title="销量"> <%if g.uniacid==18%>Sale<%else%>销量<%/if%>
                :<%g.sales%></span> </p>
              </li>
...
<script>
    ...
    // 商品动画
      var move=function(wrap,cname){
        ...
        //   $.ajax({
        //     url:"https://shop.gogo198.cn/app/index.php?i=3&c=entry&do=shop&m=sz_yi&p=&op=category_recommend",   //去请求项目二中的url
        //     dataType:"jsonp",
        //     jsonp:'callbacks',
