...
<div class="addtit-name"> 店铺页面装修<article>发挥创意装修完美的店铺</article></div>
        ...
        <table class="table">
            ...
            <tbody>
                ...
                <?php if(cv('designer.page.edit')) { ?>
                    <tr>
                        <td colspan="8">
                            <a class='btn btn-primary' href="<?php  echo $this->createPluginWebUrl('designer', array('op' => 'post'))?>"><i class="fa fa-plus"></i> 添加新页面</a>
    ...
    <div class='panel-body' ng-controller="SZ_YIController" style="padding:8px">
           <div class="fe-panel-menu">
                    <div ng-repeat="nav in navs">
                        <nav ng-click="addItem(nav.id)"><div class="comp-icon"><i class="fa {{nav.icon}}"></i></div><span ng-bind="nav.name"></span></nav>
                    </div>
            </div>
        <div class="fe">
            <div class="fe-phone">
        ...
        <div class="fe-save">
            <div class="fe-save-main">
                <div class="fe-save-info">
                    ...
                    <input name="pagename" type="text" style="height: 30px; width: 300px; border: 1px solid #bbb; border-radius: 3px; margin: 4px 10px; outline: none; padding-left: 10px;" placeholder="页面名称：输入页面名称" value="<?php  echo $datas['pagename'];?>"/>
                </div>
...
<script type="text/ng-template" id="edit-area.html"><?php (!empty($this) && $this instanceof WeModuleSite || 1) ? (include $this->template('temp/edit-area', TEMPLATE_INCLUDEPATH)) : (include template('temp/edit-area', TEMPLATE_INCLUDEPATH));?></script>
...
<script>
    ...
    var myModel = angular.module('SZ_YIEditor',['ng.ueditor'], function($compileProvider) {
    ...
    myModel.controller('SZ_YIController', ['$scope', function($scope){
            $scope.navs = [
                {id:'notice', icon:'fa-bullhorn',name:'公告',params:{color:'',bgcolor:'',notice:'',noticehref:'',scroll:'0'}},
                {id:'banner', icon:'fa-bullhorn',name:'轮播',params:{shape:'',align:'center',scroll:'2',bgcolor:''},
                   data:[
                ...
                {id:'title', icon:'fa-bold',name:'标题',params:{title1:'',title2:'',showtitle2:'1',fontsize1:'18px',fontsize2:'14px',align:'left',color:'#000',}},
                {id:'search', icon:'fa-search',name:'搜索框',params:{placeholder:'搜索：输入关键字在店内搜索',style:'style1','color':'','bgcolor':'','bordercolor':'',searchurl:'<?php  echo $this->createMobileUrl("shop/list")?>',uniacid:'<?php  echo $_W["uniacid"];?>'}},
                {id:'line', icon:'fa-bullhorn',name:'辅助线',params:{height:'2px',style:'dashed',color:'#000'}},
                {id:'blank', icon:'fa-square-o',name:'辅助空白',params:{height:'100px',bgcolor:''}},
                {id:'shop', icon:'fa-bookmark',name:'店招',params:{style:'1',bgimg:'../addons/sz_yi/plugin/designer/template/imgsrc/init-data/init-image-1.jpg',logo:'1',name:'1',menu:'1',navcolor:''},
                ...
                {id:'goods', icon:'fa-tasks',name:'商品组',params:{style:'50%',showtitle:'0',titlecolor:'',bgcolor:'',showname:'1',title:'',option:'sale-rx',buysub:'buy-3',price:'1',goodhref:'<?php  echo $this->createMobileUrl("shop/detail")?>'},data:[]},
                {id:'richtext', icon:'fa-font',name:'富文本',params:{bgcolor:'',},content:''},
                {id:'menu', icon:'fa-bullhorn',name:'按钮组',params:{num:'20%',style:'0',bgcolor:'#fff',},
                ...
                {id:'picture', icon:'fa-file-image-o',name:'单图',params:{},data:[{id:'P0000000000001',imgurl:'../addons/sz_yi/plugin/designer/template/imgsrc/init-data/init-image-4.jpg',hrefurl:'',option:'0'}]},
                {id: "cube", icon:'fa-picture-o',name: "图片组合",params: {bgcolor:'',layout: {},showIndex: 0,selection: {},currentPos: {},currentLayout: {isempty: !0}},data:[]},
                <?php  if(p('area')) { ?>
                    {id: "area", icon:'fa-area',name: "商品区域",params: {isarea:'{p("area")}',height:'',showtitle2:'1'},data:['../addons/sz_yi/plugin/designer/template/imgsrc/choose_city.png','24px']}
                <?php  } ?>                
            ];
            ...
            $scope.selectChild = function(cid){
                var category_id = cid;
                $.ajax({
                    type: 'POST',
                    dataType:'json',
                    url: "<?php  echo $this->createPluginWebUrl('designer',array('op'=>'iscategory','level'=>2,))?>",
                    data: {categoryid:category_id},
                    success: function(josn){
                        if(josn.success==1){
                            $('.categorynull2').html('');
                            $('.category2'+category_id).html(josn.html);
                        }
                        else{
                            alert(josn.html);
                        }
            ...
            $(document).on('click','.c_id',function(){
                ...
                $.ajax({
                    type: 'POST',
                    dataType:'json',
                    url: "<?php  echo $this->createPluginWebUrl('designer',array('op'=>'iscategory','level'=>3))?>",
                    data: {categoryid:category_id},
                    success: function(josn){
                        if(josn.success==1){
                            $('.categorynull3').html('');
                            $('.category3'+category_id).html(josn.html);
                        }
                        else{
                            alert(josn.html);
                        }
