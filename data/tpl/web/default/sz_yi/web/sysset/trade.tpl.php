        ...
        <div class="panel panel-default">
            ...
            <div class='panel-body'>
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-5">
                        <?php if(cv('sysset.save.trade')) { ?>
                        <label class='radio-inline'><input type='radio' name='trade[showlastorder]' value='1' <?php  if($set['trade']['showlastorder'] == 1) { ?>checked<?php  } ?>/> 开启</label>
                        <label class='radio-inline'><input type='radio' name='trade[showlastorder]' value='0' <?php  if(empty($set['trade']['showlastorder'])) { ?>checked<?php  } ?> /> 关闭</label>
                        <span class='help-block'>是否显示商城用户下单飘窗提示</span>
                        <?php  } else { ?>
                        <input type="hidden" name="trade[showlastorder]" value="<?php  echo $set['trade']['showlastorder'];?>"/>
                        <div class='form-control-static'><?php  if($set['trade']['showlastorder']==1) { ?>关闭<?php  } else { ?>开启<?php  } ?></div>
                        <?php  } ?>
                    </div>
                </div>
			</div>
            ...
            <div class='panel-body'>
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-9 col-xs-12">
                        <?php if(cv('sysset.save.trade')) { ?>
                        <label class='radio-inline'><input type='radio' name='trade[transfer]' value='0' <?php  if(empty($set['trade']['transfer'])) { ?>checked<?php  } ?>/> 开启</label>
                        <label class='radio-inline'><input type='radio' name='trade[transfer]' value='1' <?php  if($set['trade']['transfer']=='1') { ?>checked<?php  } ?> /> 关闭</label>
                        <span class='help-block'>是否允许用户对账户余额进行转账</span>
                        <?php  } else { ?>
                        <input type="hidden" name="trade[withdraw]" value="<?php  echo $set['trade']['withdraw'];?>"/>
                        <div class='form-control-static'><?php  if($set['trade']['transfer']==1) { ?>关闭<?php  } else { ?>开启<?php  } ?></div>
                        <?php  } ?>
                    </div>
                </div>
                <div class="form-group">
                ...
                <div class='panel-body'>
                    <div class="form-group">
                        ...
                        <div class="col-sm-9 col-xs-12">
                            <?php if(cv('sysset.save.trade')) { ?>
                            <div class='input-group'>
                                <input type="text" name="trade[poundage]" class="form-control" value="<?php  echo $set['trade']['poundage'];?>" onkeyup="value=value.replace(/[^\d.]/g,'');if(value >= 100){value=''}" onafterpaste="value=value.replace(/[^\d.]/g,'');if(value >= 100){value=''"/>
                                <span class='input-group-addon'>%</span>
                            </div>
                            <?php  } else { ?>
                                <input type="hidden" name="trade[poundage]" value="<?php  echo $set['trade']['poundage'];?>"/>
                                <div class='form-control-static'><?php  echo $set['trade']['poundage'];?>%</div>
                            <?php  } ?>
                        </div>
                    </div>
                </div>
                ...
                <div class="form-group">
                    <label class="col-xs-12 col-sm-3 col-md-2 control-label">开启余额提现自动打款</label>
                    <div class="col-sm-9 col-xs-12">
                        <?php if(cv('sysset.save.trade')) { ?>
                        <label class='radio-inline'><input type='radio' name='trade[withdrawnocheck]' value='1' <?php  if($set['trade']['withdrawnocheck']==1) { ?>checked<?php  } ?>/> 开启</label>
                        <label class='radio-inline'><input type='radio' name='trade[withdrawnocheck]' value='0' <?php  if($set['trade']['withdrawnocheck']==0) { ?>checked<?php  } ?> /> 关闭</label>
                        <span class='help-block'>是否允许用户余额提现时无需审核，直接打款。注：只支持微信钱包提现！</span>
                        <?php  } else { ?>
                        <input type="hidden" name="trade[withdraw]" value="<?php  echo $set['trade']['withdraw'];?>"/>
                        <div class='form-control-static'><?php  if($set['trade']['withdrawnocheck']==1) { ?>开启<?php  } else { ?>关闭<?php  } ?></div>
                        <?php  } ?>
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-9 col-xs-12">
                        <?php if(cv('sysset.save.trade')) { ?>
                        <input type="text" name="trade[withdrawautomoney]" class="form-control" value="<?php  echo $set['trade']['withdrawautomoney'];?>" />
                        <span class='help-block'>提现金额少于等于自定义的限制金额，则提现不需要审核，否则需要审核。</span>
                        <?php  } else { ?>
                        <input type="hidden" name="trade[withdrawautomoney]" value="<?php  echo $set['trade']['withdrawautomoney'];?>"/>
                        <div class='form-control-static'>
                            <?php  if(empty($set['trade']['withdrawautomoney'])) { ?>不限制<?php  } else { ?><?php  echo $set['trade']['withdrawautomoney'];?> 元 <?php  } ?>
                        </div>
                        <?php  } ?>
                    </div>
                </div>
            ...
            <div class='panel-body'>
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-9 col-xs-12">
                        ...
                        <label class='radio-inline'><input type='radio' name='trade[is_street]' value='1' <?php  if($set['trade']['is_street']==1) { ?>checked<?php  } ?>/> 开启</label>
                        <label class='radio-inline'><input type='radio' name='trade[is_street]' value='0' <?php  if($set['trade']['is_street']==0) { ?>checked<?php  } ?> /> 关闭</label>
