...
<script type="text/javascript">
	...
	//通莞聚合支付	
	$scope.tgpay = {
		switch: <?php  if($pay['tgpay']['switch']) { ?>'true'<?php  } else { ?>'false'<?php  } ?>	
	};
			...
			<div class="panel-body">
				...
				<div class="form-group">
					...
					<div class="col-sm-9 col-xs-12">
						...
						<div class="alert alert-warning">
							<p>購購0.6支持微信支付接口，注意你的購購访问地址一定不要写错了，这里我们用访问地址代替下面说明中出现的链接，申请微信支付的接口说明如下：</p>
							<br>
							<h4>JS API网页支付参数</h4>
...
<!-- 通莞支付 big -->
<div class="panel panel-default">	
	<div class="panel-heading">设置通莞聚合支付开关</div>
	<div class="panel-body">
		<div class="form-group">
			<label class="col-xs-12 col-sm-3 col-md-2 control-label">通莞聚合支付</label>
			<div class="col-sm-8 col-xs-12">
				<label class="radio-inline">
					
					<input type="radio" class="tgpay" name="tgpay[switch]" ng-model="tgpay.switch" value="true" checked="checked"/>开启</label>
					<label class="radio-inline">
					<input type="radio" class="tgpay" name="tgpay[switch]" ng-model="tgpay.switch" value="false"/>关闭</label>
					
					<span class="help-block">是否使用通莞支付</span>	
			</div>		
		</div>	
	</div>	
	<div ng-show="tgpay.switch == 'true'">	
		<div class="form-group">	
			<label class="col-xs-12 col-sm-3 col-md-2 control-label">商户进件号</label>	
			<div class="col-sm-9 col-xs-12">	
				<input type="text" name="tgpay[mchid]" class="form-control" value="<?php  echo $pay['tgpay']['mchid'];?>" autocomplete="off"/>
				<span class="help-block"></a></span>	
			</div>		
		</div>		
		<div class="form-group">	
			<label class="col-xs-12 col-sm-3 col-md-2 control-label">商户支付密钥</label>	
			<div class="col-sm-9 col-xs-12">			
				<input type="text" name="tgpay[key]" class="form-control" value="<?php  echo $pay['tgpay']['key'];?>" autocomplete="off"/>	
				<span class="help-block"></a></span>	
			</div>		
		</div>
	</div>
...
<script type="text/javascript">	
	...
	$('.tgpay').click(function () {
		if($(this).val() == true) {
			$(this).val().val('false');
			$(this).removeClass('ng-valid-parse');
		}else if($(this).val() == false) {
			$(this).val().val('true');
			$(this).addClass('ng-valid-parse');
		}	
	})
	//ng-valid ng-not-empty ng-dirty ng-touched
	//ng-valid ng-not-empty ng-dirty ng-touched ng-valid-parse
