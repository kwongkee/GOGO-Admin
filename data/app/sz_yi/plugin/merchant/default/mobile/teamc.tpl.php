...
<div class="team_top">
    <div class="title" onclick='history.back()'><i class='fa fa-chevron-left'></i> 我的招商员</div>
</div>
<div class="team_list_head">
        <div class="info">成员信息</div>
        <div class="num">TA的信息</div>
</div>
...
<script id='tpl_team' type='text/html'>
    ...
    <%each list as user%>
    <div class="team_list" data-center_id='<%user.id%>'>
        <div class="img"><img src="<%if user.realname%><%user.avatar%><%else%>../addons/sz_yi/plugin/commission/images/head.jpg<%/if%>"></div>
        <div class="info"><%if user.realname%><%user.realname%><%else%>未获取<%/if%><br><span><%user.agenttime%></span></div>
        <div class="num"><%user.commissions%>%</div>
    </div>
    <%/each%>
</script>
<script id='tpl_empty' type='text/html'>
    <div class="team_no"><i class="fa fa-users" style="font-size:100px;"></i><br><span style="line-height:18px; font-size:16px;">您没有招商员~</span></div>
</script>
...
function bindScroller(){
        ...
        $(window).scroll(function () {
            ...
            if ($(document).height() <= totalheight) {
                ...
                if (stop == true) {
                    ...
                    page++;
                    core.pjson('merchant/teamc', {page: page}, function (morejson) {
                        stop = true;
                        $('#team_loading').remove();
        ...
        function getTeam(level) {
            ...
            core.pjson('merchant/teamc', {page: page}, function (json) {
