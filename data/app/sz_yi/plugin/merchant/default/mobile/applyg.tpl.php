...
<?php  if($set['wx_withdraw'] == 1) { ?>
 <div class="balance_sub balance_sub2 <%if !cansettle%>disabled<%/if%>"  data-type="2">提现到微信钱包</div>
<?php  } ?>
    <div class="balance_sub3" onclick="location.href='<?php  echo $this->createPluginMobileUrl('merchant/logg')?>'">提现记录</div>
</script>
               ...
               $('.balance_sub').click(function(){
                   ...
                   core.tip.confirm('确认要申请提现? 提现申请通过之后给您打款后会通知您到的微信.',function(){
                       ...
                       $('.balance_sub').attr('saving',1).html('正在处理中...');
                        core.pjson('merchant/applyg',{type:type,iscenter:"<?php  echo $_GPC['iscenter'];?>"},function(pjson){
                              if(pjson.status=='1'){
                                   core.tip.show( pjson.result );
                                   location.href = core.getUrl('member/center');
                              }
