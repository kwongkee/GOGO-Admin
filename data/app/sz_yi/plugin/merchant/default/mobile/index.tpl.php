...
<style type="text/css">
    ...
    .order_main .good .info .inner .name {height:38px; width:100%; float:left; font-size:12px; color:#555;overflow:hidden;}
...
<script id='tpl_order_list' type='text/html'>
    ...
    <div class="title">我的订单(总额：<?php  echo $order_total_price?>)</div>
</div>
...
<script id='tpl_empty' type='text/html'>
    <div class="order_no"><i class="fa fa-file-text-o" style="font-size:100px;"></i><br><span style="line-height:18px; font-size:16px;">您还没有订单</span></div>
</script>
<script language='javascript'>

    var page = 1;
    require(['tpl', 'core'], function(tpl, core) {
             core.pjson('merchant/index', {'op':'<?php  echo $_GPC['op'];?>', page:page, status: '<?php  echo $_GPC['status'];?>'}, function(json) {

                    $("#container").html(tpl('tpl_order_list'));

                    $('.nav').click(function() {
                        var status = $(this).data('status');
                        location.href = core.getUrl('plugin/merchant/index', {'op':'order',status: status});
                    })
                    ...
                    /*$('.order_main').click(function(){
                        ...
                        location.href = core.getUrl('plugin/merchant/detail', {orderid: orderid});
                    });*/
                }, true);
...
<style type="text/css">
    ...
    .menu .nav8 {border-bottom:1px solid #f1f1f1; text-align:center;} .menu .nav8 i { color:#ff9901;font-size:30px;}
    .menu .nav9 {border-bottom:1px solid #f1f1f1; text-align:center;} .menu .nav9 i { color:#98cd37;font-size:30px;}
...
<script id='tpl_main' type='text/html'>
    <div class="topbar">
        ...
        <div class="user_info">
            <div class="user_name"><%member.realname%></div>
            <div class="user_name"><?php  echo $center_info['levelinfo']['level_name']?></div>
        </div>
    </div>
    <div class="comm-center">
    <ul>
        <li style='width:25%'><a href="#"><span><?php  echo $commission_ok?></span><br/>可结算金额</a></li>
        <li style='width:25%'><a href="#"><span><?php  echo $commission_total?></span><br/>累计结算金额</a></li>
        <li style='width:25%'><a href="#"><span><?php  echo $center_info['no_apply_money']?></span><br/>累计未提现金额</a></li>
        <li style='width:25%'><a class="recharge" <?php  if($commission_ok<=0 || !empty($center_info['limit_day']) ) { ?> href="javascript:;"<?php  } else { ?>href="<?php  echo $this->createPluginMobileUrl('merchant/applyg', array('iscenter' => 1))?>"<?php  } ?> id='withdraw' ><p style="margin:0" <?php  if($commission_ok<=0 || !empty($center_info['limit_day']) ) { ?> class="disabled" <?php  } ?>>提现</p></a>
        </li>
    </ul>
    </div>
    ...
    <div class="menu">  
        <a <?php  echo "href='".$this->createPluginMobileUrl('merchant/logg', array('iscenter' => 1))."'";?>><div class="nav nav1"><i class="fa  iconfont icon-record" style="background:#BC7BFF;"></i><div class="title">提现记录</div><div class="con"><span><?php  echo $commission_total?></span>元</div></div></a>
        <a <?php  echo "href='".$this->createPluginMobileUrl('merchant/index',array('op' => 'order','type'=>$_GPC['type']))."'";?>><div class="nav nav1"><i class="fa iconfont icon-dingdan" style="background:#F981A7;"></i><div class="title">我的订单</div><div class="con"><span><?php  echo $ordercount;?></span>个订单</div></div></a>
        <a href="<?php  echo $this->createPluginMobileUrl('merchant/team',array('iscenter' => 1))?>"><div class="nav nav1"><i class="fa iconfont icon-hezuotuandui" style="background:#ff6e03;"></i><div class="title">我的招商部</div><div class="con"><span><?php  echo $centercount?></span> 人</div></div></a>
        <a href="<?php  echo $this->createPluginMobileUrl('merchant/teamc')?>"><div class="nav nav1"><i class="fa iconfont icon-hezuotuandui" style="background:#ff6e03;"></i><div class="title">我的招商员</div><div class="con"><span><?php  echo $merchantcount?></span> 人</div></div></a>
    </div>
</script>
<script language="javascript">
    require(['tpl', 'core'], function(tpl, core) {
        core.pjson('merchant/applyg',{iscenter:"1"},function(json){
            ...
            $('#withdraw').click(function(){
                if(!json.result.cansettle){
                    core.tip.show('无可提佣金!');
                    return false;
                }else if (json.result.center_info.limit_day) {
                    core.tip.show('提现周期未到，下次提现时间'+json.result.center_info.last_time);
                    return false;
                }
            });
        },true);
