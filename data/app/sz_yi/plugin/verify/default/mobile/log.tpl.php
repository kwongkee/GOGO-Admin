...
<div class="log_top">
       <div class="title" onclick='history.back()'><i class='iconfont icon-chevron-left'></i> 全部(<span id='total'></span>)</div>
</div>
 
<div class="log_menu">
        <div class="nav navon" style="border-left:0px;width:18%;" data-status=''>数量</div>
        <div class="nav"  data-status='0'>待打款</div>
        <div class="nav"  data-status='1'>已打款</div>
        <div class="nav"  data-status='2'>无效</div>
</div>
...
<script id='tpl_log' type='text/html'>
  ...
  <div class="log_list"
       data-applyid='<%log.id%>'>
    <div class="left"><div class='inner'>编号: <%log.withdraw_no%><br><span>
                <%if log.status==0%>申请时间:<%log.create_time%><%/if%>
                <%if log.status==1%>打款时间:<%log.apply_time%><%/if%>
                <%if log.status==2%>无效时间:<%log.refuse_time%><%/if%>
...
function bindScroller(){
        ...
        $(window).scroll(function () {
            ...
            if ($(document).height() <= totalheight) {
                ...
                if (stop == true) {
                    ...
                    page++;
                    core.pjson('verify/log', {status:current_status,page: page}, function (morejson) {
                        stop = true;
                        $('#log_loading').remove();
        ...
        function getLog(status) {
            ...
            core.pjson('verify/log', {status:status, page: page, storeid:<?php  echo $_GPC['storeid'];?>}, function (json) {
