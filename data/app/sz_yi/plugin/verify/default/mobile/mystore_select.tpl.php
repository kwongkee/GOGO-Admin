...
<link rel="stylesheet" type="text/css" href="../addons/sz_yi/plugin/verify/template/mobile/default/static/css/dish/wei_dialog.css" media="all" />
<link rel="stylesheet" type="text/css" href="../addons/sz_yi/plugin/verify/template/mobile/default/static/css/dish/wei_canyin.css" media="all" />
<link rel="stylesheet" type="text/css" href="../addons/sz_yi/plugin/verify/template/mobile/default/static/css/dish/home.css" media="all">
<script type="text/javascript" src="../addons/sz_yi/plugin/verify/template/mobile/default/static/js/dish/jquery-1.8.3.min.js"></script>
<script type="text/javascript" src="../addons/sz_yi/plugin/verify/template/mobile/default/static/js/dish/wei_webapp_v2_common.js"></script>
    ...
    <dl >
        <dd id="erji" data-cate="<%value.id%>" data-advimg='<%value.advimg%>' data-advurl='<%value.advurl%>' class="parent_item" ><%value.name%>
         ...
         <div class="nav-erj" date-type="1">
            ...
            <!-- 三级分类 -->
                    <?php  if(intval($shopset['shop']['catlevel'])==3) { ?>
...
.right-choose{height: 30px;line-height: 30px;text-align: right;}

</style>
<div class="foot-nav">
  <span id="alltotal">0</span>
  <a class='channel' href="<?php  echo $this->createPluginMobileUrl('verify', array('id' => $_GPC['storeid']))?>">确认</a>
...
<script language='javascript'>
    ...
    require(['tpl', 'core'], function (tpl, core) {
        function setCategory(){
            ...
            core.pjson('verify/select_goods',{pcate:pcate,pageid:pageid,op:'moren',storeid:"<?php  echo $_GPC['storeid'];?>"}, function (json) {
                            result = json.result;
                            ...
                            if (store.length) {                                                             
                                for (var i = 0; i < store.length ;i++ ) {
                                    ...
                                    if (store[i].hasoption == 1) {
                                        h +=  '<div class="right-choose"><input type="text" style="display:none;" class="num" value="0"><a href="javascript:;" class="canoption" data-id="'+store[i].id+'" style="color:<?php  if($color['color']) { ?><?php  echo $color['color'];?><?php  } else { ?>black<?php  } ?>;">可选规格</a></div>';
                                    } else {
        ...
        function setNum(){   
            ...
            $.ajax({
                        url:"<?php  echo $this->createPluginMobileUrl('verify/add')?>", 
                        type:'post',
                        data: {op:'cart', storeid:"<?php  echo $_GPC['storeid'];?>"},
        ...
        function getTotalPrice(){
            require(['core'],function(core){
                ...
                core.pjson('verify/add',{op:'display', storeid:"<?php  echo $_GPC['storeid'];?>"},function(ret){
                     $('#alltotal').html(ret.result.counttotal);
                     $('#totalprice').html('总价：￥' + ret.result.totalprice + ret.result.difference);
        ...
        core.pjson('verify/select_category',{op:'category',level:3,storeid:<?php  echo $_GPC['storeid'];?>}, function (json) {
                 result = json.result;
                 ...
                 $('.parent_item').click(function(){
                     ...
                     cate = $(this).data('cate');
                     //console.log(pcate);
                     core.pjson('verify/select_goods',{'pcate':cate,pageid:pageid,op:'moren', storeid:"<?php  echo $_GPC['storeid'];?>"}, function (json) {
                            result = json.result;                           
                            var h = '';
                    ...
                    $('.parent_item1').click(function(){
                         ...
                         cate = $(this).data('cate');
                         //console.log(pcate);
                         core.pjson('verify/select_goods',{'ccate':cate,pageid:pageid,op:'second', storeid:"<?php  echo $_GPC['storeid'];?>"}, function (json) {
                                result = json.result;
                    ...
                    $('.parent_item2').click(function(){
                         ...
                         cate = $(this).data('cate');
                         //console.log(pcate);
                         core.pjson('verify/select_goods',{'tcate':cate,pageid:pageid,op:'third', storeid:"<?php  echo $_GPC['storeid'];?>"}, function (json) {
                                result = json.result;
                ...
                $('body').on('click','.dataIn',function(){
                    ...
                    <?php  } else { ?>
                      core.pjson('verify/select_goods',{id: id, op: 'getdetail'},function(json){
                ...
                $('body').on('click','.add',function(){
                    ...
                    require(['core'],function(core){
                        ...
                        core.pjson('verify/add',{op:'add', id:id,total:1, storeid:"<?php  echo $_GPC['storeid'];?>"},function(ret){
                ...
                $('body').on('input propertychange','.num',function(){
                    ...
                    require(['core'],function(core){
                        ...
                        core.pjson('verify/add',{op:'add', id:id, total:num, type:'propertychange', storeid:"<?php  echo $_GPC['storeid'];?>"},function(ret){
        ...
        $(document).on('click','.canoption',function(){
          var id = $(this).data('id');
          core.json('shop/detail',{id:id,'ischannelpay':"<?php  echo $_GPC['ischannelpay'];?>",'ischannelpick':"<?php  echo $_GPC['ischannelpick'];?>"},function (json) { 
            ...
            if(ret.no==''){
                ...
                for(var i=0;i<len;i++) {
                 ...
                 core.pjson('verify/add', {id:id, optionid:optionid, op:'gettotal', storeid:"<?php  echo $_GPC['storeid'];?>"}, function(json){
                        ...
                        $('.good_choose').fadeIn(200);
                          ...
                          if(action=='cart'){
                              require(['core'],function(core){
                                ...
                                core.pjson('verify/add',{op:'add', id:'<?php  echo $_GPC['id'];?>',optionid:$('#optionid').val(),total:$('#total').val(), is:'choose', storeid:"<?php  echo $_GPC['storeid'];?>"},function(ret){
                                      if(ret.status==1){
...
//        }
        ...
        $('.spec_items').each(function(){
                 ...
                 var data = {};
                  ...
                  <?php  if(!empty($popenid) or !empty($openid)) { ?>
                       ...
                       core.pjson('verify/add',{op:'add', id:$('#goodsid').val(),optionid:$('#optionid').val(),total:$('#total').val(), is:'choose', storeid:"<?php  echo $_GPC['storeid'];?>"},function(ret){
                                                    if(ret.status==1){ 
                                                       core.tip.show(ret.result.message);
                                                       var cartdiv = $('.cart');
        ...
        if(action=='buy'){
               ...
               <?php  if(!empty($formInfo)) { ?>
                ...
                require(['core'],function(core){
                    core.pjson('shop/detail', data, function(create_json) {
