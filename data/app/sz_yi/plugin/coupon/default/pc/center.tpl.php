...
<div class="page_topbar">
	...
	<div class="title"><?php  if($_W['uniacid'] == 18) { ?>Coupon Collection Center<?php  } else { ?>优惠券领取中心<?php  } ?> </div>
</div>
...
<div class="category_group">
   <div class='container'>
     <span class='item on' data-catid="" onclick="getCoupons('')"><?php  if($_W['uniacid'] == 18) { ?>All coupons<?php  } else { ?>全部优惠券<?php  } ?> </span>
...
<script id='tpl_empty' type='text/html'>
	<div class="coupon_no"><i class="fa fa-credit-card" style="font-size:100px;"></i><br><span style="line-height:18px; font-size:16px;"><?php  if($_W['uniacid'] == 18) { ?>No coupons posted yet<?php  } else { ?>还没有发布优惠券<?php  } ?> ~</span></div>
</script>
...
<div class="coupon_item" onclick="location.href = '<?php  echo $this->createPluginMobileUrl('coupon/detail')?>&id=<%coupon.id%>'">
	...
	<div class="cinfo" >
		<div class="inner" >
			...
			<%else%>
		 <%if coupon.timestr=='1'%>
		          <?php  if($_W['uniacid'] == 18) { ?>Within<?php  } else { ?>即<?php  } ?> <%coupon.gettypestr%><?php  if($_W['uniacid'] == 18) { ?>days<?php  } else { ?>日内<?php  } ?>  <%coupon.timedays%> <?php  if($_W['uniacid'] == 18) { ?>valid for days<?php  } else { ?>天有效<?php  } ?> 
				  <%else%>
			...
			<div class='pay'>
				...
				<%if coupon.getstatus=='0'%><span class="ccreditmoney"><?php  if($_W['uniacid'] == 18) { ?> ¥<%coupon.money%> <?php  } else { ?><%coupon.money%> 元<?php  } ?>  +  <%coupon.credit%> <?php  if($_W['uniacid'] == 18) { ?>integral<?php  } else { ?>积分<?php  } ?> </span><%/if%>
				
				<%if coupon.getstatus=='1'%><?php  if($_W['uniacid'] == 18) { ?><span class="cmoney">¥<%coupon.money%></span><?php  } else { ?><span class="cmoney"><%coupon.money%></span> 元<?php  } ?> <%/if%>
				<%if coupon.getstatus=='2'%><span class="ccredit"><%coupon.credit%></span> <?php  if($_W['uniacid'] == 18) { ?>integral<?php  } else { ?>积分<?php  } ?> <%/if%>
				<%if coupon.getstatus=='3'%><span class="cfree"><?php  if($_W['uniacid'] == 18) { ?>Free collection<?php  } else { ?>免费领取<?php  } ?> </span><%/if%>
					<%if coupon.getmax!=-1 && coupon.getmax!=0%>
				           <?php  if($_W['uniacid'] == 18) { ?>Per person<?php  } else { ?>每人限<?php  } ?>  <%coupon.getmax%> <?php  if($_W['uniacid'] == 18) { ?> <?php  } else { ?>张<?php  } ?> 
					<%/if%>
