...
<script language="javascript">
    ...
    require(['tpl', 'core'], function (tpl, core) {
        ...
        function bindScroller(){
            ...
            $(window).scroll(function () {
                ...
                if ($(document).height() <= totalheight) {
                    ...
                    if (stop == true) {
                        stop = false;
                        $('#container').append('<div id="order_loading"><i class="fa fa-spinner fa-spin"></i> 正在加1载...</div>');
                        page++;
                        core.pjson('supplier/orderj', {'op':'order', status:current_status,page: page}, function (morejson) {
                            stop = true;
        ...
        function getOrder(status) {
            $('.order_topbar .nav').removeClass('on');
            $('.order_topbar .nav[data-status=\'' + status + '\']').addClass('on');
            core.pjson('supplier/orderj', {'op':'order', status:status, page: page}, function (json) {
        ...
        $('.order_topbar .nav').unbind('click').click(function () {
...
<script id='tpl_main' type='text/html'>
    ...
    <div class="top">
        ...
        <div class="top_1">
            ...
            <div class="text">累计提现金额：<?php  echo $commission_total?> 元 订单完成<?php  echo $set['apply_day'];?>天后可提现：<?php  echo $expect_money?> 元 <br>可提现金额（元）（订单完成后可获得）</div>

        </div>

        <div class="top_2"><?php  echo $commission_ok?><a <?php  if($commission_ok<=0 || !empty($supplierinfo['limit_day'])) { ?>href="javascript:;"<?php  } else { ?>href="<?php  echo $this->createPluginMobileUrl('supplier/applyg')?>"<?php  } ?> id='withdraw' ><span <?php  if($commission_ok<=0 || !empty($supplierinfo['limit_day'])) { ?>class='disabled'<?php  } ?> >提现</span></a></div>

    </div>

    <div class="menu">

        <a <?php  echo "href=#";?>><div class="nav nav1"><i class="fa fa-cny fa-3x"></i><div class="title">累计未提现金额</div><div class="con"><span><?php  echo $supplierinfo['costmoney_total']?></span>元</div></div></a>
        ...
        <a <?php  echo "href='".$this->createPluginMobileUrl('supplier/orderj',array('op' => 'order','type'=>$_GPC['type']))."'";?>><div class="nav nav3"><i class="fa fa-list fa-3x"></i><div class="title">我的订单</div><div class="con"><span><?php  echo $ordercount;?></span>个订单</div></div></a>
    ...
    require(['tpl', 'core'], function(tpl, core) {
        ...
        core.pjson('supplier/applyg',{},function(json){
            ...
            $('#withdraw').click(function(){
                ...
                }else if (json.result.supplierinfo.limit_day) {
                    core.tip.show('提现周期未到，下次提现时间'+json.result.supplierinfo.last_time);
                    return false;
                }
