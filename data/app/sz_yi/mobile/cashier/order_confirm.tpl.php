...
<?php  if($store['creditpack'] > 0) { ?>余额奖励:订单总额的<?php  echo $store['creditpack'];?>%<br/><?php  } ?>
<?php  if($store['redpack'] > 0) { ?>红包奖励:订单总额的<?php  echo $store['redpack'];?>%<br/><?php  } ?>
<?php  if($store['credit1'] > 0) { ?>积分奖励:订单总额的<?php  echo $store['credit1'];?>%<br/><?php  } ?>
...
<?php  if($hascouponplugin) { ?>
<?php (!empty($this) && $this instanceof WeModuleSite) ? (include $this->template('coupon/chooser_cashier', TEMPLATE_INCLUDEPATH)) : (include template('coupon/chooser_cashier', TEMPLATE_INCLUDEPATH));?>
<?php  } ?>
<script>
require(['tpl', 'core'], function(tpl, core) {
    ...
    function bindCouponEvents () {
        ...
        $(document).on('click','#selectcoupon',function(){
            ...
            core.pjson('coupon/util', {op: 'query', money: money, type:0, cashier:1, sid:"<?php  echo $sid;?>"}, function (rjson) {
                if (rjson.status != 1) {
